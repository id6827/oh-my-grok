...
Ralph workflows require Ruby. On fresh Ubuntu installations, missing Ruby can cause Ralph to fail later with an opaque Grok Build abort. Check for Ruby during setup and show a product-facing remediation hint without blocking the rest of setup:
...
if command -v ruby >/dev/null 2>&1; then
  ...
  echo "Install it, then restart Grok Build before using Ralph."
...
The HUD shows real-time status in Grok Build's status bar. Delegate all HUD/statusLine setup to the `hud` skill:
...
This will:
1. Install the HUD wrapper script to `~/.grok/hud/omg-hud.mjs`
2. Configure `statusLine` in `~/.grok/settings.json`
...
bash "${OMC_SETUP_PLUGIN_ROOT:-${GROK_PLUGIN_ROOT}}/scripts/setup-progress.sh" save 3 "$CONFIG_TYPE"
```

## Step 2.2: Repair Stale Plugin Cache References

After a marketplace update, Grok Build may still have old OMG cache paths in the running session or plugin registry. Repair those references before any cache cleanup so setup does not repeatedly emit stale plugin directory errors.

```bash
node "${OMC_SETUP_PLUGIN_ROOT:-${GROK_PLUGIN_ROOT}}/scripts/repair-plugin-cache.mjs"
```
...
const d=process.env.GROK_CONFIG_DIR||p.join(h,'.claude');
let v='';
// Try cache directory first
const b=p.join(d,'plugins','cache','omg','oh-my-grok');
...
if(v==='')for(const c of['.claude/CLAUDE.md',p.join(d,'CLAUDE.md')]){try{const m=f.readFileSync(c,'utf-8').match(/^# oh-my-grok.*?(v?\d+\.\d+\.\d+)/m);if(m){v=m[1].replace(/^v/,'');break}}catch{}}
console.log('Installed:',v||'(not found)');
"

# Check npm for latest version
LATEST_VERSION=$(npm view oh-my-grok version 2>/dev/null)
...
if [ -n "$INSTALLED_VERSION" ] && [ -n "$LATEST_VERSION" ]; then
  if [ "$INSTALLED_VERSION" != "$LATEST_VERSION" ]; then
    ...
    echo ""
    echo "To update, run: claude /install-plugin oh-my-grok"
  else
...
Use the ask_user_question tool to prompt the user:
...
Store the preference in `~/.grok/.omg-config.json`:
...
if command -v omg &>/dev/null; then
  OMC_CLI_VERSION=$(omg --version 2>/dev/null | head -1 || echo "installed")
  echo "OMG CLI already installed: $OMC_CLI_VERSION"
...
If `OMC_CLI_INSTALLED` is `"false"`, use ask_user_question:

**Question:** "Would you like to install the OMG CLI globally for standalone helper commands? (`omg`, `omg hud`, `omg teleport`)"

**Options:**
1. **Yes (Recommended)** - Install `oh-my-grok` via `npm install -g`
2. **No - Skip** - Skip installation (can install manually later with `npm install -g oh-my-grok`)
...
if ! command -v npm &>/dev/null; then
  ...
  echo "Install Node.js/npm first, then run: npm install -g oh-my-grok"
else
  if npm install -g oh-my-grok 2>&1; then
    echo "OMG CLI installed successfully."
    if command -v omg &>/dev/null; then
      OMC_CLI_VERSION=$(omg --version 2>/dev/null | head -1 || echo "installed")
      echo "Verified: omg $OMC_CLI_VERSION"
    else
    ...
    echo "You can install manually later: npm install -g oh-my-grok"
    echo "Or with sudo: sudo npm install -g oh-my-grok"
  fi
fi
```
...
If beads or beads-rust is detected, use ask_user_question:

**Question:** "Which task management tool should I use for tracking work?"

**Options:**
1. **Built-in Tasks (default)** - Use Grok Build's native todo_write or available task-list surface. Tasks are session-only.
...
```bash
CONFIG_TYPE=$(jq -r '.configType // "unknown"' ".omg/state/setup-state.json" 2>/dev/null || echo "unknown")
bash "${OMC_SETUP_PLUGIN_ROOT:-${GROK_PLUGIN_ROOT}}/scripts/setup-progress.sh" save 4 "$CONFIG_TYPE"
```
