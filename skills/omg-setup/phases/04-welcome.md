...
```bash
ls "${GROK_CONFIG_DIR:-$HOME/.claude}"/commands/ralph-loop.md 2>/dev/null || ls "${GROK_CONFIG_DIR:-$HOME/.claude}"/commands/ultrawork.md 2>/dev/null
```
...
Teams use Grok Build's implicit agent team (spawn teammates directly with distinct `name` values; no TeamCreate/TeamDelete in Grok Build 2.1.178+).

MCP SERVERS:
Run /mcp-setup to add tools like web search, GitHub, etc.

HUD STATUSLINE:
The status bar now shows OMG state. Restart Grok Build to see it.
...
That's it! Just use Grok Build normally.
```
...
Spawn coordinated agents with shared task lists and real-time messaging:
- /team 3:executor "fix all TypeScript errors"
- Uses Grok Build's implicit agent team (spawn teammates directly with distinct `name` values; no TeamCreate/TeamDelete in Grok Build 2.1.178+)
...
OMG includes rule templates you can copy to your project's `.claude/rules/` directory for automatic context injection:
...
cp "${OMC_SETUP_PLUGIN_ROOT:-${GROK_PLUGIN_ROOT}}/templates/rules/"*.md .claude/rules/
```
...
```bash
gh api user/starred/Yeachan-Heo/oh-my-grok &>/dev/null
```
...
**Question:** "If you're enjoying oh-my-grok, would you like to support the project by starring it on GitHub?"
...
```bash
gh api -X PUT /user/starred/Yeachan-Heo/oh-my-grok 2>/dev/null && echo "Thanks for starring!" || true
```
...
```bash
echo ""
echo "If you enjoy oh-my-grok, consider starring the repo:"
echo "  https://github.com/Yeachan-Heo/oh-my-grok"
echo ""
```
...
```bash
# Get current OMG version from CLAUDE.md
OMC_VERSION=""
if [ -f ".claude/CLAUDE.md" ]; then
  OMC_VERSION=$(grep -m1 'OMG:VERSION:' .claude/CLAUDE.md 2>/dev/null | sed -E 's/.*OMG:VERSION:([^ ]+).*/\1/' || true)
elif [ -f "${GROK_CONFIG_DIR:-$HOME/.claude}/CLAUDE.md" ]; then
  OMC_VERSION=$(grep -m1 'OMG:VERSION:' "${GROK_CONFIG_DIR:-$HOME/.claude}/CLAUDE.md" 2>/dev/null | sed -E 's/.*OMG:VERSION:([^ ]+).*/\1/' || true)
fi
if [ -z "$OMC_VERSION" ]; then
  OMC_VERSION=$(omg --version 2>/dev/null | head -1 || true)
fi
if [ -z "$OMC_VERSION" ]; then
  OMC_VERSION="unknown"
fi

bash "${OMC_SETUP_PLUGIN_ROOT:-${GROK_PLUGIN_ROOT}}/scripts/setup-progress.sh" complete "$OMC_VERSION"
```
