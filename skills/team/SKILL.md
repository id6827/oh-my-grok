---
name: team
description: N coordinated agents on shared task list using Grok Build implicit agent teams
...
User: "/team 3:executor fix all TypeScript errors"
      ...
      [TEAM ORCHESTRATOR (Lead)]
              |
              +-- Use the session's implicit Grok Build team
              ...
              +-- Completion
                      ...
                      -> clear OMG team state (no TeamDelete call)
                      -> rm .omg/state/team-state.json
```

**Native Grok Build team model (2.1.178+):**

```
- No per-team ~/.grok/teams/<name>/ directory is created by this skill.
- No TeamCreate/TeamDelete calls are available.
- `team_name` is accepted by native Grok Build only as ignored legacy metadata; do not rely on it for routing.
- Spawn teammates directly via Agent/Task with `name="worker-N"`.
```

## Goal Workflow Relationship

Team is the OMG authority for parallel, staged execution. Use the deterministic conflict policies `refuse`, `adopt_existing`, and `artifact_only` rather than non-deterministic warning handling. If a task mentions Grok Build `/goal`, Ralph, UltraQA, or artifact-only Ultragoal, keep Team as the primary loop authority unless the leader explicitly hands off. Use `/goal` only as a documented native Grok Build handoff target or as visible evidence from the lead session; do not claim the `/goal` evaluator independently runs commands, reads files, or replaces `team-verify` / `team-fix`. Artifact-only Ultragoal references should be treated as durable goal ledger/checkpoint/evidence artifacts, not as worker execution by themselves.
...
| --------------- | ---------------------------------------- | -------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **team-plan**   | `explore` (LOW), `planner` (HIGH)        | `analyst` (HIGH), `architect` (HIGH)                                                                                 | Use `analyst` for unclear requirements. Use `architect` for systems with complex boundaries.                                                                                                               |
| **team-prd**    | `analyst` (HIGH)                         | `critic` (HIGH)                                                                                                      | Use `critic` to challenge scope.                                                                                                                                                                           |
| **team-exec**   | `executor` (MEDIUM)                      | `executor` (HIGH), `debugger` (MEDIUM), `designer` (MEDIUM), `writer` (LOW), `test-engineer` (MEDIUM)                | Match agent to subtask type. Use `executor` (HIGH) for complex autonomous work, `designer` for UI, `debugger` for compilation issues, `writer` for docs, `test-engineer` for test creation.              |
| **team-verify** | `verifier` (MEDIUM)                      | `test-engineer` (MEDIUM), `security-reviewer` (MEDIUM), `code-reviewer` (HIGH)                                       | Always run `verifier`. Add `security-reviewer` for auth/crypto changes. Add `code-reviewer` for >20 files or architectural changes. `code-reviewer` also covers style/formatting checks.                   |
| **team-fix**    | `executor` (MEDIUM)                      | `debugger` (MEDIUM), `executor` (HIGH)                                                                               | Use `debugger` for type/build errors and regression isolation. Use `executor` (HIGH) for complex multi-file fixes.                                                                                         |

**Model note:** LOW/MEDIUM/HIGH are complexity tiers, not Claude host slugs. Prefer omitting `model` on spawn (inherit) or use only `model="grok-4.5"`. Multi-model tier envs: `OMG_MODEL_LOW` / `OMG_MODEL_MEDIUM` / `OMG_MODEL_HIGH`.
...
3. **Cost mode affects complexity tier.** In downgrade: HIGH → MEDIUM → LOW where quality permits. `team-verify` always uses at least MEDIUM complexity intent.
4. **Risk level escalates review.** Security-sensitive or >20 file changes must include `security-reviewer` + `code-reviewer` (HIGH) in `team-verify`.
...
3. **On team cancellation, handoffs survive** in `.omg/handoffs/` for session resume. They are not deleted by native Grok Build team cleanup; no `TeamDelete` call exists in Grok Build 2.1.178+.
...
- **Cancel:** `/cancel` requests teammate shutdown, waits for responses (best effort), marks phase `cancelled` with `active=false`, captures cancellation metadata, then deletes team resources and clears/preserves Team state per policy. Handoff files in `.omg/handoffs/` are preserved for potential resume.
...
Use the session's implicit Grok Build team. Do **not** call `TeamCreate`; Grok Build 2.1.178+ removed that tool and automatically gives the session one implicit team when `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` is enabled.

Derive a slug such as `fix-ts-errors` for OMG state, prompt labels, handoffs, and human-readable reporting only. Native Grok Build may accept `team_name` as legacy metadata, but it is ignored for routing.
...
| `team_name`      | string  | OMG slug for state, handoffs, and reporting; ignored by native Grok Build routing |
...
Create task list entries for each subtask using todo_write or the active task-list surface. Task-list tools are for tracking only; they do not create native teams.
...
Spawn N teammates directly using the Agent/Task tool with distinct `name` values. Each teammate gets the team worker preamble (see below) plus their specific assignment. Do **not** call `TeamCreate`, and do **not** rely on `team_name`; Grok Build 2.1.178+ ignores it for native routing.
...
- Teammate is spawned into the session's implicit Grok Build team
...
2. **Task-list polling/review** -- Periodically check todo_write or the active task-list surface for overall progress:
   ```
...
1. **Verify results** -- Check that all real tasks (non-internal) are marked `completed` in todo_write or the active task-list surface
...
4. **Clean up native team state** -- Grok Build 2.1.178+ has no `TeamDelete`; after teammates acknowledge shutdown, clear OMG state and any local task bookkeeping.
...
1. CLAIM: Check todo_write or the active task-list surface for tasks assigned to you (owner = "{worker_name}").
...
5. NEXT: Check todo_write or the active task-list surface for more assigned tasks. If you have more pending tasks, go to step 1.
...
Check todo_write or the active task-list surface periodically to see if blockers have been resolved.
...
- `claude_worker`: Emphasize strict todo_write/task-list updates, active team/conversation messages, and no orchestration commands.
...
**CRITICAL: Steps must execute in exact order. Never clear OMG team state before shutdown is confirmed or timed out.**

**Step 1: Verify completion**

```
Verify via todo_write or the active task-list surface — all real tasks (non-internal) are completed or failed.
```
...
After approval, the teammate terminates or stops accepting new work. Grok Build 2.1.178+ does not expose per-team config membership or TeamDelete cleanup; track acknowledgements in OMG state/reporting instead.

**Step 4: Clear OMG team state — only after ALL teammates confirmed or timed out**

Grok Build 2.1.178+ has no `TeamDelete`. Clear OMG team state and local task bookkeeping after the blocking shutdown pass completes.

**Step 5: Orphan scan for OMG tmux/CLI workers only**

For legacy OMG tmux/CLI worker runs (`omg team` / `/omg-teams`), check for worker processes that survived cleanup:

```bash
node "${GROK_PLUGIN_ROOT}/scripts/cleanup-orphans.mjs" --team-name fix-ts-errors
```

This scans for OMG worker processes matching the team name and terminates stale orphans (SIGTERM → 5s wait → SIGKILL). Supports `--dry-run` for inspection.

**Shutdown sequence is BLOCKING:** Do not clear OMG team state until all teammates have either:
...
| --------------- | ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `claude_worker` | Claude agent           | Full Grok Build tool access (Read/file-edit/Bash/Task). Best for tasks needing Claude's reasoning + iterative tool use.                                                                  |
...
- They cannot use Grok Build's native task-list or team messaging surfaces
...
1. Spawn `spawn_subagent(subagent_type="planner", ...)` with task description + codebase context
...
This approach complements native team/conversation messaging by providing a pull-based mechanism for MCP workers that cannot use Grok Build's team messaging tools.
...
1. Lead detects via todo_write or the active task-list surface -- task stuck in `in_progress` for too long
...
3. If needed, spawn a replacement teammate with `spawn_subagent(name="worker-N", subagent_type="...")`
...
If the lead crashes mid-run, the team skill should detect existing OMG state and resume:

1. Read `state_read(mode="team")` for the active OMG team slug, phase, and worker labels
2. Use OMG handoffs and task-list/todo_write state to determine current progress
...
| Aspect                  | Team (Native Grok Build 2.1.178+)                              | Swarm (Legacy SQLite)                  |
| ----------------------- | ---------------------------------------------------------------- | -------------------------------------- |
| **Storage**             | OMG state/handoffs plus Grok Build's current task-list surface   | SQLite in `.omg/state/swarm.db`        |
| **Dependencies**        | `better-sqlite3` not needed                                      | Requires `better-sqlite3` npm package  |
| **Task claiming**       | Lead pre-assigns named workers through task-list/todo_write state  | SQLite IMMEDIATE transaction -- atomic |
...
| **Task dependencies**   | Lead-managed dependencies in task-list/todo_write state            | Not supported                          |
| **Heartbeat**           | Lead detects via missing messages/status                          | Manual heartbeat table + polling       |
| **Shutdown**            | Graceful request/response protocol plus OMG state clear           | Signal-based termination               |
| **Agent lifecycle**     | Tracked by named Agent/Task spawns and OMG state                  | Manual tracking via heartbeat table    |
| **Progress visibility** | Task list/todo_write state with named worker ownership             | SQL queries on tasks table             |
...
| **State cleanup**       | Clear OMG team state after teammate shutdown                      | Manual `rm` of SQLite database         |

**When to use Team over Swarm:** Always prefer `/team` for new native Grok Build work. It uses Grok Build's implicit agent team, requires no external dependencies, supports inter-agent coordination, and has task dependency management.
...
If teammates are unresponsive, record the timeout, avoid spawning more work, and clear OMG state only after the shutdown wait completes or the user force-cancels.
...
Optional settings live in `.claude/omg.jsonc` (project) or `~/.config/claude-omg/config.jsonc` (user). Project values override user values; `OMC_TEAM_ROLE_OVERRIDES` (env JSON) supersedes both.
...
- **ops.monitorIntervalMs** - How often to review todo_write or the active task-list surface (default: 30s)
...
| `planner`       | claude          | `HIGH` → `grok-4.5` / `OMG_MODEL_HIGH` today  |
| `analyst`       | claude          | `HIGH` → `grok-4.5` / `OMG_MODEL_HIGH` today  |
| `executor`      | claude          | `MEDIUM` → `grok-4.5` / `OMG_MODEL_MEDIUM`    |
...
An empty `team.roleRouting` preserves pre-patch behavior: every worker is Claude/Grok-session default, model tiers follow `routing.tierModels` / `mapModel` (all default to `grok-4.5` on Grok Build today), and `/team 3:executor ...` spawns three MEDIUM-complexity executors.

## State Cleanup

On successful completion:

1. Native Grok Build 2.1.178+ has no per-team `TeamDelete` cleanup. After shutdown is confirmed or timed out, clear OMG state via MCP tools:
   ```
   ...
   ```
2. For legacy OMG tmux/CLI workers, run the documented `omg team shutdown` / cleanup path.
3. Or run `/cancel` which handles OMG state cleanup automatically.

**IMPORTANT:** Clear OMG team state only AFTER all teammates have been shut down or timed out.
...
1. **Internal/lifecycle task entries may pollute task-list output** -- If Grok Build reports internal lifecycle entries for spawned teammates, filter them when counting real task progress. The subject of an internal task is often the teammate's name.

2. **No atomic claiming** -- Unlike SQLite swarm, native task-list/todo_write state does not provide transactional claiming. Two teammates could race to claim the same task. **Mitigation:** The lead should pre-assign owners before spawning teammates. Teammates should only work on tasks assigned to them.
...
4. **No TeamDelete cleanup** -- Grok Build 2.1.178+ removed `TeamDelete`; use shutdown messages plus OMG state cleanup.
...
7. **Shutdown acknowledgements are state/reporting events** -- After a teammate approves shutdown and terminates, track that acknowledgement in OMG state/reporting. Do not expect a Grok Build team membership config to update.
