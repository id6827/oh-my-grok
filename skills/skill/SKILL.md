...
Meta-skill for managing oh-my-grok skills via CLI-like commands.
...
**Built-in skill note:** Built-in skills are bundled with oh-my-grok and are discoverable/readable, but not removed or edited through `/skill remove` or `/skill edit`.
...
5. **Ask for scope:**
   - `user` → `${GROK_CONFIG_DIR:-~/.claude}/skills/omg-learned/<name>/SKILL.md`
...
✓ Created skill at ~/.grok/skills/omg-learned/custom-logger/SKILL.md
...
1. **Search for skill** in both scopes:
   - `${GROK_CONFIG_DIR:-~/.claude}/skills/omg-learned/<name>/SKILL.md`
   - `.omg/skills/<name>/SKILL.md`
2. **If found:**
   - Display skill info (name, description, scope)
   - **Ask for confirmation:** "Delete '<name>' skill from <scope>? (yes/no)"
3. **If confirmed:**
   - Delete entire skill directory (e.g., `${GROK_CONFIG_DIR:-~/.claude}/skills/omg-learned/<name>/`)
...
Assistant: Found skill 'old-logger' in user scope:
  ...
  Scope: user (~/.grok/skills/omg-learned/)
...
```
Skill: custom-logger
Scope: user (~/.grok/skills/omg-learned/custom-logger/)
Description: Enhanced logging with structured output
Triggers: log, logger, logging
Argument Hint: <level> [message]
Quality: 95% (if available)
Usage Count: 42 times (if available)
File Path: /home/user/.claude/skills/omg-learned/custom-logger/SKILL.md
...
Description: Enhanced logging with structured output
Triggers: log, logger, logging
File: ~/.grok/skills/omg-learned/custom-logger/SKILL.md
...
**Behavior:**
1. **Scan both scopes:**
   - User skills: `${GROK_CONFIG_DIR:-~/.claude}/skills/omg-learned/`
   - Project skills: `.omg/skills/`
...
USER_SKILLS_DIR="${GROK_CONFIG_DIR:-$HOME/.claude}/skills/omg-learned"
...
echo "=== USER-LEVEL SKILLS (~/.grok/skills/omg-learned/) ==="
if [ -d "${GROK_CONFIG_DIR:-$HOME/.claude}/skills/omg-learned" ]; then
  USER_COUNT=$(find "${GROK_CONFIG_DIR:-$HOME/.claude}/skills/omg-learned" -name "*.md" 2>/dev/null | wc -l)
  echo "Total skills: $USER_COUNT"

  if [ $USER_COUNT -gt 0 ]; then
    echo ""
    echo "Skills found:"
    find "${GROK_CONFIG_DIR:-$HOME/.claude}/skills/omg-learned" -name "*.md" -type f -exec sh -c '
      FILE="$1"
...
After scanning, use the ask_user_question tool to offer these options:
...
Then ask for scope:
- **User-level** (~/.grok/skills/omg-learned/) - Available across all projects
...
- `/skillify` - Extract a skill from current conversation (`/learner` is a deprecated alias)
...
✓ User skills directory exists: ~/.grok/skills/omg-learned/
...
2. **File Operations:** Use read_file/Write tools, never Edit for new files
