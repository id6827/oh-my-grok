...
Configure Model Context Protocol (MCP) servers to extend Grok Build's capabilities with external tools like web search, file system access, and GitHub integration.

## Overview

MCP servers provide additional tools that Grok Build agents can use. This skill helps you configure popular MCP servers using the `claude mcp add` command-line interface.

## Step 1: Choose a Setup Path

Use **ask_user_question** with **one question at a time** and **no more than 3 options per question**. Recent Grok Build builds reject larger option payloads as invalid tool parameters, so keep the MCP selection flow staged.
...
Ask a follow-up **ask_user_question**:
...
If the user chooses **More server choices**, ask one more **ask_user_question**:
...
NEXT STEPS:
1. Restart Grok Build for changes to take effect
...
- Add more servers: /mcp-setup or `claude mcp add ...`
...
Then point OMG at the full tool name in `.claude/omg.jsonc` or `~/.config/claude-omg/config.jsonc`:
...
- Restart Grok Build after configuration
