...
<Tool_Usage>
- Use `spawn_subagent(subagent_type="architect", ...)` for Phase 4 architecture validation
- Use `spawn_subagent(subagent_type="security-reviewer", ...)` for Phase 4 security review
- Use `spawn_subagent(subagent_type="code-reviewer", ...)` for Phase 4 quality review
- Agents form their own analysis first, then spawn Grok subagents for cross-validation
...
Optional settings in `.claude/omg.jsonc` (project) or `~/.config/claude-omg/config.jsonc` (user):
...
```sh
omg team 1:cursor "<implementation task>"
```

or the Grok Build slash compatibility surface:
