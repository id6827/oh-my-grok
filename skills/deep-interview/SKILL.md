...
- Once ready, do **not** bridge into `omg-plan`, `autopilot`, `ralph`, `team`, or the hard-deprecated `omg autoresearch` CLI. Instead write the mission/evaluator setup artifacts and invoke:
  - `skill("/autoresearch")`
...
If this skill is loaded through a qualified plugin path or `skill("/deep-interview")`, do not treat that as permission to skip OMG setup. The user-facing preferred invocation is `/deep-interview`. Regardless of invocation path, Phase 0 below remains blocking and must resolve `omg.deepInterview.ambiguityThreshold` from settings (`.grok/omg.jsonc` or documented defaults) before any announcement, state write, question, or ambiguity score.
...
If any prompt input is too large, summarize it first and then continue from the summary. Do not ask the next `ask_user_question`, score ambiguity, or hand off to execution from an over-budget raw transcript.
...
Use `ask_user_question` with the generated question. Present it clearly with the current ambiguity context:
...
**Autoresearch override:** if `--autoresearch` is active, skip the standard execution options below. The only valid bridge is the `skill("/autoresearch")` handoff described above. The `omg autoresearch` CLI is a hard-deprecated shim and must not be used for execution.

After the spec is written, mark it `pending approval` and present execution options via `ask_user_question`. Until the user selects an execution option, the deep-interview module MUST NOT run mutation-oriented shell commands, edit source files, commit, push, open PRs, invoke execution skills, or delegate implementation tasks:
...
- [ ] Execution bridge presented via ask_user_question
- [ ] Selected execution mode invoked via Skill() only after explicit execution approval (never direct implementation)
- [ ] If 3-stage pipeline selected: omg-plan --consensus --direct invoked, then stopped with the consensus plan marked `pending approval` until the user explicitly approves execution
...
Optional settings in `.grok/config.toml`:
...
**Execution is a separate approval-gated step.** The deep-interview and omg-plan skills must not auto-invoke autopilot, team, ralph, or any other execution skill merely because a spec or plan exists.
...
```
Vague prompt → ralplan gate → deep-interview (if extremely vague) → omg-plan (with clear spec) → pending approval → explicitly approved execution
```
