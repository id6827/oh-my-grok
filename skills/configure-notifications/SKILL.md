...
- If no provider is specified, use ask_user_question:
...
This is an interactive, natural-language configuration skill. Walk the user through setup by asking questions with ask_user_question. Write the result to `${GROK_CONFIG_DIR:-~/.claude}/.omg-config.json`.
...
To reconfigure: /configure-notifications telegram
To configure Discord: /configure-notifications discord
To configure Slack: /configure-notifications slack
```
...
To reconfigure: /configure-notifications discord
To configure Telegram: /configure-notifications telegram
To configure Slack: /configure-notifications slack
```
...
To reconfigure: /configure-notifications slack
To configure Discord: /configure-notifications discord
To configure Telegram: /configure-notifications telegram
```
...
Check if `${GROK_CONFIG_DIR:-~/.claude}/omc_config.hook.json` exists. If it does, show the current configuration. If not, explain what it does.
...
Read or create `${GROK_CONFIG_DIR:-~/.claude}/omc_config.hook.json` and merge the new settings:
...
- `/configure-openclaw` — Configure OpenClaw gateway integration
...
```bash
LEGACY_CONFIG="${GROK_CONFIG_DIR:-$HOME/.claude}/omc_config.openclaw.json"
