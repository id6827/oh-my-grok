...
`state_get_status`) may be registered as **deferred tools** by Grok Build. Before calling
any state tool, you MUST first load all of them via `ToolSearch`:

```
ToolSearch(query="select:mcp__plugin_oh-my-grok_t__state_clear,mcp__plugin_oh-my-grok_t__state_read,mcp__plugin_oh-my-grok_t__state_write,mcp__plugin_oh-my-grok_t__state_list_active,mcp__plugin_oh-my-grok_t__state_get_status")
```
...
4. Team artifacts (`~/.grok/teams/*/`, `~/.grok/tasks/*/`, `.omg/state/team-state.json`) are best-effort cleared as part of the legacy fallback.
...
#### If Team Active (Grok Build implicit team)

Teams are detected through OMG team state, not removed Grok Build `~/.grok/teams` config directories:

```bash
# Check for active OMG team state
...
```
For the active OMG team state:
  1. Read team_name and worker labels from OMG state/handoffs/task bookkeeping
...
After graceful pass:
  ...
  3. Do not call TeamDelete; Grok Build 2.1.178+ removed per-team native cleanup
```
...
```
  ...
  3. Run OMG tmux/CLI orphan scan only for legacy `omg team` / `/omg-teams` workers (see below)
  4. Emit structured cancel report
```

**Orphan Detection (Post-Cleanup):**

For legacy OMG tmux/CLI worker runs, verify no worker processes remain:
```bash
node "${GROK_PLUGIN_ROOT}/scripts/cleanup-orphans.mjs" --team-name "{team_name}"
```

The orphan scanner:
1. Checks `ps aux` (Unix) or `tasklist` (Windows) for OMG worker processes with `--team-name` matching the cleaned-up team
...
Team "{team_name}" cancelled:
  ...
  - OMG state cleared: yes/no
  - Manual cleanup needed: yes/no
    Path: OMG team state / tmux worker processes, if any
```

**Implementation note:** The cancel skill is executed by the LLM, not as a bash script. When you detect an active team:
1. Read `state_read(mode="team")` to find the active OMG team
...
5. Team artifacts (`~/.grok/teams/*/`, `~/.grok/tasks/*/`, `.omg/state/team-state.json`) remain best-effort cleanup items invoked during the legacy/global pass.
...
- **Team-aware**: Detects native Grok Build teams and performs graceful shutdown
