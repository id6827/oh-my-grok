...
role: config-writer  # DOCUMENTATION ONLY - This skill writes to ~/.grok/ paths
scope: ~/.grok/**  # DOCUMENTATION ONLY - Allowed write scope
---
...
| Path | Purpose |
|------|---------|
| `.omg/state/hud-status.txt` | Snapshot line(s) |
| `.omg/state/hud-state.json` | Structured modes |
| `.omg/state/subagent-tracking.json` | Agent rows for HUD |
...
1. Check if `~/.grok/hud/omg-hud.mjs` exists
2. Check if `statusLine` is configured in `~/.grok/settings.json`
3. If missing, create the HUD wrapper script and configure settings
4. Report status and prompt to restart Grok Build if changes were made

**IMPORTANT**: If the argument is `setup` OR if the HUD script doesn't exist at `~/.grok/hud/omg-hud.mjs`, you MUST create the HUD files directly using the instructions below.

### Setup Instructions (Run These Commands)

**Step 1:** Check if setup is needed:
```bash
node -e "const p=require('path'),f=require('fs'),d=process.env.GROK_CONFIG_DIR||p.join(require('os').homedir(),'.claude');console.log(f.existsSync(p.join(d,'hud','omg-hud.mjs'))?'EXISTS':'MISSING')"
```

**Step 2:** Verify the plugin is installed:
```bash
node -e "const p=require('path'),f=require('fs'),d=process.env.GROK_CONFIG_DIR||p.join(require('os').homedir(),'.claude'),b=p.join(d,'plugins','cache','omg','oh-my-grok');try{const v=f.readdirSync(b).filter(x=>/^\d/.test(x)).sort((a,c)=>a.localeCompare(c,void 0,{numeric:true}));if(v.length===0){console.log('Plugin not installed - run: /plugin install oh-my-grok');process.exit()}const l=v[v.length-1],h=p.join(b,l,'dist','hud','index.js');console.log('Version:',l);console.log(f.existsSync(h)?'READY':'NOT_FOUND - try reinstalling: /plugin install oh-my-grok')}catch{console.log('Plugin not installed - run: /plugin install oh-my-grok')}"
```
...
```bash
HUD_DIR="${GROK_CONFIG_DIR:-$HOME/.claude}/hud"
mkdir -p "$HUD_DIR/lib"
cp "${GROK_PLUGIN_ROOT}/scripts/lib/hud-wrapper-template.txt" "$HUD_DIR/omg-hud.mjs"
cp "${GROK_PLUGIN_ROOT}/scripts/lib/config-dir.mjs" "$HUD_DIR/lib/config-dir.mjs"
```
...
```bash
node -e "if(process.platform==='win32'){console.log('Skipped (Windows)')}else{require('fs').chmodSync(require('path').join(process.env.GROK_CONFIG_DIR||require('path').join(require('os').homedir(),'.claude'),'hud','omg-hud.mjs'),0o755);console.log('Done')}"
```

**Step 5:** Update settings.json to use the HUD:

Read `${GROK_CONFIG_DIR:-~/.claude}/settings.json`, then update/add the `statusLine` field.
...
```bash
node -e "const p=require('path').join(require('os').homedir(),'.claude','hud','omg-hud.mjs').split(require('path').sep).join('/');console.log(JSON.stringify(p))"
```

**IMPORTANT:** The command path MUST use forward slashes on all platforms. Grok Build executes statusLine commands via bash, which interprets backslashes as escape characters and breaks the path.
...
```json
{
  "statusLine": {
    "type": "command",
    "command": "node C:/Users/username/.claude/hud/omg-hud.mjs"
  }
}
```
...
```bash
node -e "const p=require('path'),f=require('fs'),d=process.env.GROK_CONFIG_DIR||p.join(require('os').homedir(),'.claude'),t=p.join(d,'hud','omg-hud.js');try{if(f.existsSync(t)){f.unlinkSync(t);console.log('Removed legacy omg-hud.js')}else{console.log('No legacy script found')}}catch{}"
```

**Step 7:** Tell the user to restart Grok Build for changes to take effect.
...
```
[OMG] repo:oh-my-grok branch:main | ralph:3/10 | US-002 (2/5) | ultrawork | ctx:[████░░]67% | agents:3 | bg:3/5 | todos:2/5
...
HUD config is stored in `~/.grok/settings.json` under the `omcHud` key (or your custom config directory if `GROK_CONFIG_DIR` is set).

Legacy config location (deprecated): `~/.grok/.omg/hud-config.json`
...
2. Restart Grok Build after setup completes
3. If still not working, run `/omg-doctor` for full diagnostics

**Legacy string format migration:** Older OMG versions wrote `statusLine` as a plain string (e.g., `"~/.grok/hud/omg-hud.mjs"`). Modern Grok Build (v2.1+) requires an object format. Running the installer or `/hud setup` will auto-migrate legacy strings to the correct object format:
```json
{
  "statusLine": {
    "type": "command",
...
Manual verification:
- HUD script: `~/.grok/hud/omg-hud.mjs`
- Settings: `~/.grok/settings.json` should have `statusLine` configured as an object with `type` and `command` fields
