...
description: Remind the user to rebuild OMG after editing TypeScript when running from a local fork. Triggered automatically by the AI whenever it notices it (or the user) just changed a src/**/*.ts file in an OMG dev install.
---

# Local Build Reminder

**Always-on reminder for OMG fork development.** When OMG is running in local
mode (HUD shows `[OMG#X.Y.ZL]` with an `L` suffix), Grok Build loads compiled
...
3. The user is about to restart Grok Build and the working tree has TS edits with no rebuild.
4. The user runs an OMG command and expects new behavior tied to a TS edit.
...
> Grok Build — `dist/` won't reflect the change otherwise.
...
- The user is in a Grok Build session that isn't running OMG locally
  (no `L` in the HUD).
...
triggers a rebuild within a second, so `restart Grok Build` is all that's
needed afterwards.

## Detection signal — how the AI knows it's "local mode"

The HUD's `[OMG#X.Y.ZL]` suffix is the visible cue. Programmatically, the
...
When running inside the OMG fork repo itself, the AI is by definition in
