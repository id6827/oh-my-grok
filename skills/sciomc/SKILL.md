...
```
/sciomc AUTO: Comprehensive security analysis of the authentication system
...
| Command | Action |
|---------|--------|
| `/sciomc status` | Show current session progress |
| `/sciomc resume` | Resume most recent interrupted session |
| `/sciomc resume <session-id>` | Resume specific session |
| `/sciomc list` | List all sessions with status |
| `/sciomc report <session-id>` | Generate/regenerate report |
| `/sciomc cancel` | Cancel current session (preserves state) |
