...
2. Claude runs via CLI (skill nesting not supported):
   ...
   - `omg ask antigravity "<antigravity prompt>"`
     (or `omg ask gemini "<gemini prompt>"` for enterprise)
...
> **Note:** Skill nesting (invoking a skill from within an active skill) is not supported in Grok Build. Always use the direct CLI path via Bash tool.
...
.omg/artifacts/ask/antigravity-*.md
...
```bash
/ccg Review this PR - architecture/security via Codex and UX/readability via Antigravity
```
