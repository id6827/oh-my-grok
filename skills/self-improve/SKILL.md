...
You are the **loop controller** for the self-improvement system. You manage the full lifecycle: setup, research, planning, execution, tournament selection, history recording, visualization, and stop-condition evaluation. You delegate to specialized OMG agents and coordinate their inputs and outputs.
...
OMG mode lifecycle: `.omg/state/sessions/{sessionId}/self-improve-state.json`
...
All git operations happen inside the target repo, NOT in the OMG project root.
...
- **Worktree setup** (SKILL.md creates before each executor):
  ...
  ```
- **Winner merges** via `oh-my-grok:git-master`:
  ```
...
**6a. Architecture Review**: Spawn `oh-my-grok:architect` with the plan + 6-point checklist:
...
**6b. Critic Review**: Spawn `oh-my-grok:critic` with the plan + harness rules:
...
4. **Ranked-candidate loop** — for each candidate in rank order (best first):
   ...
   b. **Merge** via `oh-my-grok:git-master`: `git merge experiment/round_{n}_executor_{id} --no-ff -m "Iteration {n}: {hypothesis} (score: {before} → {after})"`
...
4. Print summary report:
   ...
   ```
5. Run `/cancel` for clean state cleanup
