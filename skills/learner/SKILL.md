...
- **User-level**: `${GROK_CONFIG_DIR:-~/.claude}/skills/omg-learned/<skill-name>.md` - Rare. Only for truly portable insights.
- **Project-level**: `.omg/skills/<skill-name>.md` - Default. Intended to be committed with the repo when you want the team to keep the skill. In linked worktrees, uncommitted skills are still worktree-local and disappear if that worktree is deleted.
