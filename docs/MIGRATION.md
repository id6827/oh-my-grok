...
This guide covers all migration paths for oh-my-grok. Find your current version below.
...
2. Update skills/prompts from `/omg-teams ...` to `omg team ...` syntax.
...
| -------------------- | -------------------------------------- |
| `cancel-autopilot`   | `/oh-my-grok:cancel`             |
| `cancel-ralph`       | `/oh-my-grok:cancel`             |
| `cancel-ultrawork`   | `/oh-my-grok:cancel`             |
| `cancel-ultraqa`     | `/oh-my-grok:cancel`             |
| `omg-default`        | `/oh-my-grok:omg-setup --local`  |
| `omg-default-global` | `/oh-my-grok:omg-setup --global` |
| `planner`            | `/oh-my-grok:plan`               |
...
```bash
/oh-my-grok:cancel-ralph      # Cancel ralph specifically
/oh-my-grok:omg-default       # Configure local project
/oh-my-grok:planner "task"    # Start planning
```

**After v3.5.3:**

```bash
/oh-my-grok:cancel            # Auto-detects and cancels any active mode
/oh-my-grok:omg-setup --local # Configure local project
/oh-my-grok:plan "task"       # Start planning (includes interview mode)
```

### New Features

**New skill: `/learn-about-omg`**

- Analyzes your OMG usage patterns
...
```bash
/oh-my-grok:plan --consensus "task"  # Iterative planning with Critic review
/oh-my-grok:ralplan "task"           # Alias for plan --consensus
```
...
3. **Re-run `/omg-setup`** if you want to update your CLAUDE.md configuration
...
**Before 3.0:** Explicitly invoke 25+ commands like `/oh-my-grok:ralph "task"`, `/oh-my-grok:ultrawork "task"`

**After 3.0:** Just work naturally - Claude auto-activates the right behaviors. One-time setup: just say "setup omg"

### Project Rebrand

The project was rebranded to better reflect its purpose and improve discoverability.

- **Project/brand name**: `oh-my-grok` (GitHub repo, plugin name, commands)
- **npm package name**: `oh-my-grok` (unchanged)

> **Why the difference?** The npm package name `oh-my-grok` was kept for backward compatibility with existing installations. The project, GitHub repository, plugin, and all commands use `oh-my-grok`.
...
/oh-my-grok:ralph "implement user authentication"       # Persistence mode
/oh-my-grok:ultrawork "refactor the API layer"          # Maximum parallelism
/oh-my-grok:planner "plan the new dashboard"            # Planning interview
/oh-my-grok:deepsearch "find database schema files"     # Deep search
/oh-my-grok:git-master "commit these changes"           # Git expertise
/oh-my-grok:deepinit ./src                              # Index codebase
/oh-my-grok:analyze "why is this test failing?"         # Deep analysis
```
...
No directory rename is required for the current OMG state paths. Keep existing `.omg/` project state and `~/.omg/` global state directories in place.

Only update genuinely legacy or custom paths that predate the OMG layout:

| Area | Old | New |
| ---- | --- | --- |
| Config file | `~/.grok/omg/mnemosyne.json` | `~/.grok/omg/learner.json` |
...
| -------------------------------------- | -------------------------------------------------- | ---------------------- |
| `/oh-my-grok:ralph "task"`       | Say "don't stop until done" OR use `ralph` keyword | ✅ YES (both ways)     |
| `/oh-my-grok:ultrawork "task"`   | Say "fast" or "parallel" OR use `ulw` keyword      | ✅ YES (both ways)     |
| `/oh-my-grok:ultrawork-ralph`    | Say "ralph ulw:" prefix                            | ✅ YES (keyword combo) |
| `/oh-my-grok:planner "task"`     | Say "plan this" OR use `plan` keyword              | ✅ YES (both ways)     |
| `/oh-my-grok:plan "description"` | Start planning naturally                           | ✅ YES                 |
| `/oh-my-grok:review [path]`      | Invoke normally                                    | ✅ YES (unchanged)     |
| `/oh-my-grok:deepsearch "query"` | Say "find" or "search"                             | ✅ YES (auto-detect)   |
| `/oh-my-grok:analyze "target"`   | Say "analyze" — routes to debugger/architect agent | ✅ YES (keyword route) |
| `/oh-my-grok:deepinit [path]`    | Invoke normally                                    | ✅ YES (unchanged)     |
| `/oh-my-grok:git-master`         | Say "git", "commit", "atomic commit"               | ✅ YES (auto-detect)   |
| `/oh-my-grok:frontend-ui-ux`     | Say "UI", "styling", "component", "design"         | ✅ YES (auto-detect)   |
| `/oh-my-grok:note "content"`     | Say "remember this" or "save this"                 | ✅ YES (auto-detect)   |
| `/oh-my-grok:cancel-ralph`       | Say "stop", "cancel", or "abort"                   | ✅ YES (auto-detect)   |
| `/oh-my-grok:omg-doctor`         | Invoke normally                                    | ✅ YES (unchanged)     |
...
No more `/oh-my-grok:cancel-ralph` - just say "cancel"!
...
> **Note**: npm/bun global installs no longer provide the in-session plugin surface by themselves. Use the plugin system for slash commands, hooks, and skills; use the published npm package `oh-my-grok` when you need the terminal `omg` CLI.

#### 3. Preserve Existing OMG Directories

Do not rename current OMG directories. Existing project state in `.omg/` and global state in `~/.omg/` are already on the current paths.
...
```bash
mv ~/.grok/omg/mnemosyne.json ~/.grok/omg/learner.json
```

#### 5. Review Scripts and Configurations

Search your local scripts and docs for stale references to removed commands or the old config filename. Keep the npm package name as `oh-my-grok` for npm/bun installs; do not rewrite it to the project/plugin brand name.

#### 6. Run One-Time Setup

In Grok Build, just say "setup omg", "omg setup", or any natural language equivalent.
...
3. **Test a simple command**:
   Run `/oh-my-grok:omg-help` in Grok Build to ensure the plugin is loaded correctly.
...
```
Before: "OK, I need to use /oh-my-grok:ultrawork for speed..."
...
#### 3. Learned Skills (`/oh-my-grok:skillify`)

Extract reusable insights from problem-solving. `/oh-my-grok:learner` remains as a deprecated compatibility alias:
...
- **User-level**: `~/.grok/skills/omg-learned/` (portable)
...
Run `/oh-my-grok:hud setup` to install. Presets: minimal, focused, full.
...
Or use `/oh-my-grok:note` to save discoveries manually:

```bash
/oh-my-grok:note Project uses PostgreSQL with Prisma ORM
```
...
```bash
/oh-my-grok:ralph-init "implement OAuth with multiple providers"
    ↓
...
```bash
/oh-my-grok:pipeline explore:haiku -> architect:opus -> executor:sonnet
```
...
- `/oh-my-grok:cancel-ralph` (deprecated)
- `/oh-my-grok:cancel-ultraqa` (deprecated)
- `/oh-my-grok:cancel-ultrawork` (deprecated)
- `/oh-my-grok:cancel-autopilot` (deprecated)

Use `/oh-my-grok:cancel` instead.
...
```typescript
Task(
  (subagent_type = "oh-my-grok:explore-high"),
  (model = "opus"),
...
Users set their default mode preference via `/oh-my-grok:omg-setup`.
...
Set your preferred execution mode in `~/.grok/.omg-config.json`:
...
- Watch the [GitHub repository](https://github.com/Yeachan-Heo/oh-my-grok) for announcements
...
```
/oh-my-grok:ultrawork "implement the todo list feature"
```
...
```
/oh-my-grok:ralph "debug the memory leak"
```
...
```
/oh-my-grok:planner "design the new authentication system"
```
...
Apply oh-my-grok to current project only:

```
/oh-my-grok:omg-default
```
...
Apply to all Grok Build sessions:

```
/oh-my-grok:omg-default-global
```

Creates: `~/.grok/CLAUDE.md`
...
A: Keep using them! `/oh-my-grok:ralph`, `/oh-my-grok:ultrawork`, and `/oh-my-grok:plan` work. Note: `/oh-my-grok:planner` now redirects to `/oh-my-grok:plan`.

**Q: How do I know what Claude is doing?**
A: Claude announces major behaviors: "I'm activating ralph-loop..." or set up `/oh-my-grok:hud` for real-time status.
...
- **Diagnose issues**: Run `/oh-my-grok:omg-doctor`
- **See all commands**: Run `/oh-my-grok:omg-help`
- **View real-time status**: Run `/oh-my-grok:hud setup`
- **Review detailed changelog**: See [CHANGELOG.md](../CHANGELOG.md)
- **Report bugs**: [GitHub Issues](https://github.com/Yeachan-Heo/oh-my-grok/issues)
