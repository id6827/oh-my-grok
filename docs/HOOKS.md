...
> OMG's 21 hooks intercept Grok Build lifecycle events to enable magic keywords, context injection, and quality enforcement.

## What Are Hooks?

Hooks are scripts that execute automatically in response to Grok Build lifecycle events. oh-my-grok extends Grok Build's default behavior with 21 hooks.
...
OMG hooks fall into four categories:
...
Grok Build emits events throughout a session. OMG attaches hooks to these events to extend behavior. There are 11 lifecycle events.
...
Blocks only deterministic recurring workflow drift at the Grok Build `Stop` lifecycle point. The boundary follows the official Grok Build [hooks reference](https://code.claude.com/docs/en/hooks): Stop hooks receive `last_assistant_message`, may return `decision: "block"` with a `reason`, and must account for `stop_hook_active` to avoid self-reinforcing loops. Plugin/Hookify installs follow the official Grok Build [plugins reference](https://code.claude.com/docs/en/plugins-reference): plugin hooks can live in `hooks/hooks.json` at the plugin root and respond to the same lifecycle events as user hooks.

- **Event**: Stop
- **Behavior**: Blocks only a supported, local selection fork in the final assistant message. The block reason directs Claude to use `ask_user_question` with 2–4 options and `allowOther` unless free-form input is unsafe.
...
- **Event**: Stop
- **Behavior**: Checks `.omg/state/` for active mode state files. If any mode (ralph, ultragoal, autopilot, ultrawork, ultraqa, team, pipeline) is active, injects a reinforcement message to prevent Claude from stopping.
...
- **Cancel**: Use `/oh-my-grok:cancel` to deactivate modes
...
- **Completion**: after the final quality gate and ultragoal checkpoint, mark the state inactive or run `/oh-my-grok:cancel` so the state file is cleared with other workflow state.
...
Grok Build's context window is finite. During long sessions, compaction occurs and previous conversation content is summarized. OMG's context management hooks prepare for compaction, preserve important information, and maintain project-level memory.
...
OMG's context management hooks cooperate with the following strategy:
...
Magic keywords automatically activate OMG skills or execution modes when specific words or patterns are detected in the user's natural language prompt. No slash command is needed — include a keyword in your prompt and the feature activates automatically.
...
```
/oh-my-grok:team 3:executor "build a fullstack todo app"
```
