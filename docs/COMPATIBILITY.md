...
The Compatibility Layer enables oh-my-grok to discover, register, and use external plugins, MCP servers, and tools. It provides a unified interface for managing external tools while maintaining security through an integrated permission system.
...
3. **Permission Adapter** - Integrates with OMG's permission system for safe tool execution
4. **MCP Bridge** - Connects to MCP servers and exposes their tools for use

```
Plugins              MCP Configs          OMG Tools
   ↓                      ↓                    ↓
 Discovery System ────────────────────────────┐
                                              ↓
                           Tool Registry ← ← ←┘
...
- `~/.grok/plugins/` - OMG/Grok Build plugins directory
- `~/.grok/installed-plugins/` - Alternative plugins location
- `~/.grok/settings.json` - Grok Build MCP server configs
- `~/.grok/claude_desktop_config.json` - Claude Desktop MCP server configs
...
Integrates external tools with OMG's permission system:
...
Plugins are discovered from `~/.grok/plugins/` and `~/.grok/installed-plugins/`:
...
**Skills** are discovered from `SKILL.md` files in supported skills directories. OMG's canonical project-local write target remains `.omg/skills/`, and it also reads Grok Build project skills from `.claude/skills/` plus project-local compatibility skills from `.agents/skills/`. Each skill directory must contain a SKILL.md with frontmatter:
...
**Agents** are discovered from `.md` files in the agents directory with similar frontmatter structure. Supported runtime fields depend on Grok Build; OMG's bundled agent files currently rely on `name`, `description`, `model`, optional tool restrictions, and prompt body guidance. They do not currently ship an `effort:` frontmatter override, so effort inherits from the parent Grok Build session unless a custom agent explicitly adds one.

## MCP Server Discovery

### Claude Desktop Config

Located at `~/.grok/claude_desktop_config.json`:
...
### Remote MCP / Remote OMG Shape

OMG can sync and preserve **remote MCP** entries in the unified registry. That is the supported narrow answer to "connect to a remote OMG":
...
This supports remote MCP endpoints. It does **not** create a general multi-host OMG cluster or a transparent shared remote filesystem view.
...
const result = await initializeCompatibility({
  pluginPaths: ['~/.grok/plugins'],
  mcpConfigPath: '~/.grok/claude_desktop_config.json',
...
const servers = discoverMcpServers({
  mcpConfigPath: '~/.grok/claude_desktop_config.json',
  settingsPath: '~/.grok/settings.json'
});
...
**Checklist:**
- Plugins are in `~/.grok/plugins/` or `~/.grok/installed-plugins/`
- Each plugin has a `plugin.json` in the root or `.claude-plugin/` subdirectory
- Plugin name doesn't conflict with reserved names (e.g., 'oh-my-grok')
