...
> How oh-my-grok orchestrates multi-agent workflows.

## Overview

oh-my-grok enables Grok Build to orchestrate specialized agents through a skill-based routing system. It is built on four interlocking systems: **Hooks** detect lifecycle events, **Skills** inject behaviors, **Agents** execute specialized work, and **State** tracks progress across context resets.
...
OMG provides 19 specialized agents organized into 4 lanes. Each agent is invoked as `oh-my-grok:<agent-name>` and runs on the appropriate model tier.
...
Skills are **behavior injections** that modify how the orchestrator operates. Instead of swapping agents, skills add capabilities on top of existing agents. OMG provides 31 skills total (28 user-invocable + 3 internal/pipeline).
...
**Slash commands:**
```bash
/oh-my-grok:autopilot build me a todo app
/oh-my-grok:ralph refactor the auth module
/oh-my-grok:team 3:executor "implement fullstack app"
```
...
```bash
/oh-my-grok:team 3:executor "implement fullstack todo app"
```
...
|-------|-------------|---------|
| `cancel` | Cancel active execution mode | `/oh-my-grok:cancel` |
| `hud` | Status bar configuration | `/oh-my-grok:hud` |
| `omg-setup` | Initial setup wizard | `/oh-my-grok:omg-setup` |
| `omg-doctor` | Diagnose installation | `/oh-my-grok:omg-doctor` |
| `skillify` | Extract reusable skills from session | `/oh-my-grok:skillify` (`learner` deprecated alias) |
| `skill` | Manage local skills (list/add/remove) | `/oh-my-grok:skill` |
| `trace` | Evidence-driven causal tracing | `/oh-my-grok:trace` |
| `release` | Automated release workflow | `/oh-my-grok:release` |
| `deepinit` | Generate hierarchical AGENTS.md | `/oh-my-grok:deepinit` |
| `deep-interview` | Socratic deep interview | `/deep-interview` |
| `sciomc` | Parallel scientist agent orchestration | `/oh-my-grok:sciomc` |
| `external-context` | Parallel document-specialist research | `/oh-my-grok:external-context` |
| `ai-slop-cleaner` | Clean AI expression patterns | `/oh-my-grok:ai-slop-cleaner` |
| `writer-memory` | Memory system for writing projects | `/oh-my-grok:writer-memory` |
...
Hooks are code that reacts to Grok Build lifecycle events. They run automatically when a user submits a prompt, uses a tool, or starts/ends a session. OMG implements agent delegation, keyword detection, and state persistence through this hook system.

### Lifecycle Events

Grok Build provides 11 lifecycle events. OMG registers hooks on these events:

| Event | When It Fires | OMG Usage |
|-------|---------------|-----------|
...
OMG hooks are declared in `hooks.json`. Each hook is a Node.js script with a timeout:
...
OMG stores task progress and project knowledge in the `.omg/` directory. The state system preserves critical information even when context compaction resets the context window.
...
OMG keeps orchestration metadata separate from large durable artifacts:
