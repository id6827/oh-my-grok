...
This document defines the normalized event contract OMG emits through the OpenClaw bridge for native Clawhip-style consumers.
...
- `ask_user_question` now emits only the dedicated `question.requested` signal instead of also emitting generic tool lifecycle events.
