...
the Grok Build `/goal` slash command. It stores plan/ledger artifacts under
...
- **It is**: a small filesystem state machine for breaking a brief into
  ...
  evidence.
- **It isn't**: a way for a shell command to mutate Grok Build `/goal`
  ...
  success). OMG cannot invoke `/goal` for the model — the handoff text is
...
- `aggregate` (default): one Claude `/goal` covers the whole ultragoal run;
  OMG stories `G001`/`G002`/… are bookkeeping in the ledger.
...
- The Claude `/goal` slash command is a session-scoped, in-session
  ...
  authoritative proof; OMG only verifies textual consistency between the
