...
Complete reference for oh-my-grok. For quick start, see the main [README.md](../README.md).
...
- [Grok Build `/goal` Adapter Design](#claude-code-goal-adapter-design)
...
OMG has two supported public surfaces. Use the Grok Build plugin for in-session slash commands, hooks, agents, skills, and statusline behavior. Use the npm-installed `omg` CLI for terminal commands, setup/update automation, and CI-safe checks.
...
This integrates directly with Grok Build's plugin system and uses Node.js hooks.

### Terminal CLI

```bash
npm i -g oh-my-grok@latest
omg setup
```

The npm package exposes both `oh-my-grok` and `omg`; examples prefer `omg` unless troubleshooting needs the long alias. The CLI does not make in-session slash skills available by itself; install the plugin for `/autopilot`, `/ralph`, `/team`, and other interactive skills.

### Requirements

- [Grok Build](https://docs.anthropic.com/claude-code) installed
- One of:
...
Configure omg for the current project only:

```
/oh-my-grok:omg-setup --local
```
...
Configure omg for all Grok Build sessions:

```
/oh-my-grok:omg-setup
```

- Creates `~/.grok/CLAUDE.md` globally
- Configuration applies to all projects
- **Default**: explicitly overwrites existing `~/.grok/CLAUDE.md`
- **Optional preserve mode**: keeps the base file, writes OMG to `~/.grok/CLAUDE-omg.md`, and lets `omg` force-load that companion config at launch while plain `claude` stays unchanged

### What Configuration Enables

| Feature           | Without     | With omg Config         |
| ----------------- | ----------- | ----------------------- |
...
```
./.claude/CLAUDE.md  (project)   →  Overrides  →  ~/.grok/CLAUDE.md  (global)
```
...
| -------------------------- | -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `OMC_STATE_DIR`            | _(unset)_            | Centralized state directory. When set, OMG stores state at `$OMC_STATE_DIR/{project-id}/` instead of `{worktree}/.omg/`. This preserves state across worktree deletions. The project identifier is derived from the git remote URL (or worktree path for local-only repos). |
...
| `DISABLE_OMC`              | _(unset)_            | Set to `1` or `true` to disable all OMG hooks |
| `OMC_SKIP_HOOKS`           | _(unset)_            | Comma-separated list of hook names to skip                                                                                                                                                                                                                                  |

#### Centralized State with `OMC_STATE_DIR`

By default, OMG stores state in `{worktree}/.omg/`. This is lost when worktrees are deleted. To preserve state across worktree lifecycles, set `OMC_STATE_DIR`:
...
If both a legacy `{worktree}/.omg/` directory and a centralized directory exist, OMG logs a notice and uses the centralized directory. You can then migrate data from the legacy directory and remove it.

#### OMG state, gitignore, worktree, and workspace contract

OMG's project-local state root is `.omg/` unless `OMC_STATE_DIR` or `.omg-workspace` changes the root resolution described below. The default root contains runtime and audit artifacts such as:
...
- `.omg/skills/` — the only project-local `.omg` subtree intended to be committed when the team wants to share OMG-authored skills.
...
- **Linked git worktrees**: without `OMC_STATE_DIR`, each linked worktree has its own `{worktree}/.omg/`; removing that worktree removes its local OMG state. Re-run setup from the worktree you are actively using so installed hooks and generated instructions match that checkout.
- **Persistent state across worktree deletion**: set `OMC_STATE_DIR`; OMG writes to `$OMC_STATE_DIR/{project-id}/`, where the project id is stable across linked worktrees when a remote or primary git dir is available.
- **Multi-repo workspace**: add `.omg-workspace` to a non-git parent when independent sibling repos should share `{parent}/.omg/`. This is for multi-repo workspaces, not ordinary monorepos.
...
Cleanup rule of thumb: after OMG sessions are stopped, it is safe to delete ignored runtime subtrees such as `.omg/state/`, `.omg/logs/`, `.omg/artifacts/`, `.omg/research/`, or `.omg/ultragoal/` if you no longer need their recovery/audit history. Do not delete `.omg/skills/` unless you intend to remove project-scoped skills.

#### Multi-repo workspaces with `.omg-workspace`

When you have several independent git repos under one parent directory and the parent itself is **not** a git repo, OMG cannot infer a shared root via `git rev-parse --show-toplevel`. Each sub-repo would get its own isolated `.omg/`. To anchor a single `.omg/` at the parent, drop a `.omg-workspace` marker file there:

```bash
cd /path/to/my-workspace            # parent dir (not a git repo)
echo '{}' > .omg-workspace          # empty JSON is fine
```

From any sub-directory (including inside any sub-git-repo), OMG resolves `.omg/` to `/path/to/my-workspace/.omg/`. The marker may also carry an explicit project identifier so all sessions share state regardless of the parent dir name:
...
2. `.omg-workspace` marker (multi-repo workspace).
3. `git rev-parse --show-toplevel` (monorepo / single repo).
4. `process.cwd()` (last resort).

Once a workspace is anchored, multiple Grok Build sessions in different sub-repos can run `/ultragoal`, `/ralph`, `/ultrawork`, `/autopilot` in parallel without bleeding state. For `/ultragoal` specifically, pass `--plan-id <id>` or `--auto-plan-id` on `create-goals` so each session writes to `.omg/ultragoal/plans/{planId}/` instead of the shared `goals.json` — see "ultragoal multi-plan" below. The PARALLEL SESSION WARNING in `session-start.mjs` performs a PID-aware liveness check and no longer suppresses restore when the owner session is dead.
...
- Handoff files survive team cancellation and OMG state cleanup intentionally — they are post-mortem artifacts. Grok Build 2.1.178+ has no `TeamDelete`.
...
When you adopt `OMC_STATE_DIR` or `.omg-workspace` on a repo that already has existing `{worktree}/.omg/state/` files, you can opt in to a one-shot copy of legacy state into the new session-scoped path:
...
If the workspace-marker resolution causes unexpected behaviour (e.g., after dropping a stale `.omg-workspace` marker), you can disable multi-repo path resolution in one env-var flip:
...
Exact semantics:
- **Skips** `.omg-workspace` marker detection — `findWorkspaceRoot()` returns `null` immediately.
...
- **Different machines**: Run on each machine where you use Grok Build
- **New projects**: Run `/oh-my-grok:omg-setup --local` in each project that needs omg

> **NOTE**: After updating the plugin (via `npm update`, `git pull`, or Grok Build's plugin update), you MUST re-run `/oh-my-grok:omg-setup` to apply the latest CLAUDE.md changes.

### Remote OMG / Remote MCP Access

Issue #1653 asked whether OMG can "connect to a remote OMG" so one development machine can browse files on lab/test machines without opening an interactive SSH session.

The narrow, coherent answer today is:

- **Supported**: connect to a **remote MCP server** through the unified MCP registry
- **Not implemented**: a general "OMG cluster", shared remote filesystem view, or automatic remote-OMG federation
- **Still appropriate for full remote shell workflows**: SSH, worktrees, or a mounted/network filesystem

If a remote host already exposes an MCP endpoint, add it to your MCP registry (or Claude settings and then re-run setup so OMG syncs the registry to Codex too):
...
This gives OMG a coherent remote connection surface for MCP-backed tools. It does **not** make all remote files magically appear as a local workspace, and it does **not** replace SSH for arbitrary shell access.
...
OMG also supports a narrow company-context contract on top of the existing MCP surface.

Configure it in the standard OMG config files:
...
- The MCP server itself is still registered through the normal Claude/OMG MCP setup path.
...
Edit agent files in `~/.grok/agents/` to customize behavior:
...
# Optional: effort inherits from the parent Grok Build session unless you add an explicit override.
# effort: high
---
Your custom system prompt here...
```

Bundled OMG agent prompts currently do **not** ship an `effort:` frontmatter field. Any effort language inside `agents/*.md` is behavioral guidance for the prompt body, while runtime effort inherits from the parent Grok Build session unless the agent markdown explicitly declares an override.
...
OMG documentation should describe goal and workflow artifacts by their logical role first, then map that role to the runtime-specific storage root. Do not treat `.omx/` as a universal path: it is the legacy OMX runtime root, while OMG uses `.omg/` for local project state.

### Runtime root mapping

| Runtime                      | Project-local root     | User/global root            | Notes                                                                                                                                                   |
| ---------------------------- | ---------------------- | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| OMG                          | `.omg/`                | `~/.omg/`                   | Canonical OMG storage for project-local state, plans, notepads, logs, research, and ask artifacts.                                                      |
...
| OMO native                   | runtime-owned OMO path | runtime-owned OMO user path | OMO-native storage is owned by that runtime. OMG docs should name the logical artifact role unless an OMO command explicitly documents a concrete path. |

### Logical goal artifact roles

Use these names when writing docs or handoffs so the same concept remains portable across OMG, OMX compatibility, and OMO-native runtimes:

| Logical role            | OMG path                                                                   | OMX compatibility path                                                     | Purpose                                                                                                                                                                            |
| ----------------------- | -------------------------------------------------------------------------- | -------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
When an environment variable such as `OMC_STATE_DIR` centralizes storage, resolve the OMG project-local root through that setting before expanding the paths above. In docs, phrase this as "the OMG state root" or "the team coordination root" when the exact filesystem path may vary.

### `/goal` interoperability notes

Grok Build's `/goal` feature owns its hidden goal state. OMG integrations should not mutate hidden Grok Build goal storage directly. When OMG needs a goal-related artifact, create or update an explicit OMG artifact such as `.omg/specs/<slug>.md`, `.omg/plans/<slug>.md`, or `.omg/state/<mode>.json` and record any `/goal` relationship as metadata or prose in that artifact.
...
- Use `.omg/...` examples for OMG-facing docs and commands.
- Use `.omx/...` examples only for OMX compatibility behavior.
- For OMO-native behavior, link to or quote the OMO command's documented path instead of inventing an OMG/OMX path.

---

## Plugin directory flags

When you launch OMG via a local development checkout instead of the marketplace plugin, you can configure how OMG discovers agents, skills, and commands.
...
- **What it does**: Parses `--plugin-dir <path>` (or `--plugin-dir=<path>`), resolves it to an absolute path, sets `OMC_PLUGIN_ROOT` environment variable, then passes the flag through to Grok Build untouched.
- **Non-consuming**: The flag stays in the argument list so Grok Build's plugin loader still sees it.
...
- **Pair with setup**: `--plugin-dir` alone only affects the current Claude session. You must **also** run `omg setup --plugin-dir-mode` (or let auto-detection kick in from `OMC_PLUGIN_ROOT`) so HUD, hooks, and CLAUDE.md are installed for the linked checkout. Skipping this step leaves `~/.grok/` pointing at a stale plugin root.

### `claude --plugin-dir <path>` (direct)

**Usage**: When you launch Grok Build directly without the `omg` shim.
...
- **What it does**: Skips copying agents and bundled skills into `~/.grok/` because the plugin already provides them at runtime via `--plugin-dir`.
- **Still installs**:
  - HUD bundle (`~/.grok/hud/`)
  - Git hooks (`.git/hooks/`, if applicable)
  - CLAUDE.md configuration files
  - `.omg-config.json` state
...
omg doctor conflicts --plugin-dir /path/to/oh-my-grok
```
...
**Usage**: Authoritative source for the active plugin root when launching Grok Build.
...
| ------------------------------------- | ------------------------------------------------------------ | ------------------------------- | ------------------------------------------------------------------- |
| **Marketplace plugin** (recommended)  | `omg` or `claude` (default)                                  | `omg setup`                     | Normal: agents/skills copied to `~/.grok/`                        |
| **Local dev checkout, want OMG shim** | `omg --plugin-dir /path`                                     | `omg setup --plugin-dir-mode`   | Dev mode: agents/skills loaded from `/path`, not copied             |
| **Local dev checkout, no OMG shim**   | `claude --plugin-dir /path` + `export OMC_PLUGIN_ROOT=/path` | `omg setup --plugin-dir-mode`   | Dev mode + manual env: agents/skills loaded from `/path`            |
| **Local dev, want bundled skills**    | `omg --plugin-dir /path`                                     | `omg setup --no-plugin`         | Forces local bundled skills to `~/.grok/skills/`, ignoring plugin |
...
- Skill entrypoint: `/oh-my-grok:ask <claude|codex|gemini|antigravity|grok|cursor> <prompt>` routes to this command
...
Use OMG's terminal and library surfaces in non-interactive environments:
...
- Avoid interactive slash skills (`/autopilot`, `/ralph`, `/ultrawork`, `/deep-interview`, `/team`) in CI jobs; they require an active Grok Build session and user-visible conversation loop.
- OMG does not currently provide a VS Code extension or VS Code-specific automation contract. The documented IDE path is to use Grok Build's own integrations, then install OMG through the Grok Build plugin surface.
...
```json
{
  "mcpServers": {
    "team": {
      "command": "node",
      "args": ["${GROK_PLUGIN_ROOT}/bridge/team-mcp.cjs"]
    }
  }
}
```
...
OMG handoffs follow an artifact-first discipline:
...
Always use `oh-my-grok:` prefix when calling via Task tool.
...
OMG exposes several ways to pursue a goal-shaped task. They are complementary, not interchangeable. Choose one primary loop authority per session and use the others as evidence producers or handoff targets.

| Surface                 | Runtime owner                                     | User-facing promise                                           | Completion evidence                                                          | Notes                                                                                                                                                           |
| ----------------------- | ------------------------------------------------- | ------------------------------------------------------------- | ---------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Grok Build `/goal`     | Grok Build native session loop                   | Keep working toward one stated completion condition           | Evidence surfaced in the conversation for the `/goal` evaluator              | Cite Grok Build/Anthropic docs or changelog only for `/goal` behavior. The evaluator must not be described as independently running commands or reading files. |
| Ralph                   | OMG skill + Stop-hook enforcement                 | Persistent single-owner implementation until PRD stories pass | Tests/build/lint/typecheck plus reviewer verification                        | Prefer when correctness depends on OMG's PRD, progress, and reviewer gates.                                                                                     |
| Team                    | OMG native/team or CLI team runtime               | Coordinated multi-agent execution over assigned tasks         | Worker task results, commits, staged `team-verify`/`team-fix` evidence       | Prefer when ownership boundaries and parallel lanes matter.                                                                                                     |
| UltraQA                 | OMG QA cycling skill                              | Repeat diagnose/fix cycles until a quality gate passes        | Command output from the requested QA goal each cycle                         | Prefer after the implementation target is known but verification still fails.                                                                                   |
| Artifact-only Ultragoal | Durable goal ledger/checkpoints/handoff artifacts | Track goal state without starting another active loop         | Goal artifact, checkpoints, handoff prompt, attached command/review evidence | Prefer when `/goal` is unavailable, unsafe, or conflicts with an active OMG loop.                                                                               |

### `/goal` source and evidence boundary

Use Grok Build/Anthropic sources for `/goal` facts, including:

- Grok Build `/goal` documentation: <https://code.claude.com/docs/en/goal>
- Anthropic Grok Build changelog: <https://raw.githubusercontent.com/anthropics/claude-code/main/CHANGELOG.md>

Do not cite OpenAI/Codex documentation as authority for Grok Build `/goal`. In OMG docs, examples, and handoff prompts, keep this limitation explicit: the `/goal` evaluator judges visible conversation evidence. It should not be described as independently executing shell commands, reading files, inspecting hidden state, or replacing OMG's final review gates.
...
2. **adopt_existing**: if Grok Build `/goal` is already active and the OMG workflow can attach evidence to that same condition without changing loop ownership.
...
`/goal` evaluator success can be useful evidence, but OMG completion should still require the relevant durable proof: command output, changed files, reviewer verdicts, task results, or release artifacts.
...
Marketplace/plugin installs compact the native plugin `skills/*/SKILL.md` files during `omg setup`: Grok Build receives concise registry descriptions for every bundled skill, while the full on-demand instructions are preserved under `skill-bodies/*/SKILL.md` and loaded by OMG when a skill is invoked. Source checkouts and standalone installs keep the full `skills/*/SKILL.md` bodies in place.



| Skill                     | Description                                                      | Manual Command                              |
| ------------------------- | ---------------------------------------------------------------- | ------------------------------------------- |
| `ai-slop-cleaner`         | Anti-slop cleanup workflow with optional reviewer-only `--review` pass | `/oh-my-grok:ai-slop-cleaner`         |
| `ask`                     | Ask Claude, Codex, Gemini, Antigravity, or Grok via local CLI and capture a reusable artifact | `/oh-my-grok:ask`               |
| `autoresearch`            | Stateful single-mission evaluator-driven improvement loop           | `/oh-my-grok:autoresearch`            |
| `autopilot`               | Full autonomous execution from idea to working code              | `/oh-my-grok:autopilot`               |
| `cancel`                  | Unified cancellation for active modes                            | `/oh-my-grok:cancel`                  |
| `ccg`                     | Tri-model workflow via `ask codex` + `ask antigravity`, then Claude synthesis | `/oh-my-grok:ccg`                     |
| `configure-notifications` | Configure notification integrations (Telegram, Discord, Slack) via natural language | `/oh-my-grok:configure-notifications` |
| `deep-dive`               | Two-stage trace → deep-interview pipeline with context handoff   | `/oh-my-grok:deep-dive`               |
| `deep-interview`          | Socratic deep interview with ambiguity gating                    | `/deep-interview`                           |
| `deepinit`                | Generate hierarchical AGENTS.md docs                             | `/oh-my-grok:deepinit`                |
| `external-context`        | Parallel document-specialist research                            | `/oh-my-grok:external-context`        |
| `hud`                     | Configure HUD/statusline                                         | `/oh-my-grok:hud`                     |
| `skillify`                | Extract reusable skill from session                              | `/oh-my-grok:skillify`                |
| `learner`                 | **Deprecated** compatibility alias for `skillify`                | `/oh-my-grok:learner`                 |
| `mcp-setup`               | Configure MCP servers                                            | `/oh-my-grok:mcp-setup`               |
| `omg-doctor`              | Diagnose and fix installation issues                             | `/oh-my-grok:omg-doctor`              |
| `omg-plan`                | Planning workflow (`/plan` safe alias; bundled directory ID is `plan`) | `/oh-my-grok:plan`                    |
| `omg-reference`           | Detailed OMG agent/tools/team/commit reference skill             | Auto-loaded reference only                  |
| `omg-setup`               | One-time setup wizard                                            | `/oh-my-grok:omg-setup`               |
| `omg-teams`               | Spawn `claude`/`codex`/`gemini`/`antigravity`/`grok`/`cursor` tmux workers for parallel execution | `/oh-my-grok:omg-teams`             |
| `project-session-manager` | Manage isolated dev environments (git worktrees + tmux)          | `/oh-my-grok:project-session-manager` |
| `psm` | **Deprecated** compatibility alias for `project-session-manager` | `/oh-my-grok:psm` |
| `ralph`                   | Persistence loop until verified completion                       | `/oh-my-grok:ralph`                   |
| `ralplan`                 | Consensus planning alias for `/plan --consensus`                 | `/oh-my-grok:ralplan`                 |
| `release`                 | Automated release workflow                                       | `/oh-my-grok:release`                 |
| `self-improve`            | Autonomous evolutionary code improvement engine with tournament selection; artifacts are topic-scoped under `.omg/self-improve/topics/<topic-slug>/` by default, with flat `.omg/self-improve/` preserved for legacy single-track resumes | `/oh-my-grok:self-improve`    |
| `setup`                   | Unified setup entrypoint for install, diagnostics, and MCP configuration | `/oh-my-grok:setup`              |
| `sciomc`                  | Parallel scientist orchestration                                 | `/oh-my-grok:sciomc`                  |
| `skill`                   | Manage local skills (list/add/remove/search/edit)                | `/oh-my-grok:skill`                   |
| `team`                    | Coordinated multi-agent workflow                                 | `/oh-my-grok:team`                    |
| `trace`                   | Evidence-driven tracing lane with parallel tracer hypotheses     | `/oh-my-grok:trace`                   |
| `ultraqa`                 | QA cycle until goal is met                                       | `/oh-my-grok:ultraqa`                 |
| `ultrawork`               | Maximum parallel throughput mode                                 | `/oh-my-grok:ultrawork`               |
| `visual-verdict`          | Structured visual QA verdict for screenshot/reference comparisons | `/oh-my-grok:visual-verdict`          |
| `wiki`                    | LLM Wiki — persistent markdown knowledge base that compounds across sessions | `/oh-my-grok:wiki`           |
| `writer-memory`           | Agentic memory system for writing projects                       | `/oh-my-grok:writer-memory`           |
...
Most installed skills are exposed as `/oh-my-grok:<skill-name>`. Deep Interview is intentionally documented with the short `/deep-interview` path because that path receives OMG's rendered runtime threshold guidance before the interview starts. The skills table above is the full runtime-backed list; the commands below highlight common entrypoints and aliases. Compatibility keyword modes like `deep-analyze` and `tdd` are prompt-triggered behaviors, not standalone slash commands. OMG's manual compaction helper is plugin-scoped as `/oh-my-grok:compact`; bare `/compact` remains Grok Build's native command and is not shadowed by OMG. The helper preserves the user's note and instructs them to run bare `/compact`; OMG does not invoke native compaction itself because Grok Build's built-in `/compact` is not a prompt skill.

| Command                                                  | Description                                                                                   |
| -------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `/oh-my-grok:ai-slop-cleaner <target>`             | Run the anti-slop cleanup workflow (`--review` for reviewer-only pass)                        |
| `/oh-my-grok:ask <claude\|codex\|gemini\|antigravity\|grok\|cursor> <prompt>` | Route a prompt through the selected advisor CLI and capture an ask artifact                   |
| `/oh-my-grok:autopilot <task>`                     | Full autonomous execution                                                                     |
| `/oh-my-grok:configure-notifications`              | Configure notification integrations                                                           |
| `/oh-my-grok:compact [note]`                        | Prepare an OMG-safe manual handoff telling the user to run bare `/compact [note]`              |
| `/oh-my-grok:deep-dive <problem>`                  | Run the trace → deep-interview pipeline                                                       |
| `/deep-interview <idea>`                                 | Socratic interview with ambiguity scoring before execution                                    |
| `/oh-my-grok:deepinit [path]`                      | Index codebase with hierarchical AGENTS.md files                                              |
| `/oh-my-grok:mcp-setup`                            | Configure MCP servers                                                                         |
| `/oh-my-grok:omg-doctor`                           | Diagnose and fix installation issues                                                          |
| `/oh-my-grok:plan <description>`                   | Start planning session (supports consensus structured deliberation)                           |
| `/oh-my-grok:omg-setup`                            | One-time setup wizard                                                                         |
| `/oh-my-grok:omg-teams <N>:<agent> <task>`         | Spawn `claude`/`codex`/`gemini`/`antigravity`/`grok`/`cursor` tmux workers for legacy parallel execution                    |
| `/oh-my-grok:project-session-manager <arguments>`  | Manage isolated dev environments with git worktrees + tmux                                    |
| `/oh-my-grok:psm <arguments>`                      | Deprecated alias for project session manager                                                  |
| `/oh-my-grok:ralph <task>`                         | Self-referential loop until task completion (`--critic=architect \| critic \| codex`)       |
| `/oh-my-grok:ralplan <description>`                | Iterative planning with consensus structured deliberation (`--deliberate` for high-risk mode) |
| `/oh-my-grok:release`                              | Automated release workflow                                                                    |
| `/oh-my-grok:setup`                                | Unified setup entrypoint (`setup`, `setup doctor`, `setup mcp`)                               |
| `/oh-my-grok:sciomc <topic>`                       | Parallel research orchestration                                                               |
| `/oh-my-grok:team <N>:<agent> <task>`              | Coordinated native team workflow                                                              |
| `/oh-my-grok:trace`                                | Evidence-driven tracing lane that orchestrates parallel tracer hypotheses in team mode        |
| `/oh-my-grok:ultraqa <goal>`                       | Autonomous QA cycling workflow                                                                |
| `/oh-my-grok:ultrawork <task>`                     | Maximum performance mode with parallel agents                                                 |
| `/oh-my-grok:visual-verdict <task>`                | Structured visual QA verdict for screenshot/reference comparisons                             |
...
When present, OMG appends a standardized **Skill Pipeline** section to the rendered skill prompt so the current stage, handoff artifact, and explicit next `Skill("oh-my-grok:...")` invocation are carried forward consistently.

### Skills 2.0 Compatibility (MVP)

OMG's canonical project-local skill directory remains `.omg/skills/`, and the runtime also reads Grok Build project skills from `.claude/skills/` plus compatibility skills from `.agents/skills/`.

For builtin and slash-loaded skills, OMG also appends a standardized **Skill Resources** section when the skill directory contains bundled assets such as helper scripts, templates, or support libraries. This helps agents reuse packaged skill resources instead of recreating them ad hoc.

---

## Grok Build `/goal` Adapter Design

OMG treats Grok Build `/goal` as a native execution loop that can be handed off to, not as the durable source of truth for OMG completion. The design contract is documented in [docs/design/CLAUDE_CODE_GOAL_ADAPTER.md](./design/CLAUDE_CODE_GOAL_ADAPTER.md).

Key contract points:

- Grok Build `/goal` facts must cite Grok Build or Anthropic sources only. OpenAI/Codex references are comparison sources, not authority for Grok Build behavior.
- The adapter renders a measurable `/goal <condition>` handoff; it must not mutate hidden Grok Build session state directly.
...
- `/goal` evaluator success is evidence for OMG final review, not completion by itself; OMG still requires surfaced command/test/docs evidence.
- OMG stores durable goal ledgers and evidence under OMG-owned logical artifacts and `.omg/`-resolved paths, not hardcoded `.omx/` paths.

---

## Hooks System

OMG registers 21 hook scripts across 11 Grok Build lifecycle events. For detailed documentation, see [HOOKS.md](./HOOKS.md).
...
explicitly enabled via the global OMG config file:

- Linux/Unix default: `${XDG_CONFIG_HOME:-~/.config}/omg/config.json`
...
| **web_search** | ✅ Available | Search the web        |
| **web_fetch**  | ✅ Available | Fetch web pages       |
| **Task**      | ✅ Available | Spawn subagents       |
| **todo_write** | ✅ Available | Track tasks           |
...
oh-my-grok includes comprehensive monitoring for agent performance, token usage, and debugging parallel workflows.
...
Configure HUD elements in `~/.grok/settings.json`:
...
| `omcLabel`   | Show [OMG] label                                                                                                     | `true`  |
| `updateNotification` | Show available-update prompt text after the OMG label                                                                  | `true`  |
...
| Issue                 | Solution                                                                         |
| --------------------- | -------------------------------------------------------------------------------- |
| Commands not found    | Re-run `/oh-my-grok:omg-setup`                                             |
| Hooks not executing   | Check hook permissions: `chmod +x ~/.grok/hooks/**/*.sh`                       |
| Agents not delegating | Verify CLAUDE.md is loaded: check `./.claude/CLAUDE.md` or `~/.grok/CLAUDE.md` |
| LSP tools not working | Install language servers: `npm install -g typescript-language-server`            |
| Token limit errors    | Use `/oh-my-grok:` for token-efficient execution                           |
...
To manually update, re-run the plugin install command or use Grok Build's built-in update mechanism.

### Uninstall

Use Grok Build's plugin management:

```
/plugin uninstall oh-my-grok@oh-my-grok
```

Or manually remove the installed files:

```bash
rm ~/.grok/agents/{architect,document-specialist,explore,designer,writer,vision,critic,analyst,executor,qa-tester}.md
rm ~/.grok/commands/{analyze,autopilot,deepsearch,plan,review,ultrawork}.md
```
