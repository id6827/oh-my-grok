...
Grok Build exposes `/goal` as a native, session-scoped work loop. OMG can use that loop as an execution surface, but OMG must keep its own durable audit trail, hook safety rules, and Ralph/Team/UltraQA boundaries. The adapter described here is a design contract for future implementation; it does not mutate hidden Grok Build goal state.

## Source authority boundary

Grok Build `/goal` facts in OMG docs and code comments must come only from Grok Build or Anthropic sources, such as:

- Grok Build docs: <https://code.claude.com/docs/en/goal>
- Anthropic Grok Build changelog: <https://raw.githubusercontent.com/anthropics/claude-code/main/CHANGELOG.md>

OpenAI, Codex, OMX, or OMO references may be used for comparisons with their own goal/runtime behavior, but they are not authority for Grok Build `/goal` facts.

## Adapter responsibilities

The Grok Build `/goal` adapter is an OMG-facing boundary that renders safe handoff text and durable evidence. It must:

1. Detect or receive a capability verdict for `/goal` before suggesting native handoff.
2. Render a measurable `/goal <completion condition>` handoff prompt instead of writing hidden Grok Build session state directly.
3. Preserve OMG auditability by recording the requested condition, status snapshots, surfaced evaluator reasons, command evidence, and final review outcome in OMG-owned artifacts.
...
| `source_refs`          | Grok Build `/goal` claims cite only Grok Build/Anthropic sources.                                             |
| `evidence`             | Surfaced command output, docs updates, test output, reviewer verdicts, and status snapshots.                    |
| `status`               | Distinguishes evaluator success from OMG final review: `evaluator_passed` is not `complete`.                    |
...
| `artifact_only`  | Write the durable goal ledger and handoff artifact only. Do not ask Grok Build to activate `/goal`.                            |
...
OMG completion requires a final review after any `/goal` evaluator success:

1. `/goal` evaluator passes from surfaced evidence.
2. OMG records `evaluator_passed` with the status snapshot/evaluator reason.
3. OMG runs or records required verification evidence.
4. A final reviewer marks `complete`, `review_blocked`, or `failed`.

Direct `evaluator_passed -> complete` transitions are invalid because they skip OMG-owned verification.

## Storage boundary

The adapter should use logical artifact names first and resolve them through OMG runtime paths second:

| Logical artifact    | OMG path intent                                                |
| ------------------- | -------------------------------------------------------------- |
| Goal ledger         | `.omg/goals/` or the configured OMG state root.                |
...
Docs may compare OMX `.omx` or OMO-native paths, but OMG adapter code must not require `.omx/` to exist in an OMG-only workspace.
