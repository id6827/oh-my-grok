...
When worktree mode is active, OMG uses this stable layout:
...
- OMG must check the leader workspace before provisioning worktrees. If the leader repo is dirty, startup should refuse worktree provisioning rather than copying an unsafe base state.
