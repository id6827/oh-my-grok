...
> OMG provides MCP tools for state management, code intelligence, and data analysis.
...
State tools manage the state of OMG execution modes (autopilot, ralph, ultrawork, etc.). Each mode records its current progress, active status, and configuration in state files.
...
When Grok Build compacts context:
...
python_repl(code="""
import json
with open('.omg/research/session-1/state.json') as f:
    state = json.load(f)
...
Loads skills from the global user directory (`~/.grok/skills/`).
...
Lists all available OMG skills (built-in + local + global).
...
Used internally by the `deepinit` skill (`/oh-my-grok:deepinit`) to enable incremental AGENTS.md regeneration instead of full re-scans.
