...
OMG supports a narrow, prompt-level contract for vendor-owned company context.
...
reduces that miss mode at the spec layer without changing OMG runtime behavior.

This is **not** a runtime enforcement feature. It gives OMG workflows:

- a consistent tool shape,
- an explicit OMG config block,
...
OMG skill clauses should treat the returned markdown the same way `deep-dive` treats injected trace context: useful reference material, never instruction authority.

## OMG Configuration

Configure the contract in the standard OMG config surface:
...
- the unified MCP registry that OMG syncs to Codex
