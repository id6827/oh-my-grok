...
Comprehensive guide to monitoring, debugging, and optimizing Grok Build and oh-my-grok performance.
...
oh-my-grok provides comprehensive monitoring capabilities for tracking agent performance, token usage, costs, and identifying bottlenecks in multi-agent workflows. This guide covers both built-in tools and external resources for monitoring Claude's performance.
...
```typescript
import { getAgentObservatory } from 'oh-my-grok/hooks/subagent-tracker';
...
The legacy analytics workflow described in older docs (`omg-analytics`, `omg cost`, `omg backfill`, and the `analytics` HUD preset) is no longer part of current `dev`.
...
```typescript
import { getReplaySummary } from 'oh-my-grok/hooks/subagent-tracker/session-replay';
...
```typescript
import { getAgentPerformance } from 'oh-my-grok/hooks/subagent-tracker';
...
```typescript
import { detectFileConflicts } from 'oh-my-grok/hooks/subagent-tracker';
...
OMG automatically detects problematic agents:
...
```typescript
import { suggestInterventions } from 'oh-my-grok/hooks/subagent-tracker';
...
```typescript
import { calculateParallelEfficiency } from 'oh-my-grok/hooks/subagent-tracker';
...
```typescript
import { cleanupStaleAgents } from 'oh-my-grok/hooks/subagent-tracker';
...
|----------|-------------|------|
| Grok Build Discord | Community support and tips | [discord.gg/anthropic](https://discord.gg/anthropic) |
| OMG GitHub Issues | Bug reports and feature requests | [GitHub Issues](https://github.com/Yeachan-Heo/oh-my-grok/issues) |
...
```typescript
import { cleanupReplayFiles } from 'oh-my-grok/hooks/subagent-tracker/session-replay';
...
2. Verify HUD / hooks are installed: `/oh-my-grok:hud setup`
