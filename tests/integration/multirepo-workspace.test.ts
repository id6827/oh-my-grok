 ...
 * that share a .omg-workspace marker at a common parent directory.
 */
...
describe('multi-repo workspace anchor', () => {
  ...
  beforeEach(() => {
    clearWorktreeCache();
    // Fresh temp parent dir per test — no .git, no .omg-workspace yet
    parent = mkdtempSync(join(tmpdir(), 'omg-multirepo-'));
  ...
  it('sibling sub-repos both resolve .omg root to the parent workspace anchor', () => {
  ...
  it('marker with {"id":"myws"} derives project identifier from sanitized id', () => {
    writeFileSync(join(parent, '.omg-workspace'), JSON.stringify({ id: 'myws' }));
    clearWorktreeCache();
  ...
  it('OMC_STATE_DIR overrides workspace marker and ignores .omg-workspace', () => {
    writeFileSync(join(parent, '.omg-workspace'), '{}');
    clearWorktreeCache();

    const stateDir = mkdtempSync(join(tmpdir(), 'omg-statedir-'));
    try {
