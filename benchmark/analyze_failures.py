...
def compare_failures(
    ...
    if len(comparison["unique_to_vanilla"]) > len(comparison["unique_to_omc"]):
        ...
        insights.append({
            "type": "negative",
            "message": f"OMG introduced {len(comparison['unique_to_omc'])} new failures compared to vanilla."
        })
...
def generate_failure_report(
    ...
    if comparison:
        lines.extend([
            ...
            f"- **OMG Failures:** {comparison['omc_failures']}",
            f"- **Fixed by OMG:** {len(comparison['unique_to_vanilla'])}",
            f"- **New in OMG:** {len(comparison['unique_to_omc'])}",
...
Examples:
    ...
    if args.compare:
        ...
        omc_results = load_results(args.omg)
        ...
        for pred_path in [args.omg / "predictions.json", args.omg.parent / "omc_predictions.json"]:
            if pred_path.exists():
        ...
        print(f"OMG Failures:     {omc_analysis['total_failures']}")
        print(f"Fixed by OMG:     {len(comparison['unique_to_vanilla'])}")
        print(f"New in OMG:       {len(comparison['unique_to_omc'])}")
