 ...
 * - Linux/fallback: Reads from ~/.grok/.credentials.json
 *
...
function getLegacyCachePath() {
    return join(getClaudeConfigDir(), 'plugins', 'oh-my-grok', '.usage-cache.json');
}
/**
 * Get the provider-specific cache file path
 */
function getCachePath(source) {
    return join(getClaudeConfigDir(), 'plugins', 'oh-my-grok', `.usage-cache-${source}.json`);
}
/**
 ...
 * GROK_CONFIG_DIR value rather than the expanded filesystem path. Preserve
 * that behavior so ~-prefixed profiles keep matching Claude Code's own
 * Keychain entries.
 */
function getKeychainServiceName() {
    const configDir = process.env.GROK_CONFIG_DIR;
    if (configDir) {
 ...
 * When they originated from file, updates ~/.grok/.credentials.json.
