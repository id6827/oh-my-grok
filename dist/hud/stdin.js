...
function getStdinCachePath() {
    ...
    // Legacy flat path must also resolve through the shared OMG-root helper so
    // `OMG_STATE_DIR`-backed deployments land on the same directory as writers.
 ...
 * `omg hud --watch` reader that did not inherit `CLAUDE_SESSION_ID`).
 *
 * Uses the same OMG-root helpers as the writers (`listSessionIds` /
 * `getSessionStateDir`) so this fallback honors `OMG_STATE_DIR` and any
