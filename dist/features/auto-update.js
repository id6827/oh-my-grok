 ...
 * Auto-Update System
 *
 * Provides version checking and auto-update functionality for oh-my-grok.
 *
 * Features:
...
export const REPO_NAME = 'oh-my-grok';
...
function restoreGlobalClaudeCodeIfNeeded(beforeUpdate, verbose = false) {
    ...
    if (verbose) {
        console.log(`[omg update] Restoring global ${packageSpec} after npm update...`);
    }
    ...
    if (verbose) {
        console.log(`[omg update] Restored global ${CLAUDE_CODE_NPM_PACKAGE}`);
    }
    return { restored: true };
}
/**
 * Best-effort sync of the Claude Code marketplace clone.
 * The marketplace clone at ~/.grok/plugins/marketplaces/omg/ is used by
...
function deriveUpdatedPluginInstallPath(existingInstallPath, fallbackInstallPath, newVersion) {
    if (existingInstallPath?.trim()) {
        ...
        if (normalized.includes('/plugins/cache/') && normalized.includes('/oh-my-grok/')) {
...
function syncInstalledPluginRegistryVersion(newVersion, fallbackInstallPath) {
    ...
    try {
        ...
        for (const [pluginId, entriesValue] of Object.entries(plugins)) {
            ...
            const isOmcPlugin = normalizedPluginId === 'oh-my-grok@omg'
                || normalizedPluginId === 'oh-my-grok';
...
export function syncPluginCache(verbose = false) {
    const pluginCacheRoot = join(getClaudeConfigDir(), 'plugins', 'cache', 'omg', 'oh-my-grok');
    ...
    try {
        ...
        const sourceRoot = join(npmRoot, 'oh-my-grok');
        ...
        if (result.synced && result.errors.length === 0) {
            ...
            if (registryResult.updated && verbose) {
                console.log('[omg update] Updated Claude plugin registry');
            }
        }
        if (result.synced) {
...
/** Installation paths (respects GROK_CONFIG_DIR env var) */
export const GROK_CONFIG_DIR = getClaudeConfigDir();
export const VERSION_FILE = join(GROK_CONFIG_DIR, '.omg-version.json');
export const CONFIG_FILE = join(GROK_CONFIG_DIR, OMC_CONFIG_FILE_REL);
/**
 ...
 * Checks ~/.grok/settings.json first, then env var fallback
 */
export function isTeamEnabled() {
    try {
        const settingsPath = join(GROK_CONFIG_DIR, 'settings.json');
...
export function getInstalledVersion() {
    if (!existsSync(VERSION_FILE)) {
        ...
        try {
            ...
            const result = execSync('npm list -g oh-my-grok --json', {
                encoding: 'utf-8',
                timeout: 5000,
                stdio: 'pipe'
            });
            const data = JSON.parse(result);
            if (data.dependencies?.['oh-my-grok']?.version) {
                return {
                    version: data.dependencies['oh-my-grok'].version,
...
export function reconcileUpdateRuntime(options) {
    ...
    // legacy ~/.grok/hooks/* entries and causes duplicate hook execution.
    //
    ...
    try {
        ...
        if (pluginSyncResult.errors.length > 0) {
            ...
            if (options?.verbose) {
                for (const err of pluginSyncResult.errors) {
                    console.warn(`[omg] Plugin cache sync error: ${err}`);
                }
            }
        }
    }
        ...
        if (options?.verbose) {
            console.warn(`[omg] Plugin cache sync error: ${message}`);
        }
    }
...
export async function performUpdate(options) {
    ...
    try {
        ...
        // Standalone terminals may inherit GROK_PLUGIN_ROOT and should still update.
        if (shouldBlockStandaloneUpdateInCurrentSession() && !options?.standalone) {
            return {
                success: false,
                previousVersion,
                newVersion: 'unknown',
                message: 'Running inside an active Claude Code plugin session. Use "/plugin install oh-my-grok" to update, or pass --standalone to force npm update.',
            };
        }
        ...
        try {
            execSync('npm install -g oh-my-grok@latest', npmExecOptions(options?.verbose ?? false));
            try {
            ...
            throw new Error('Auto-update via npm failed. Please run manually:\n' +
                '  npm install -g oh-my-grok@latest\n' +
                'Or use: /plugin install oh-my-grok\n' +
...
export function formatUpdateNotification(checkResult) {
    if (!checkResult.updateAvailable) {
        return `oh-my-grok is up to date (v${checkResult.currentVersion ?? 'unknown'})`;
    }
    const lines = [
        '╔═══════════════════════════════════════════════════════════╗',
        '║           oh-my-grok Update Available!              ║',
        '╚═══════════════════════════════════════════════════════════╝',
        '',
        ...
        '  Or reinstall via: /plugin install oh-my-grok',
        ''
    ];
...
const SILENT_UPDATE_STATE_FILE = join(GROK_CONFIG_DIR, '.omg-silent-update.json');
/**
...
export async function silentAutoUpdate(config = {}) {
    const { checkIntervalHours = 24, autoApply = true, logFile = join(GROK_CONFIG_DIR, '.omg-update.log'), maxRetries = 3 } = config;
