...
/** Patterns that indicate Claude Code is running outside the OMG HUD */
...
/** Anchored OMG HUD status line, not copied output embedded in logs/tests. */
const OMC_HUD_STATUS_LINE_PATTERN = /^\s*\[OMG#[^\]\s]*\]\s*\|.*\b(?:Model:|ctx:|session:|5h:|wk:|thinking)\b/i;
/** OMG HUD mode/help line rendered in the captured UI footer. */
...
/** Rate-limit text shown by the live Claude/OMG limit screen, not arbitrary API/log output. */
...
export function analyzePaneContent(content) {
    ...
    // Check for Claude Code indicators. OMG HUD footer evidence only counts as a
