...
function buildPerStoryClaudeGoalInstruction(goal, plan) {
    ...
    return [
        ...
        '',
        'Claude /goal integration constraints (model-facing — OMG cannot mutate Claude /goal state from a shell):',
...
function buildAggregateClaudeGoalInstruction(goal, plan) {
    ...
    return [
        ...
        '',
        'Claude /goal integration constraints (model-facing — OMG cannot mutate Claude /goal state from a shell):',
        '- Claude /goal = the whole ultragoal run; OMG G001/G002/etc. = ledger stories.',
        ...
        '- If the active /goal already reports the same aggregate objective as active, continue this OMG story without setting a new /goal.',
        ...
        finalStory
            ...
            : '- This is not the final story: do not clear the /goal yet; the aggregate Claude /goal must remain active while later OMG stories remain.',
        finalStory
        ...
        finalStory
            ...
            : null,
        `- Checkpoint this OMG story with a fresh /goal snapshot whose objective matches the aggregate payload and whose status is ${checkpointStatus}:`,
        finalStory
            ? `  omg ultragoal checkpoint --goal-id ${goal.id} --status complete --evidence "<tests/files/PR evidence>" --claude-goal-json "<fresh complete /goal snapshot JSON or path>" --quality-gate-json "<quality gate JSON or path>"`
            : `  omg ultragoal checkpoint --goal-id ${goal.id} --status complete --evidence "<tests/files/PR evidence>" --claude-goal-json "<fresh /goal snapshot JSON or path>"`,
        '- If blocked or failed, checkpoint with --status failed and the failure evidence; rerun complete-goals --retry-failed to resume.',
