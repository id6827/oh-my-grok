 ...
 * - User config: ~/.config/claude-omg/config.jsonc
 * - Project config: .claude/omg.jsonc
...
export function getConfigPaths() {
    ...
    return {
        user: join(userConfigDir, "claude-omg", "config.jsonc"),
        project: join(process.cwd(), ".claude", "omg.jsonc"),
    };
}
/**
...
function warnOnDeprecatedDelegationRouting(config) {
    ...
    console.warn("[OMG] delegationRouting to Codex/Gemini is deprecated and falls back to Claude Task. Use /team for Codex/Gemini CLI workers instead.");
}
/**
...
export function validateTeamConfig(config) {
    ...
    if (ops && typeof ops === "object") {
        if (ops.defaultAgentType !== undefined) {
            if (typeof ops.defaultAgentType !== "string" ||
                ...
                throw new Error(`[OMG] team.ops.defaultAgentType: invalid value "${String(ops.defaultAgentType)}". Allowed: ${[...TEAM_ROLE_PROVIDERS].join(", ")}`);
            }
        }
        if (ops.worktreeMode !== undefined) {
            ...
            if (typeof ops.worktreeMode !== "string" || !allowed.has(ops.worktreeMode)) {
                throw new Error(`[OMG] team.ops.worktreeMode: invalid value "${String(ops.worktreeMode)}". Allowed: ${[...allowed].join(", ")}`);
            }
        }
    }
    ...
    for (const [rawRoleKey, rawSpec] of Object.entries(roleRouting)) {
        ...
        if (!CANONICAL_TEAM_ROLE_SET.has(normalized)) {
            throw new Error(`[OMG] team.roleRouting: unknown role "${rawRoleKey}". Allowed roles: ${[...CANONICAL_TEAM_ROLE_SET].join(", ")}`);
        }
        if (!rawSpec || typeof rawSpec !== "object" || Array.isArray(rawSpec)) {
            throw new Error(`[OMG] team.roleRouting.${rawRoleKey}: must be an object, got ${Array.isArray(rawSpec) ? "array" : typeof rawSpec}`);
        }
        const spec = rawSpec;
        ...
        if (normalized === "orchestrator") {
            for (const key of Object.keys(spec)) {
                if (key !== "model") {
                    throw new Error(`[OMG] team.roleRouting.orchestrator: key "${key}" is not allowed (orchestrator is pinned to claude; only "model" is configurable)`);
                }
            }
            if (spec.model !== undefined && !isValidModelValue(spec.model)) {
                throw new Error(`[OMG] team.roleRouting.orchestrator.model: must be a tier name (HIGH|MEDIUM|LOW) or model ID string, got ${typeof spec.model}`);
            }
            continue;
        }
        if (spec.provider !== undefined) {
            if (typeof spec.provider !== "string" || !TEAM_ROLE_PROVIDERS.has(spec.provider)) {
                throw new Error(`[OMG] team.roleRouting.${rawRoleKey}.provider: invalid value "${String(spec.provider)}". Allowed: ${[...TEAM_ROLE_PROVIDERS].join(", ")}`);
            }
            if (spec.provider === "cursor" && !CURSOR_EXECUTOR_TEAM_ROLE_SET.has(normalized)) {
                throw new Error(`[OMG] team.roleRouting.${rawRoleKey}.provider: cursor is only supported for executor-style roles (${[...CURSOR_EXECUTOR_TEAM_ROLE_SET].join(", ")})`);
            }
        }
        if (spec.model !== undefined && !isValidModelValue(spec.model)) {
            throw new Error(`[OMG] team.roleRouting.${rawRoleKey}.model: must be a tier name (HIGH|MEDIUM|LOW) or a non-empty model ID string`);
        }
        if (spec.agent !== undefined) {
            if (typeof spec.agent !== "string" || !KNOWN_AGENT_NAME_SET.has(spec.agent)) {
                throw new Error(`[OMG] team.roleRouting.${rawRoleKey}.agent: unknown agent "${String(spec.agent)}". Allowed: ${[...KNOWN_AGENT_NAME_SET].join(", ")}`);
            }
        }
    }
}
...
function workflowError(source, path, message) {
    throw new Error(`[OMG] ${source} ${path}: ${message}`);
}
...
export function validateAutopilotConfig(config) {
    ...
    if (autopilot.execution !== undefined &&
        ...
        throw new Error(`[OMG] autopilot.execution: invalid value "${String(autopilot.execution)}". Allowed: ${[...AUTOPILOT_EXECUTION_BACKENDS].join(", ")}`);
    }
    if (autopilot.planning !== undefined &&
        ...
        throw new Error(`[OMG] autopilot.planning: invalid value "${String(autopilot.planning)}". Allowed: ralplan, direct, false`);
    }
    ...
    if (team.agentTypes !== undefined) {
        if (!Array.isArray(team.agentTypes)) {
            throw new Error("[OMG] autopilot.team.agentTypes: must be an array");
        }
        for (const agentType of team.agentTypes) {
            if (typeof agentType !== "string" ||
                !AUTOPILOT_TEAM_AGENT_TYPES.has(agentType)) {
                throw new Error(`[OMG] autopilot.team.agentTypes: invalid value "${String(agentType)}". Allowed: ${[...AUTOPILOT_TEAM_AGENT_TYPES].join(", ")}`);
            }
        }
    }
}
...
function parseTeamRoleOverridesFromEnv() {
    ...
    try {
        ...
        if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
            console.warn("[OMG] OMC_TEAM_ROLE_OVERRIDES: expected a JSON object; ignoring.");
            return undefined;
        }
        return parsed;
    }
    catch (err) {
        console.warn(`[OMG] OMC_TEAM_ROLE_OVERRIDES: invalid JSON, ignoring (${err.message})`);
        return undefined;
    }
}
...
export function generateConfigSchema() {
    return {
        ...
        title: "Oh-My-Grok Configuration",
        type: "object",
        properties: {
            agents: {
                type: "object",
            ...
            routing: {
                ...
                properties: {
                    ...
                    forceInherit: {
                        type: "boolean",
                        default: false,
                        description: "Force all agents to inherit the parent model, bypassing OMG model routing. When true, no model parameter is passed to Task/Agent calls, so agents use the user's Claude Code model setting. Auto-enabled for non-Claude providers (CC Switch, custom ANTHROPIC_BASE_URL), AWS Bedrock, and Google Vertex AI.",
                    },
                },
            },
            externalModels: {
