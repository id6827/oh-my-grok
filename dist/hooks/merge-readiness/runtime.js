...
export function collectMergeReadinessEvidence(directory, baseRef, sessionId) {
    ...
    if (!/test|spec|qa|verify|validation/.test(evidenceText)) {
        missingEvidence.push("No test or verification artifact was detected under .omg.");
    }
    if (!/review|risk|security|verdict/.test(evidenceText)) {
        missingEvidence.push("No review or risk artifact was detected under .omg.");
    }
...
function extractChangeSummary(promptText) {
    return promptText
        .replace(/^\s*\/(?:oh-my-grok:|omg:)?merge-readiness\b/i, "")
...
export function createInitialMergeReadinessState(directory, promptText, sessionId, baseRef) {
    ...
    if (missingEvidence) {
        state.validation_errors = unsupportedFromPr
            ? ["--from-pr is unsupported: merge-readiness uses local git and .omg evidence only."]
...
export function formatMergeReadinessQuestionMessage(state) {
    ...
    if (state.result === "blocked") {
        ...
        const evidenceGuidance = noDiff
            ? `No diff detected. If changes are committed, ${diffHint}; if relying on .omg artifacts, use --from-artifacts. If there are truly no changes, a merge-readiness gate is not needed.`
            : "Produce the test/review evidence under .omg, then re-run /merge-readiness (merge_readiness_start).";
        return [
...
export function handleMergeReadinessPromptSubmit(directory, promptText, sessionId) {
    ...
    const override = /^\/(?:oh-my-grok:|omg:)?merge-readiness\s+--override\s+(.+)$/i.exec(promptText.trim());
    if (!override)
        return { handled: false };
