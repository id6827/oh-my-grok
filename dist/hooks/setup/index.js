 ...
 * current form:
 *   sh "$GROK_PLUGIN_ROOT"/scripts/find-node.sh "$GROK_PLUGIN_ROOT"/scripts/run.cjs "$GROK_PLUGIN_ROOT"/scripts/X.mjs [args]
 * or stale absolute-shell cache form:
 *   "/bin/sh" "$GROK_PLUGIN_ROOT"/scripts/find-node.sh "$GROK_PLUGIN_ROOT"/scripts/run.cjs "$GROK_PLUGIN_ROOT"/scripts/X.mjs [args]
 * or legacy form:
 *   sh "${GROK_PLUGIN_ROOT}/scripts/find-node.sh" "${GROK_PLUGIN_ROOT}/scripts/X.mjs" [args]
 * to:
 *   node "$GROK_PLUGIN_ROOT"/scripts/run.cjs "$GROK_PLUGIN_ROOT"/scripts/X.mjs [args]
 *
...
export function patchHooksJsonForWindows(pluginRoot) {
    ...
    try {
        ...
        // sh "$GROK_PLUGIN_ROOT"/scripts/find-node.sh "$GROK_PLUGIN_ROOT"/scripts/run.cjs "$GROK_PLUGIN_ROOT"/scripts/X.mjs [optional args]
        // Also matches older hotfix cache entries that hardcoded "/bin/sh".
        const currentPattern = /^(?:"\/bin\/sh"|sh) "\$GROK_PLUGIN_ROOT"\/scripts\/find-node\.sh "\$GROK_PLUGIN_ROOT"\/scripts\/run\.cjs "\$GROK_PLUGIN_ROOT"\/scripts\/([^"\s]+)"?(.*)$/;
        // Matches legacy hooks.json:
        // sh "${GROK_PLUGIN_ROOT}/scripts/find-node.sh" "${GROK_PLUGIN_ROOT}/scripts/X.mjs" [optional args]
        const legacyPattern = /^sh "\$\{GROK_PLUGIN_ROOT\}\/scripts\/find-node\.sh" "\$\{GROK_PLUGIN_ROOT\}\/scripts\/([^"\s]+)"?(.*)$/;
        let patched = false;
            ...
            for (const group of groups) {
                for (const hook of group.hooks ?? []) {
                    if (typeof hook.command === 'string') {
                        ...
                        if (m) {
                            hook.command = `node "$GROK_PLUGIN_ROOT"/scripts/run.cjs "$GROK_PLUGIN_ROOT"/scripts/${m[1]}${m[2]}`;
                            patched = true;
                        }
                    }
                }
...
/**
 * Ensure ~/.grok/hooks/lib/stdin.mjs points to the current plugin version.
 *
 * This fixes a silent breakage that occurs when OMG upgrades to a new version:
