...
/**
 * Get the global state file path (in ~/.grok/) for modes that support it
        ...
        for (const exclusiveMode of EXCLUSIVE_MODES) {
            if (exclusiveMode !== mode &&
                ...
                return {
                    ...
                    message: `Cannot start ${MODE_CONFIGS[mode].name} while ${config.name} is active. Cancel ${config.name} first with /oh-my-grok:cancel.`,
                };
            }
        }
    }
 ...
 * - Global state file if applicable (~/.grok/{mode}-state.json)
 *
