 ...
 * 2. Active ralph loop (until cancelled via /oh-my-grok:cancel)
...
async function checkRalphLoop(sessionId, directory, cancelInProgress) {
    ...
    if (hardMax > 0 && state.iteration >= hardMax) {
        ...
        return {
            shouldBlock: true,
            message: `[RALPH - HARD LIMIT] Reached hard max iterations (${hardMax}). Mode auto-disabled. Restart with /oh-my-grok:ralph if needed.`,
            mode: 'ralph',
...
4. When FULLY complete (after ${state.critic_mode === 'codex' ? 'Codex critic' : state.critic_mode === 'critic' ? 'Critic' : 'Architect'} verification), run \`/oh-my-grok:cancel\` to cleanly exit and clean up state files. If cancel fails, retry with \`/oh-my-grok:cancel --force\`.
...
When done, run \`/oh-my-grok:cancel\` to cleanly exit.
...
async function checkUltrawork(sessionId, directory, _hasIncompleteTodos, cancelInProgress) {
    ...
    if (hardMax > 0 && state.reinforcement_count >= hardMax) {
        ...
        return {
            shouldBlock: true,
            message: '[ULTRAWORK - HARD LIMIT] Reached hard max iterations (' + hardMax + '). Mode auto-disabled. Restart with /oh-my-grok:ultrawork if needed.',
            mode: 'ultrawork',
...
async function resolvePersistentModeBlock(sessionId, directory, stopContext // NEW: from todo-continuation types
    ...
    // Fix for: https://github.com/Yeachan-Heo/oh-my-grok/issues/777
