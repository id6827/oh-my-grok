...
export function resolveTaskAssignment(task, resolvedRouting, roleRoutingConfig, resolvedBinaryPaths, fallbackAgent) {
    ...
    if (!hasExplicitRole && !hasConfigForRole) {
        if (fallbackAgent === 'cursor' && !CURSOR_EXECUTOR_TEAM_ROLES.includes(canonical)) {
            throw new Error(`Cursor workers are executor-style only; inferred role "${canonical}" for task "${task.subject}" must run on a native Claude/OMG reviewer agent or another supported CLI worker.`);
        }
        return { agentType: fallbackAgent, model: '', role: canonical };
    }
...
export async function startTeamV2(config) {
    ...
    // Direct launches like `omg team 1:antigravity` flow through `agentTypes` as the
