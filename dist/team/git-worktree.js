 ...
 * Branch naming (branch mode): omg-team/{teamName}/{workerName}
 *
...
function assertCleanLeaderWorktree(repoRoot) {
    const status = git(repoRoot, ['status', '--porcelain'])
        .split('\n')
        .filter(line => line.trim() !== '' && !/^\?\? \.omg(?:\/|$)/.test(line))
        .join('\n')
        .trim();
    if (status.length > 0) {
...
function readRootAgentsBackup(repoRoot, teamName, workerName) {
    ...
    try {
        ...
        process.stderr.write(`[omg] warning: worktree root AGENTS backup parse error: ${msg}\n`);
...
export function installWorktreeRootAgents(teamName, workerName, repoRoot, worktreePath, overlayContent) {
    ...
    // getOmcRoot(repoRoot) — which in a .omg-workspace layout sits ABOVE repoRoot.
    // Validate against the shared OMG root (and the worktree itself for AGENTS.md),
...
function readMetadataResult(repoRoot, teamName) {
    ...
    for (const metaPath of paths) {
        ...
        try {
            ...
            process.stderr.write(`[omg] warning: worktrees.json parse error at ${metaPath}: ${message}
`);
        }
    }
...
export function removeWorkerWorktree(teamName, workerName, repoRoot) {
    ...
    withFileLockSync(metaLockPath, () => {
        ...
        // only after the shared path guard proves it is an OMG team worktree child.
...
export function cleanupTeamWorktrees(teamName, repoRoot) {
    ...
    for (const entry of entries) {
        try {
            ...
            process.stderr.write(`[omg] warning: preserved worktree ${entry.path}: ${reason}\n`);
        }
    }
    return { removed, preserved };
}
