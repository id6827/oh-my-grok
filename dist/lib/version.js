...
export function getRuntimePackageVersion() {
    ...
    // the path itself contains the version: .../oh-my-grok/4.11.2/dist/lib/version.js
    try {
        const __filename = fileURLToPath(import.meta.url);
        const pathMatch = __filename.match(/oh-my-grok\/(\d+\.\d+\.\d+[^/]*)\//);
        if (pathMatch?.[1]) {
            return pathMatch[1];
        }
    }
...
/**
 * Detect whether OMG is running from a local fork / dev install rather
 ...
 *    (e.g. `npm link`, or a manual junction in `~/.grok/plugins/marketplaces/`)
...
export function isRuntimePackageLocal() {
    try {
        ...
        // a parent dir like ~/.grok/plugins/marketplaces/omg is the junction).
        let cursor = pkgRoot;
        for (let i = 0; i < 6; i++) {
