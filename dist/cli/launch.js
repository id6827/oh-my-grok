...
const OMC_RUNTIME_DIRNAME = '.omg-launch';
function hasOmcMarkers(path) {
    if (!existsSync(path))
        return false;
    const content = readFileSync(path, 'utf-8');
    return content.includes('<!-- OMG:START -->') && content.includes('<!-- OMG:END -->');
}
...
export function prepareOmcLaunchConfigDir(baseConfigDir = getClaudeConfigDir()) {
    const companionPath = join(baseConfigDir, 'CLAUDE-omg.md');
    ...
    writeFileSync(join(runtimeConfigDir, '.omg-launch-profile.json'), JSON.stringify({ sourceConfigDir: baseConfigDir, sourceClaudeMd: companionPath }, null, 2));
...
/**
 * Extract the OMG-specific --openclaw flag from launch args.
 ...
 * normalizeClaudeLaunchArgs strips them so we can apply OMG-specific
...
function abortMadmaxRequiresTmux(reason) {
    if (reason === 'missing') {
        console.error('[omg] Error: --madmax/--yolo on macOS requires tmux, but tmux is not installed.');
        console.error('  Install it with: brew install tmux');
    }
    else {
        console.error('[omg] Error: --madmax/--yolo on macOS requires tmux, but launching tmux failed.');
...
function runClaudeInsideTmux(cwd, args) {
    ...
    try {
        ...
        const err = error;
        if (err.code === 'ENOENT') {
            console.error('[omg] Error: claude CLI not found in PATH.');
            process.exit(1);
        }
        // Propagate Claude's exit code so omg does not swallow failures
 ...
 * process's, so vars set on process.env (e.g. GROK_CONFIG_DIR at launch)
...
export async function launchCommand(args) {
    ...
    // Extract OMG-specific --notify flag before passing remaining args to Claude CLI
    const { notifyEnabled, remainingArgs } = extractNotifyFlag(args);
    if (!notifyEnabled) {
        process.env.OMC_NOTIFY = '0';
    }
    // Extract OMG-specific --openclaw flag (presence-based, no value consumption)
    ...
    // Extract OMG-specific --telegram flag (presence-based)
    ...
    // Extract OMG-specific --discord flag (presence-based)
    ...
    // Extract OMG-specific --slack flag (presence-based)
    ...
    // Extract OMG-specific --webhook flag (presence-based)
    ...
    if (process.env.CLAUDECODE) {
        console.error('[omg] Error: Already inside a Claude Code session. Nested launches are not supported.');
        process.exit(1);
    }
    // Pre-flight: check claude CLI availability
    if (!isClaudeAvailable()) {
        console.error('[omg] Error: claude CLI not found. Install Claude Code first:');
