...
export function checkWindowsUnsafePluginHooks() {
    ...
    const roots = [process.env.GROK_PLUGIN_ROOT, ...readInstalledPluginRoots()]
...
function genericClaudeMdFiles(configDir) {
    try {
        return readdirSync(configDir)
            .filter(name => /^CLAUDE-.+\.md$/i.test(name) && name.toLowerCase() !== 'claude-omg.md')
            .sort()
...
export function checkClaudeMdStatus() {
    ...
    const activePath = join(configDir, 'CLAUDE-omg.md');
...
const SETUP_FALLBACK_SKILL_NAMES = new Set(['omg-reference']);
...
function readInstalledPluginRoots() {
    ...
    try {
        ...
        return Object.entries(plugins)
            .filter(([key]) => key.startsWith('oh-my-grok'))
...
function isSupportedSetupFallbackSkill(legacySkillsDir, entry, baseName) {
    ...
    // skills/omg-reference/SKILL.md file into ~/.grok/skills/omg-reference/SKILL.md
    // as a Claude CLI fallback. Suppress only that exact, unmodified sync so real
    // legacy collisions and user-edited omg-reference copies still surface.
...
export function checkConfigIssues() {
    const unknownFields = [];
    const configPath = join(getClaudeConfigDir(), '.omg-config.json');
...
export function checkWorkspaceMarker() {
    ...
    const stateDirEnvValue = process.env.OMG_STATE_DIR && process.env.OMG_STATE_DIR.trim()
        ? process.env.OMG_STATE_DIR.trim()
        : null;
...
export function runConflictCheck() {
    ...
    const hasConflicts = hookConflicts.some(h => !h.isOmc) || // Non-OMG hooks present
...
export function formatReport(report, json) {
    ...
    const lines = [];
    lines.push('');
    lines.push(colors.bold('🔍 Oh-My-Grok Conflict Diagnostic'));
    ...
    if (report.hookConflicts.length > 0) {
        ...
        for (const hook of report.hookConflicts) {
            const status = hook.isOmc ? colors.green('✓ OMG') : colors.yellow('⚠ Other');
    ...
    if (report.claudeMdStatus) {
        ...
        if (report.claudeMdStatus.hasMarkers) {
            ...
            lines.push(`    ${colors.gray('Run /oh-my-grok:omg-setup to add markers to the selected guide')}`);
            if (report.claudeMdStatus.dirtyFiles.length > 0) {
        ...
        if (report.claudeMdStatus.exactLegacyPaths.length > 0) {
            ...
            lines.push(`    ${colors.gray('Run /oh-my-grok:omg-setup for coordinator-backed cleanup with a verified backup.')}`);
        }
    ...
    if (report.windowsUnsafePluginHooks.length > 0) {
        ...
        lines.push(`    ${colors.gray('Run /oh-my-grok:omg-setup or update/reinstall the plugin to rewrite hooks to direct node run.cjs commands.')}`);
        lines.push('');
    }
    // Config issues
    ...
    lines.push('');
    // Workspace marker
    lines.push(colors.bold('🗂  Workspace Marker (.omg-workspace)'));
    lines.push('');
    ...
    if (wm.stateDirEnvSet) {
        lines.push(`  ${colors.green('✓')} OMG_STATE_DIR is set: ${wm.stateDirEnvValue}`);
    }
    else {
        lines.push(`  ${colors.gray('ℹ')} OMG_STATE_DIR not set`);
    }
    if (wm.precedenceConflict) {
        lines.push(`  ${colors.yellow('⚠')} Both OMG_STATE_DIR and ${WORKSPACE_MARKER} are active.`);
        lines.push(`    ${colors.gray('OMG_STATE_DIR takes precedence (resolution order: OMG_STATE_DIR > .omg-workspace > git > cwd).')}`);
        lines.push(`    ${colors.gray('If you intended .omg-workspace to anchor state, unset OMG_STATE_DIR.')}`);
    }
    lines.push('');
    // Summary
    lines.push(colors.gray('━'.repeat(60)));
    if (report.hasConflicts) {
        ...
        lines.push(`${colors.gray('Review the issues above and run /oh-my-grok:omg-setup if needed')}`);
    }
    else {
