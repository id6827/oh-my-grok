...
function cleanCanonical(source) {
    ...
    if (markers.state === 'corrupt')
        throw new Error(`Canonical source has corrupt OMG markers: ${markers.diagnostics.join(', ')}`);
    if (markers.state !== 'complete' || markers.managedRanges.length !== 1)
        throw new Error('Canonical source missing required OMG markers or does not contain exactly one complete managed block');
...
function cleanedExisting(content) {
    ...
    if (analysis.markers.state === 'corrupt')
        throw new Error(`Existing CLAUDE.md has corrupt OMG markers: ${analysis.markers.diagnostics.join(', ')}`);
...
export function executeClaudeMdTransaction(request) {
    ...
    const companion = resolve(root, 'CLAUDE-omg.md');
    try {
        ...
        if (request.mode !== 'local' && companionBytes && parseClaudeMdMarkers(decodeClaudeMdUtf8(companionBytes, companion)).state === 'corrupt')
            throw new Error('Existing companion has corrupt OMG markers');
        const operations = [];
        if (request.mode === 'local')
