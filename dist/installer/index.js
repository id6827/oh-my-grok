...
export const GROK_CONFIG_DIR = getClaudeConfigDir();
export const AGENTS_DIR = join(GROK_CONFIG_DIR, 'agents');
export const COMMANDS_DIR = join(GROK_CONFIG_DIR, 'commands');
export const SKILLS_DIR = join(GROK_CONFIG_DIR, 'skills');
export const HOOKS_DIR = join(GROK_CONFIG_DIR, 'hooks');
export const HUD_DIR = join(GROK_CONFIG_DIR, 'hud');
export const SETTINGS_FILE = join(GROK_CONFIG_DIR, 'settings.json');
export const VERSION_FILE = join(GROK_CONFIG_DIR, '.omg-version.json');
const OMC_MANAGED_SKILL_MARKER = '.omg-managed';
const PLUGIN_FULL_SKILL_BODIES_DIR = 'skill-bodies';
const PLUGIN_COMPACT_SKILL_SHIM_MARKER = '<!-- OMG:COMPACT-PLUGIN-SKILL -->';
/**
...
const OMC_VERSION_MARKER_PATTERN = /<!-- OMG:VERSION:([^\s]+) -->/;
...
function getNewestInstalledVersionHint() {
    ...
    const claudeCandidates = [
        join(GROK_CONFIG_DIR, 'CLAUDE.md'),
...
function buildStatusLineCommand(nodeBin, hudScriptPath, findNodePath, cacheWrapperPath) {
    ...
    if (cacheWrapperPath) {
        if (isDefaultClaudeConfigDirPath(GROK_CONFIG_DIR)) {
            return 'sh ${GROK_CONFIG_DIR:-$HOME/.claude}/hud/omg-hud-cache.sh ${GROK_CONFIG_DIR:-$HOME/.claude}/hud/omg-hud.mjs';
        }
        return `sh ${quoteShellArg(cacheWrapperPath.replace(/\\/g, '/'))} ${quoteShellArg(normalizedHudScriptPath)}`;
    }
    if (isDefaultClaudeConfigDirPath(GROK_CONFIG_DIR)) {
        if (findNodePath) {
            return 'sh ${GROK_CONFIG_DIR:-$HOME/.claude}/hud/find-node.sh ${GROK_CONFIG_DIR:-$HOME/.claude}/hud/omg-hud.mjs';
        }
        return 'node ${GROK_CONFIG_DIR:-$HOME/.claude}/hud/omg-hud.mjs';
    }
    if (findNodePath) {
...
export function isHudEnabledInConfig() {
    const configPath = join(GROK_CONFIG_DIR, OMC_CONFIG_FILE_REL);
...
export function isOmcStatusLine(statusLine) {
    if (!statusLine)
        return false;
    // Legacy string format (pre-v4.5): "~/.grok/hud/omg-hud.mjs"
...
/**
 * Known OMG hook script filenames installed into .claude/hooks/.
...
export function isOmcHook(command) {
    ...
    // Check for known OMG hook filenames in .claude/hooks/ path.
...
export function isRunningAsPlugin() {
    // Check for GROK_PLUGIN_ROOT env var (set by plugin system)
...
export function isProjectScopedPlugin() {
    ...
    // Global plugins are installed under ~/.grok/plugins/
    const globalPluginBase = join(GROK_CONFIG_DIR, 'plugins');
...
function pruneLegacyStandaloneHookScripts(log, activeStandaloneOmcHookFilenames = new Set()) {
    ...
    if (removed > 0) {
        log(`  Removed ${removed} legacy hook script file${removed === 1 ? '' : 's'} from ${basename(GROK_CONFIG_DIR)}/hooks`);
    }
}
...
function mergeHookGroups(eventType, existingGroups, newOmcGroups, options, log, result) {
    ...
    if (options.forceHooks && !options.allowPluginHookRefresh) {
        if (hasNonOmcHook) {
            log(`  Warning: Overwriting non-OMG ${eventType} hook with --force-hooks: ${nonOmcCommand}`);
    ...
    if (options.force) {
        if (hasNonOmcHook) {
            log(`  Merged ${eventType} hooks (updated OMG hooks, preserved non-OMG hook: ${nonOmcCommand})`);
            result.hookConflicts.push({ eventType, existingCommand: nonOmcCommand });
        }
        else {
    ...
    if (hasNonOmcHook) {
        log(`  Warning: ${eventType} hook has non-OMG hook. Skipping. Use --force-hooks to override.`);
        result.hookConflicts.push({ eventType, existingCommand: nonOmcCommand });
    }
    else {
...
export function cleanupStaleAgents(log) {
    ...
    for (const file of readdirSync(agentsDir)) {
        ...
        if (currentAgentFiles.has(file))
            continue;
        // Check if this looks like an OMG-created agent (kebab-case .md with frontmatter)
...
export function prunePluginDuplicateSkills(log) {
    ...
    for (const entry of readdirSync(skillsDir, { withFileTypes: true })) {
        if (!entry.isDirectory())
            continue;
        if (entry.name === 'omg-learned' || entry.name === '.omg-trash')
            continue;
        ...
        try {
            ...
            // plugin's copy, OR the directory is explicitly marked as OMG-owned via the
            // .omg-managed marker file. Frontmatter structure alone is not a reliable
...
export function getInstalledOmcPluginRoots() {
    ...
    const installedPluginsPath = join(GROK_CONFIG_DIR, 'plugins', 'installed_plugins.json');
    ...
    try {
        ...
        for (const [pluginId, entries] of Object.entries(plugins)) {
            if (!pluginId.toLowerCase().includes('oh-my-grok') || !Array.isArray(entries)) {
                continue;
            }
...
function getKnownMarketplaceInstallRoots() {
    const knownMarketplacesPath = join(GROK_CONFIG_DIR, 'plugins', 'known_marketplaces.json');
    ...
    try {
        ...
        for (const [marketplaceId, entry] of Object.entries(raw)) {
            const isOmcMarketplace = marketplaceId.toLowerCase().includes('omg')
                || marketplaceId.toLowerCase().includes('oh-my-grok');
            if (!isOmcMarketplace) {
                continue;
            }
...
function getGlobalInstalledPackageRoot() {
    try {
        ...
        const globalPackageRoot = join(npmRoot, 'oh-my-grok');
...
function isCacheInstalledPluginRoot(root) {
    ...
    const cacheBase = normalizePath(join(GROK_CONFIG_DIR, 'plugins', 'cache'));
...
function renderCompactPluginSkillShim(skillDirName, content) {
    ...
    const rawDescription = getFrontmatterStringValue(parsed.metadata, 'short_description')
        ...
        ?? `Invoke the ${skillDirName} OMG skill.`;
...
export function syncInstalledPluginPayload() {
    ...
    if (!sourceRoot) {
        ...
        return {
            synced: false,
            errors: [
                'Unable to find a complete OMG package source to repair installed plugin roots',
...
export function hasEnabledOmcPlugin() {
    ...
    try {
        ...
        // an enabled OMG plugin entry in either field.
        for (const candidate of [settings.enabledPlugins, settings.plugins]) {
            if (Array.isArray(candidate)) {
                if (candidate.some(plugin => typeof plugin === 'string' && plugin.toLowerCase().includes('oh-my-grok'))) {
                    return true;
                }
            }
            else if (candidate && typeof candidate === 'object') {
                if (Object.entries(candidate).some(([pluginId, value]) => pluginId.toLowerCase().includes('oh-my-grok') && value !== false)) {
                    return true;
                }
            }
        }
...
function markSkillAsOmcManaged(skillDir) {
    writeFileSync(getManagedSkillMarkerPath(skillDir), 'omg-managed\n');
}
...
export function extractOmcVersionFromClaudeMd(content) {
    ...
    const headingMatch = content.match(/^#\s+oh-my-grok.*?\b(v?\d+\.\d+\.\d+(?:[-+][^\s]+)?)\b/m);
    if (headingMatch?.[1]) {
...
export function syncPersistedSetupVersion(options) {
    const configPath = options?.configPath ?? join(GROK_CONFIG_DIR, OMC_CONFIG_FILE_REL);
    let config = {};
    if (existsSync(configPath)) {
    ...
    if (!detectedVersion) {
        const claudeMdPath = options?.claudeMdPath ?? join(GROK_CONFIG_DIR, 'CLAUDE.md');
...
export function mergeClaudeMd(existingContent, omcContent, version) {
    ...
    if (sourceMarkers.state === 'corrupt' || sourceMarkers.managedRanges.length > 1) {
        throw new Error('OMG content must contain at most one complete managed block');
    }
    ...
    cleanOmcContent = cleanOmcContent.replace(/<!-- OMG:VERSION:[^\s]*? -->\n?/, '');
    ...
    if (existingMarkers.state === 'corrupt') {
        throw new Error(`Existing CLAUDE.md has corrupt OMG markers: ${existingMarkers.diagnostics.join(', ')}`);
    }
...
export function install(options = {}) {
    ...
    if (pluginPayloadSync.errors.length > 0) {
        ...
        if (pluginPayloadSync.targetRoots.length > 0) {
            ...
            result.message = 'Installation failed: OMG plugin cache is incomplete and could not be repaired';
            return result;
        }
    }
    if (pluginPayloadSync.synced) {
        ...
        log(`Repaired installed OMG plugin payload from ${sourceSummary} -> ${targetSummary}`);
    }
    ...
    // Dev plugin-dir mode: user launched OMG via `claude --plugin-dir <path>` or
    ...
    if (runningAsPlugin) {
        ...
        log('Detected installed OMG plugin agent definitions - skipping legacy ~/.grok/agents sync');
    }
    ...
    try {
        ...
        if ((!projectScoped || shouldInstallBundledSkills) && !existsSync(GROK_CONFIG_DIR)) {
            mkdirSync(GROK_CONFIG_DIR, { recursive: true });
        }
        ...
        // Plugin system handles these via ${GROK_PLUGIN_ROOT}
        if (!runningAsPlugin) {
            // Create directories
            ...
            // Clean up stale OMG-created agents from previous versions
            ...
            // Commands are accessible via the plugin system (${GROK_PLUGIN_ROOT}/commands/)
            ...
            // Standalone installs still need ~/.grok/hooks/* scripts because their
            // settings.json hook entries execute those local paths directly. Plugin installs
            // keep using hooks/hooks.json + scripts/ under GROK_PLUGIN_ROOT.
        ...
        if (shouldInstallBundledSkills) {
            log(options.noPlugin
                ...
                : !enabledOmcPlugin
                    ? 'Installing bundled skills from local package (no enabled OMG plugin detected)...'
        ...
        // Clean up stale OMG-created skills from previous versions
        ...
        if (!projectScoped) {
            ...
            try {
                ...
                const templateVersionStamp = {
                    version: targetVersion,
                    installedAt: new Date().toISOString(),
                    pluginRoot: process.env.GROK_PLUGIN_ROOT ?? null
                };
