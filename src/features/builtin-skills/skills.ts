...
import { rewriteOmcCliInvocations } from '../../utils/omg-cli-rendering.js';
...
/**
 * Claude Code native commands that must not be shadowed by OMG skill short names.
...
function readDeepInterviewThresholdFromSettings(path: string): number | null {
  ...
  const omg = settings?.omg;
  if (!omg || typeof omg !== 'object' || Array.isArray(omg)) {
    return null;
  }

  const deepInterview = (omg as Record<string, unknown>).deepInterview;
...
function getDeepInterviewAmbiguityThresholdResolution(): DeepInterviewThresholdResolution {
  ...
  if (profileThreshold !== null) {
    return { threshold: profileThreshold, source: '[$GROK_CONFIG_DIR|~/.claude]/settings.json' };
  }
...
function readSkillBodyOverride(skillPath: string, metadata: Record<string, unknown>, fallbackBody: string): string {
  const bodyPath = getFrontmatterString(metadata, 'omg-full-body');
  if (!bodyPath) {
    return fallbackBody;
  }
...
function applyDeepInterviewRuntimeSettings(template: string): string {
  ...
  const withRuntimeSettings = withResolvedPlaceholders.includes('3.5. **Load runtime settings**:')
    ...
    : withResolvedPlaceholders.replace(
      ...
      [
        `3.5. **Load runtime settings** from \`~/.grok/settings.json\` and \`./.claude/settings.json\` before state init (project overrides profile). For this run, use \`ambiguityThreshold = ${threshold}\`.`,
        '4. **Initialize state** via `state_write(mode="deep-interview")`:',
      ].join('\n'),
    );
...
function normalizeSkillNameForRuntimeRendering(skillName: string): string {
  return skillName.trim().toLowerCase().replace(/^oh-my-grok:/, '').replace(/^omg:/, '');
}
