...
function canonicalizeSubagentType(subagentType: string): string {
  const hasPrefix = subagentType.startsWith('oh-my-grok:');
  const rawAgentType = subagentType.replace(/^oh-my-grok:/, '');
  const canonicalAgentType = normalizeDelegationRole(rawAgentType);
  return hasPrefix ? `oh-my-grok:${canonicalAgentType}` : canonicalAgentType;
}
...
export function enforceModel(agentInput: AgentInput): EnforcementResult {
  ...
  const agentType = canonicalSubagentType.replace(/^oh-my-grok:/, '');
...
export function getModelForAgent(agentType: string): string {
  const normalizedType = normalizeDelegationRole(agentType.replace(/^oh-my-grok:/, ''));
