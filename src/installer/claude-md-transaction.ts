...
export const CLAUDE_MD_IMPORT_START = '<!-- OMG:IMPORT:START -->';
export const CLAUDE_MD_IMPORT_END = '<!-- OMG:IMPORT:END -->';
...
function cleanCanonical(source: string): string {
  ...
  if (markers.state === 'corrupt') throw new Error(`Canonical source has corrupt OMG markers: ${markers.diagnostics.join(', ')}`);
  if (markers.state !== 'complete' || markers.managedRanges.length !== 1) throw new Error('Canonical source missing required OMG markers or does not contain exactly one complete managed block');
...
function renderManaged(canonical: string, version?: string): string {
  const body = cleanCanonical(canonical).replace(/<!-- OMG:VERSION:[^\s]*? -->\r?\n?/g, '');
  return `${OMC_START_MARKER}\n${version ? `<!-- OMG:VERSION:${version} -->\n` : ''}${body}\n${OMC_END_MARKER}\n`;
}
function importRanges(content: string): Array<{ start: number; end: number }> {
  ...
  for (let index = 0; index + 2 < lines.length; index += 1) {
    if (lines[index].text === CLAUDE_MD_IMPORT_START && lines[index + 1].text === '@CLAUDE-omg.md' && lines[index + 2].text === CLAUDE_MD_IMPORT_END) {
...
function cleanedExisting(content: string): { content: string; ranges: Array<{ start: number; end: number }>; variants: string[] } {
  ...
  if (analysis.markers.state === 'corrupt') throw new Error(`Existing CLAUDE.md has corrupt OMG markers: ${analysis.markers.diagnostics.join(', ')}`);
...
function atomicWrite(operation: PlannedOperation, fs: ClaudeMdTransactionFs): void {
  ...
  operation.tempPath = `${directory}/.${basename(operation.path)}.omg-tmp-${randomBytes(12).toString('hex')}`;
...
export function executeClaudeMdTransaction(request: ClaudeMdTransactionRequest): ClaudeMdTransactionResult {
  ...
  const main = resolve(root, 'CLAUDE.md'); const companion = resolve(root, 'CLAUDE-omg.md');
  try {
    ...
    if (request.mode !== 'local' && companionBytes && parseClaudeMdMarkers(decodeClaudeMdUtf8(companionBytes, companion)).state === 'corrupt') throw new Error('Existing companion has corrupt OMG markers');
