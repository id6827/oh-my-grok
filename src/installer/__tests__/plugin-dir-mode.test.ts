 ...
 *   2. `OMC_PLUGIN_ROOT` env var (set by `omg --plugin-dir`) → CLI auto-detects
 ...
 *   5. Real OMG plugin enabled → existing skip behavior unchanged (independent
 *      of pluginDirMode).
 *
...
beforeEach(() => {
  testDir = mkdtempSync(join(tmpdir(), 'omg-pdm-'));
...
describe('5. real OMG plugin enabled → existing skip behavior unchanged', () => {
