 ...
 * `<configDir>/plugins/cache/omg/oh-my-grok/0.0.0-test-stub/dist/hud/index.js`.
...
function makeStubConfigDir(rootDir: string): string {
  ...
  const stubDir = join(
    configDir,
    'plugins', 'cache', 'omg', 'oh-my-grok', CACHE_STUB_VERSION, 'dist', 'hud',
  );
...
describe('HUD wrapper — OMC_PLUGIN_ROOT resolution', () => {
  ...
  it('case 5: symmetry — installer source and plugin-setup.mjs both produce identical wrapper text', async () => {
    ...
    expect(txt).not.toContain('Workspace/oh-my-grok');
    expect(txt).not.toContain('projects/oh-my-grok');
  });
