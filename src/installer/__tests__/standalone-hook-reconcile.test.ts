...
function writeCompletePluginPayload(root: string): void {
  ...
  writePluginFile(join(root, 'commands', 'omg-setup.md'), 'Read skills/omg-setup/SKILL.md and pass $ARGUMENTS.\n');
...
describe('install() standalone hook reconciliation', () => {
  beforeEach(() => {
    testClaudeDir = mkdtempSync(join(tmpdir(), 'omg-standalone-hooks-'));
    testHomeDir = mkdtempSync(join(tmpdir(), 'omg-home-'));
  ...
  it('restores OMG settings hooks for standalone installs during forced reconciliation', async () => {
  ...
  it('installs standalone hooks with all runtime helper imports', async () => {
    const projectDir = mkdtempSync(join(tmpdir(), 'omg-standalone-hook-project-'));
    try {
  ...
  it('preserves non-OMG ~/.claude/hooks commands while adding standalone OMG hooks', async () => {
    const settingsPath = join(testClaudeDir, 'settings.json');
    mkdirSync(testClaudeDir, { recursive: true });
  ...
  it('removes legacy OMG settings hooks in plugin mode without re-injecting them', async () => {
    const settingsPath = join(testClaudeDir, 'settings.json');
...
describe('install() plugin-provided hook deduplication (#2252)', () => {
  ...
  beforeEach(() => {
    testClaudeDir = mkdtempSync(join(tmpdir(), 'omg-hook-dedup-'));
    testHomeDir = mkdtempSync(join(tmpdir(), 'omg-home-dedup-'));
    mkdirSync(testHomeDir, { recursive: true });
    writeFileSync(join(testHomeDir, 'CLAUDE.md'), '# test home claude');
    process.env.CLAUDE_CONFIG_DIR = testClaudeDir;
    process.env.HOME = testHomeDir;
  ...
  function setupPluginWithHooks() {
    ...
    fakePluginRoot = mkdtempSync(join(tmpdir(), 'omg-fake-plugin-'));
  ...
  it('does not write OMG hook entries to settings.json when plugin provides hooks', async () => {
    setupPluginWithHooks();
    ...
    expect(result.success).toBe(true);
    // OMG hooks should NOT be in settings.json — plugin handles them via hooks.json
    expect(writtenSettings.hooks).toBeUndefined();
  });

  it('removes stale OMG hook entries from settings.json when plugin provides hooks', async () => {
    // Pre-populate settings.json with stale OMG hook entries (simulating prior standalone install)
    mkdirSync(testClaudeDir, { recursive: true });
    writeFileSync(join(testClaudeDir, 'settings.json'), JSON.stringify({
      enabledPlugins: { 'oh-my-grok': true },
      hooks: {
    ...
    expect(result.success).toBe(true);
    // Stale OMG hook entries should be cleaned up by legacy cleanup,
    // and NOT re-added because plugin provides hooks
    expect(writtenSettings.hooks).toBeUndefined();
  });

  it('prunes legacy standalone OMG hook files when plugin handles hooks', async () => {
    setupPluginWithHooks();
  ...
  it('preserves helpers when a default-format standalone OMG hook remains active in plugin mode', async () => {
    setupPluginWithHooks();
  ...
  it('removes default-format standalone OMG hook entries before plugin stale helper pruning', async () => {
    setupPluginWithHooks();
  ...
  it('preserves same-basename non-OMG hook files while pruning shipped OMG payloads', async () => {
    setupPluginWithHooks();
  ...
  it('does not prune standalone hook files when plugin is not handling hooks', async () => {
    ...
    writeFileSync(join(hooksDir, 'keyword-detector.mjs'), 'legacy omg payload');
  ...
  it('preserves non-OMG hooks in settings.json when pruning plugin duplicates', async () => {
    // Set up plugin first (creates settings.json with enabledPlugins)
    setupPluginWithHooks();

    // Then overwrite settings.json with mixed OMG + non-OMG hooks
    writeFileSync(join(testClaudeDir, 'settings.json'), JSON.stringify({
      enabledPlugins: { 'oh-my-grok': true },
      hooks: {
        UserPromptSubmit: [
