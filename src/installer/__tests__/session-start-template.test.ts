...
${'- preserve this startup guidance\n'.repeat(400)}
  ...
  it('compacts large OMG AGENTS guidance and caps aggregate session context', () => {
...
describe('session-start PID-aware liveness (#E2)', () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'omg-pid-liveness-'));
    fakeHome = join(tempDir, 'home');
    fakeProject = join(tempDir, 'project');
    // validateCwd in session-start.mjs requires .git or .omg-workspace
    mkdirSync(join(fakeProject, '.git'), { recursive: true });
    mkdirSync(join(fakeProject, '.omg', 'state'), { recursive: true });
  });
...
describe('session-start template cwd validation (Wave B1)', () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'omg-session-start-cwdval-'));
    fakeHome = join(tempDir, 'home');
    mkdirSync(fakeHome, { recursive: true });
    // A truly empty directory with no .omg-workspace or .git marker.
    // Keep it under fakeHome so validateCwd stops before ambient /tmp markers.
    emptyCwd = mkdtempSync(join(fakeHome, 'omg-empty-cwd-'));
  });
  ...
  it('emits warning to stderr and outputs no-op JSON when cwd has no .omg-workspace or .git marker', () => {
    ...
    // Must warn on stderr
    expect(stderr).toContain('[OMG] session-start: refusing to use cwd');
    expect(stderr).toContain('no .omg-workspace or .git marker');
  ...
  it('does NOT warn or skip when cwd contains a .git directory', () => {
    const gitProject = mkdtempSync(join(tmpdir(), 'omg-git-project-'));
    try {
      ...
      expect(stderr).not.toContain('[OMG] session-start: refusing to use cwd');
  ...
  it('does NOT warn or skip when cwd contains a .omg-workspace marker', () => {
    const wsProject = mkdtempSync(join(tmpdir(), 'omg-ws-project-'));
    try {
      writeFileSync(join(wsProject, '.omg-workspace'), '{}');
  ...
  it('does NOT warn or skip when cwd is a SUBDIRECTORY of a .git repo (walks up)', () => {
    const gitProject = mkdtempSync(join(tmpdir(), 'omg-git-subdir-'));
    try {
      mkdirSync(join(gitProject, '.git'), { recursive: true });
      mkdirSync(join(gitProject, '.omg', 'state'), { recursive: true });
  ...
  it('does NOT warn or skip when cwd is a nested SUBDIR whose only anchor is a .omg-workspace at an ancestor (no .git anywhere)', () => {
    const wsRoot = mkdtempSync(join(tmpdir(), 'omg-ws-ancestor-'));
    try {
      // Place .omg-workspace at the ancestor root (no .git anywhere)
      writeFileSync(join(wsRoot, '.omg-workspace'), '{}');
      // cwd is a deeply nested subdirectory — no .git, no .omg-workspace at this level
      ...
      // validateCwd must walk up, find .omg-workspace at wsRoot, and accept
      expect(stderr).not.toContain('[OMG] session-start: refusing to use cwd');
