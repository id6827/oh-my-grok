/**
 * Stale OMG Agent/Skill Cleanup Tests
 *
 * Verifies that the installer removes stale OMG-created files from the config
 * directory while preserving user-created files.
 *
 * Contract: setup must clean up ~/.claude/agents and ~/.claude/skills that were
 * created by OMG in previous versions but are no longer shipped.
 */
...
function createUserFile(dir: string, filename: string): void {
  // User-created file without OMG frontmatter
...
function createManagedSkillMarker(dir: string, skillName: string): void {
  writeFileSync(join(dir, skillName, '.omg-managed'), 'omg-managed\n');
}
...
describe('cleanupStaleAgents', () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'omg-stale-agents-'));
  ...
  it('removes agent files that have OMG frontmatter but are no longer in the package', async () => {
    ...
    // Create a fake "stale" agent that looks like OMG-created but isn't in current package
  ...
  it('preserves user-created files without OMG frontmatter', async () => {
    vi.resetModules();
    const { cleanupStaleAgents: cleanup, AGENTS_DIR: agentsDir } = await import('../index.js');
...
describe('cleanupStaleSkills', () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'omg-stale-skills-'));
    originalConfigDir = process.env.CLAUDE_CONFIG_DIR;
    process.env.CLAUDE_CONFIG_DIR = tempDir;
    log.mockClear();
  });
  ...
  it('removes stale skills only when OMG ownership is explicitly marked', async () => {
    vi.resetModules();
  ...
  it('preserves user-created skill directories without OMG frontmatter', async () => {
    vi.resetModules();
    const { cleanupStaleSkills: cleanup, SKILLS_DIR: skillsDir } = await import('../index.js');
  ...
  it('preserves third-party skills with standard frontmatter when no OMG marker is present', async () => {
    vi.resetModules();
    const { cleanupStaleSkills: cleanup, SKILLS_DIR: skillsDir } = await import('../index.js');
  ...
  it('preserves symlinked skill directories without an OMG marker', async () => {
    vi.resetModules();
    const { cleanupStaleSkills: cleanup, SKILLS_DIR: skillsDir } = await import('../index.js');

    mkdirSync(skillsDir, { recursive: true });

    const externalRoot = mkdtempSync(join(tmpdir(), 'omg-third-party-skill-'));
  ...
  it('preserves omg-learned directory (user-created skills)', async () => {
    vi.resetModules();
    const { cleanupStaleSkills: cleanup, SKILLS_DIR: skillsDir } = await import('../index.js');

    mkdirSync(skillsDir, { recursive: true });

    // omg-learned is the user skills directory — must never be removed
    createSkillDir(skillsDir, 'omg-learned', 'omg-learned');

    const removed = cleanup(log);

    expect(removed).not.toContain('omg-learned');
    expect(existsSync(join(skillsDir, 'omg-learned'))).toBe(true);
  });
...
describe('prunePluginDuplicateSkills', () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'omg-prune-dupes-'));
    originalConfigDir = process.env.CLAUDE_CONFIG_DIR;
    process.env.CLAUDE_CONFIG_DIR = tempDir;
    log.mockClear();
  });
  ...
  it('removes standalone skills that match plugin-provided skills when marked as OMG-owned', async () => {
    vi.resetModules();
    ...
    // and mark it as OMG-owned — this is what a prior `omg setup` would have done
    createSkillDir(skillsDir, 'ralph', 'ralph');
  ...
  it('preserves user-authored skills without OMG frontmatter even if name matches', async () => {
    vi.resetModules();
    const { prunePluginDuplicateSkills: prune, SKILLS_DIR: skillsDir } = await import('../index.js');

    mkdirSync(skillsDir, { recursive: true });

    // User-created skill with a name that collides with plugin skill but no OMG frontmatter
  ...
  it('preserves user skills with standard frontmatter that have different content from plugin version (issue #2573)', async () => {
    ...
    // `---\nname:` frontmatter as OMG-owned and deleted it during update,
    ...
    // No .omg-managed marker — this is user-owned
  ...
  it('removes exact-match standalone alias duplicates like omg-plan while preserving alias lookup behavior', async () => {
    vi.resetModules();
    const { prunePluginDuplicateSkills: prune, SKILLS_DIR: skillsDir } = await import('../index.js');
    ...
    const aliasSkillDir = join(skillsDir, 'omg-plan');
    mkdirSync(aliasSkillDir, { recursive: true });
    writeFileSync(join(aliasSkillDir, 'SKILL.md'), packagePlanSkill);

    const removed = prune(log);

    expect(removed).toContain('omg-plan');
    expect(existsSync(aliasSkillDir)).toBe(false);
  });

  it('preserves user-authored standalone alias skills like omg-plan when content differs from plugin copy', async () => {
    vi.resetModules();
    const { prunePluginDuplicateSkills: prune, SKILLS_DIR: skillsDir } = await import('../index.js');
    ...
    expect(removed).not.toContain('omg-plan');
    expect(existsSync(aliasSkillDir)).toBe(true);
  });

  it('preserves omg-learned directory', async () => {
    vi.resetModules();
    const { prunePluginDuplicateSkills: prune, SKILLS_DIR: skillsDir } = await import('../index.js');
...
describe('prunePluginDuplicateAgents', () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'omg-prune-agent-dupes-'));
    originalConfigDir = process.env.CLAUDE_CONFIG_DIR;
    process.env.CLAUDE_CONFIG_DIR = tempDir;
    log.mockClear();
  });
  ...
  it('preserves user-created agents without OMG frontmatter', async () => {
    vi.resetModules();
    const { prunePluginDuplicateAgents: prune, AGENTS_DIR: agentsDir } = await import('../index.js');
