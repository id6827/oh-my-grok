...
describe('install() CLAUDE.md target resolution', () => {
  beforeEach(() => {
    tempRoot = mkdtempSync(join(tmpdir(), 'omg-claude-target-'));
  ...
  it('updates ~/.claude/CLAUDE.md even when ~/CLAUDE.md exists', async () => {
    ...
    expect(updatedConfig).toContain(`<!-- OMG:VERSION:${VERSION} -->`);
  ...
  it('preserves project-scoped behavior by skipping global CLAUDE.md writes', async () => {
    process.env.GROK_PLUGIN_ROOT = join(tempRoot, 'project', '.claude', 'plugins', 'oh-my-grok');
