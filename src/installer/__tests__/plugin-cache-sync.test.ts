...
function writePayloadTree(root: string, version = '9.9.9-test'): void {
  ...
  writeFile(join(root, 'commands', 'omg-setup.md'), 'Read skills/omg-setup/SKILL.md and pass $ARGUMENTS.\n');
  writeFile(join(root, 'templates', 'deliverables.json'), '{}\n');
  writeFile(join(root, 'docs', 'CLAUDE.md'), '# docs\n');
  writeFile(join(root, '.claude-plugin', 'plugin.json'), JSON.stringify({ name: 'oh-my-grok', commands: './commands/', skills: ['./skills/plan/'] }, null, 2));
...
describe('syncInstalledPluginPayload', () => {
  ...
  beforeEach(() => {
    tempRoot = mkdtempSync(join(tmpdir(), 'omg-plugin-cache-sync-'));
  ...
  it('repairs incomplete cache installs from the known marketplace source instead of reusing the installed root', async () => {
    ...
    const cacheRoot = join(configDir, 'plugins', 'cache', 'omg', 'oh-my-grok', '4.12.0');
    ...
    writeFileSync(
      ...
      JSON.stringify({
        version: 2,
        plugins: {
          'oh-my-grok@omg': [{ installPath: cacheRoot, version: '4.12.0' }],
        },
      }, null, 2),
    );
    writeFileSync(
    ...
    expect(existsSync(join(cacheRoot, 'commands', 'omg-setup.md'))).toBe(true);
  ...
  it('repairs incomplete cache installs during setup before plugin-provided file detection runs', async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({ enabledPlugins: ['oh-my-grok@omg'] }, null, 2),
    );
  ...
  it('does not accept a cache root as plugin-provided when required commands are missing', async () => {
    const configDir = process.env.CLAUDE_CONFIG_DIR as string;
    const cacheRoot = join(configDir, 'plugins', 'cache', 'omg', 'oh-my-grok', '4.14.4');
    ...
    writeFileSync(
      ...
      JSON.stringify({
        version: 2,
        plugins: {
          'oh-my-grok@omg': [{ installPath: cacheRoot, version: '4.14.4' }],
        },
      }, null, 2),
    );
  ...
  it('rejects partial command and manifest-declared skill surfaces', async () => {
    ...
    writePayloadTree(cacheRoot, '4.14.4');
    rmSync(join(cacheRoot, 'commands', 'omg-setup.md'), { force: true });
    ...
    expect(validation.valid).toBe(false);
    expect(validation.errors).toEqual(expect.arrayContaining([
      'Missing required plugin command file: commands/omg-setup.md',
  ...
  it('rejects required plugin file paths that exist only as directories', async () => {
    ...
    rmSync(join(cacheRoot, 'commands', 'omg-setup.md'), { force: true });
    mkdirSync(join(cacheRoot, 'commands', 'omg-setup.md'), { recursive: true });
  ...
  it('rejects cache install roots that escape the cache directory via .. segments', async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        version: 2,
        plugins: {
          'oh-my-grok@omg': [{ installPath: escapedInstallPath, version: '4.12.0' }],
        },
      }, null, 2),
    );
    writeFileSync(
