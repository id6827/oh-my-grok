...
describe('install() user-skill compatibility shims', () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'omg-installer-user-skill-compat-'));
  ...
  it('syncs existing omg-learned user skills into flat Claude Code skill directories during install', async () => {
    const learnedDir = join(tempDir, 'skills', 'omg-learned');
