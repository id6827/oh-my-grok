...
beforeEach(() => {
  testDir = mkdtempSync(join(tmpdir(), 'omg-pdm-e2e-'));
  savedEnv = {};
...
describe('install() — plugin-dir-mode end-to-end filesystem shape', () => {
  it('case 1: pluginDirMode=true → installs HUD/CLAUDE.md/settings/.omg-config but NOT agents/skills', async () => {
    ...
    expect(claudeMdContent).toContain('<!-- OMG:START -->');
    expect(claudeMdContent).toContain('<!-- OMG:END -->');
    ...
    // .omg-config.json present
    expect(existsSync(join(testDir, '.omg-config.json'))).toBe(true);
