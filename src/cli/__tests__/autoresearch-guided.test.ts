...
describe('spawnAutoresearchTmux', () => {
  ...
  it('uses explicit cwd, login-shell wrapping, and verifies startup before logging success', () => {
    ...
    expect(tmuxExecMock).toHaveBeenCalledWith(['set-option', '-t', 'omg-autoresearch-demo', 'set-clipboard', 'on'], { stripTmux: true, stdio: 'ignore' });
    expect(tmuxExecMock).toHaveBeenCalledWith(['set-option', '-at', 'omg-autoresearch-demo', 'terminal-features', ',*:clipboard'], { stripTmux: true, stdio: 'ignore' });
    ...
    try {
      ...
      tmuxExecMock.mockImplementation((args: string[]) => {
        if (args[0] === 'new-session') {
          ...
          expect(String(args[9])).toContain(`CODEX_HOME=${repo}/.omx/tmp/omg-autoresearch-setup-kf12oi/codex-home`);
      ...
      expect(buildTmuxShellCommandWithEnvMock).toHaveBeenCalledWith(
        ...
        { CODEX_HOME: `${repo}/.omx/tmp/omg-autoresearch-setup-kf12oi/codex-home` },
      );
      expect(wrapWithLoginShellMock).toHaveBeenCalledWith(`CODEX_HOME=${repo}/.omx/tmp/omg-autoresearch-setup-kf12oi/codex-home claude --dangerously-skip-permissions`);
      ...
      expect(tmuxExecMock).toHaveBeenCalledWith(['set-option', '-t', 'omg-autoresearch-setup-kf12oi', 'set-clipboard', 'on'], { stripTmux: true, stdio: 'ignore' });
      expect(tmuxExecMock).toHaveBeenCalledWith(['set-option', '-at', 'omg-autoresearch-setup-kf12oi', 'terminal-features', ',*:clipboard'], { stripTmux: true, stdio: 'ignore' });
  ...
  it('builds native Windows-friendly setup commands without env-prefix argv hacks', async () => {
    ...
    try {
      ...
      expect(wrapWithLoginShellMock).toHaveBeenCalledWith(
        `CODEX_HOME=${repo}/.omx/tmp/omg-autoresearch-setup-kf12oi/codex-home claude --dangerously-skip-permissions`,
      );
    } finally {
