 ...
 *
 * Verifies that env vars set on the omg process actually arrive inside
...
describe.skipIf(!HAS_TMUX)('tmux env forwarding — integration', () => {
  const SESSION_NAME = `omg-env-test-${Date.now()}`;
  let tempDir: string;
  let outFile: string;

  beforeAll(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'omg-tmux-env-'));
  ...
  it('CLAUDE_CONFIG_DIR set via buildEnvExportPrefix reaches the tmux pane', () => {
    const testValue = '/tmp/omg-test-config-dir';
