...
describe('omg ask command', () => {
  ...
  it('allows antigravity ask inside a Claude Code session', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omg-ask-cli-antigravity-nested-'));
    try {
      const stubPath = writeAdvisorStub(wd);
      const result = runCli(
  ...
  it('allows cursor ask inside a Claude Code session', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omg-ask-cli-cursor-nested-'));
    try {
      const stubPath = writeAdvisorStub(wd);
      const result = runCli(
...
describe('run-provider-advisor script contract', () => {
  ...
  it.each([
    ...
    const wd = mkdtempSync(join(tmpdir(), `omg-ask-${provider}-advisor-env-`));
    try {
  ...
  it('launches grok as `grok -p <prompt> --always-approve` and never pipes stdin', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omg-ask-grok-args-'));
    try {
      const capturePath = join(wd, 'spawn-sync-calls.json');
      const preludePath = writeSpawnSyncCapturePrelude(wd);
  ...
  it('launches cursor as `cursor-agent --print --force --trust --sandbox disabled <prompt>` and never pipes stdin', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omg-ask-cursor-args-'));
    try {
      const capturePath = join(wd, 'spawn-sync-calls.json');
      const preludePath = writeSpawnSyncCapturePrelude(wd);
  ...
  it('guards antigravity on Windows with a clear error and never spawns agy', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omg-ask-antigravity-win32-guard-'));
    try {
      const capturePath = join(wd, 'spawn-sync-calls.json');
  ...
  it('pipes the Windows gemini prompt over stdin to avoid --prompt conflicts and AttachConsole failures', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omg-ask-gemini-win32-stdin-'));
    try {
      const capturePath = join(wd, 'spawn-sync-calls.json');
      const preludePath = writeSpawnSyncCapturePrelude(wd);
      const result = runAdvisorScriptWithPrelude(
  ...
  it('pipes multiline codex prompts over stdin on non-Windows shells', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omg-ask-codex-multiline-stdin-'));
  ...
  it('pipes long gemini prompts over stdin on non-Windows shells', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omg-ask-gemini-long-stdin-'));
  ...
  it('passes multiline antigravity prompts as the -p arg value (agy cannot read the prompt from stdin)', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omg-ask-antigravity-multiline-argv-'));
    const multilinePrompt = 'line one\nline two\nline three';
    try {
      const capturePath = join(wd, 'spawn-sync-calls.json');
  ...
  it('passes long antigravity prompts as the -p arg value (no stdin pipe path)', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omg-ask-antigravity-long-argv-'));
    const longPrompt = `prefix ${'x'.repeat(520)}`;
    try {
      const capturePath = join(wd, 'spawn-sync-calls.json');
      const preludePath = writeSpawnSyncCapturePreludeNative(wd);
  ...
  it('launches antigravity as `agy --dangerously-skip-permissions -p <prompt>` for short prompts', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omg-ask-antigravity-short-argv-'));
  ...
  it('treats an empty-output antigravity run (exit 0, no stdout) as a failure (#76)', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omg-ask-antigravity-empty-'));
    try {
      const capturePath = join(wd, 'spawn-sync-calls.json');
      const preludePath = writeSpawnSyncCapturePreludeNative(wd);
      const result = runAdvisorScriptWithPrelude(
  ...
  it('treats an antigravity timeout/hang (SIGTERM/ETIMEDOUT) as a failure (#76)', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omg-ask-antigravity-timeout-'));
    try {
      const capturePath = join(wd, 'spawn-sync-calls.json');
      const preludePath = writeSpawnSyncCapturePreludeNative(wd);
      const result = runAdvisorScriptWithPrelude(
  ...
  it('spawns antigravity with a hard-kill timeout bound (SIGKILL) so a hung agy cannot block forever (#76)', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omg-ask-antigravity-killcfg-'));
    try {
      const capturePath = join(wd, 'spawn-sync-calls.json');
      const preludePath = writeSpawnSyncCapturePreludeNative(wd);
      const result = runAdvisorScriptWithPrelude(
  ...
  it('ignores an invalid OMC_ANTIGRAVITY_TIMEOUT_MS override and falls back to the default bound', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omg-ask-antigravity-badtimeout-'));
    try {
      const capturePath = join(wd, 'spawn-sync-calls.json');
      const preludePath = writeSpawnSyncCapturePreludeNative(wd);
      const result = runAdvisorScriptWithPrelude(
  ...
  it('pipes multiline claude prompts over stdin so the prompt is never a raw argv value (#3221)', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omg-ask-claude-multiline-stdin-'));
    const multilinePrompt = 'line one\nline two\nline three';
    try {
      const capturePath = join(wd, 'spawn-sync-calls.json');
      const preludePath = writeSpawnSyncCapturePreludeNative(wd);
  ...
  it('pipes frontmatter claude prompts over stdin so a leading dash is not parsed as a CLI option (#3221)', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omg-ask-claude-frontmatter-stdin-'));
  ...
  it('pipes a short claude prompt that begins with a dash over stdin instead of as argv (#3221)', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omg-ask-claude-leading-dash-stdin-'));
  ...
  it('keeps a short single-line claude prompt as a `-p <prompt>` argv without piping stdin', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omg-ask-claude-short-argv-'));
    const shortPrompt = 'review this change';
    try {
      const capturePath = join(wd, 'spawn-sync-calls.json');
      const preludePath = writeSpawnSyncCapturePreludeNative(wd);
  ...
  it.each([
    ...
    const wd = mkdtempSync(join(tmpdir(), `omg-ask-${provider}-stdin-close-`));
    try {
      const capturePath = join(wd, 'spawn-sync-calls.json');
      const preludePath = writeSpawnSyncCapturePreludeNative(wd);
      const result = runAdvisorScriptWithPrelude(
