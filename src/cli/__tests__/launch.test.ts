...
// ---------------------------------------------------------------------------
// runClaude — OMG HUD pane spawning disabled
// ---------------------------------------------------------------------------
describe('runClaude OMG HUD behavior', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    (execFileSync as ReturnType<typeof vi.fn>).mockReturnValue(Buffer.from(''));
  });
...
describe('launchCommand — env var propagation', () => {
  ...
  it('OMG flags are stripped from args passed to Claude', async () => {
...
describe('prepareOmcLaunchConfigDir / launchCommand OMG companion loading', () => {
  ...
  it('uses a runtime launch profile when a preserved CLAUDE-omg.md companion exists', async () => {
    const configDir = join(tempRoot!, '.claude');
    mkdirSync(join(configDir, 'skills'), { recursive: true });
    writeFileSync(join(configDir, 'CLAUDE.md'), '# User base config\n');
    writeFileSync(join(configDir, 'CLAUDE-omg.md'), '<!-- OMG:START -->\n# OMG companion\n<!-- OMG:END -->\n');
    ...
    const runtimeDir = join(configDir, '.omg-launch');
    expect(process.env.CLAUDE_CONFIG_DIR).toBe(runtimeDir);
    expect(existsSync(join(runtimeDir, 'CLAUDE.md'))).toBe(true);
    expect(readFileSync(join(runtimeDir, 'CLAUDE.md'), 'utf-8')).toContain('# OMG companion');
  ...
  it('leaves CLAUDE_CONFIG_DIR unchanged when no preserved companion exists', () => {
    const configDir = join(tempRoot!, '.claude');
    mkdirSync(configDir, { recursive: true });
    writeFileSync(join(configDir, 'CLAUDE.md'), '<!-- OMG:START -->\n# OMG base\n<!-- OMG:END -->\n');

    expect(prepareOmcLaunchConfigDir(configDir)).toBe(configDir);
    expect(existsSync(join(configDir, '.omg-launch'))).toBe(false);
  });
...
describe('TMUX_ENV_FORWARD allowlist', () => {
  ...
  it('includes all OMG launch flags', () => {
