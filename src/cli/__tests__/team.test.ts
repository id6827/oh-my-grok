...
describe('team cli', () => {
  ...
  it('teamCommand start --json outputs valid JSON envelope', async () => {
    ...
    const cwd = mkdtempSync(join(tmpdir(), 'omg-team-cli-start-json-'));
  ...
  it('teamCommand start forwards --new-window to runtime-cli payload', async () => {
    ...
    const unref = vi.fn();
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => undefined);
    const cwd = mkdtempSync(join(tmpdir(), 'omg-team-cli-new-window-'));
  ...
  it('teamCommand start --json with --count expands agent types', async () => {
    ...
    const unref = vi.fn();
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => undefined);
    const cwd = mkdtempSync(join(tmpdir(), 'omg-team-cli-count-'));
  ...
  it('teamCommand start --agent antigravity --count expands antigravity worker types', async () => {
    ...
    const unref = vi.fn();
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => undefined);
    const cwd = mkdtempSync(join(tmpdir(), 'omg-team-cli-agy-'));
  ...
  it('teamCommand start rejects an unsupported --agent value', async () => {
    const cwd = mkdtempSync(join(tmpdir(), 'omg-team-cli-bad-agent-'));
    const { teamCommand } = await import('../team.js');
    await expect(
      teamCommand([
  ...
  it('legacy team alias reuses an approved short follow-up launch hint', async () => {
    ...
    const unref = vi.fn();
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => undefined);
    const cwd = mkdtempSync(join(tmpdir(), 'omg-team-cli-approved-followup-'));
  ...
  it('legacy team alias fails closed for incomplete approved short follow-up hints', async () => {
    const cwd = mkdtempSync(join(tmpdir(), 'omg-team-cli-approved-incomplete-'));
    const plansDir = join(cwd, '.omg', 'plans');
    mkdirSync(plansDir, { recursive: true });
    writeFileSync(
      join(plansDir, 'prd-feature.md'),
  ...
  it('legacy team alias fails closed for ambiguous approved short follow-up hints', async () => {
    const cwd = mkdtempSync(join(tmpdir(), 'omg-team-cli-approved-ambiguous-'));
    const plansDir = join(cwd, '.omg', 'plans');
    mkdirSync(plansDir, { recursive: true });
    writeFileSync(
      join(plansDir, 'prd-feature.md'),
  ...
  it('teamCommand start without --json outputs non-JSON', async () => {
    ...
    const unref = vi.fn();
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => undefined);
    const cwd = mkdtempSync(join(tmpdir(), 'omg-team-cli-start-plain-'));
  ...
  it('cleanupTeamJob keeps state root when worktree cleanup preserves metadata', async () => {
    ...
    const jobId = 'omg-cleanup3';
    const cwd = mkdtempSync(join(tmpdir(), 'omg-team-cli-preserve-cleanup-'));
    const stateRoot = join(cwd, '.omg', 'state', 'team', 'demo-team');
    mkdirSync(stateRoot, { recursive: true });
  ...
  it('cleanupTeamJob blocks state cleanup when panes artifact is missing and config still has workers', async () => {
    ...
    const jobId = 'omg-cleanup5';
    const cwd = mkdtempSync(join(tmpdir(), 'omg-team-cli-unknown-liveness-'));
    const stateRoot = join(cwd, '.omg', 'state', 'team', 'demo-team');
    mkdirSync(stateRoot, { recursive: true });
    writeFileSync(join(stateRoot, 'config.json'), JSON.stringify({
      name: 'demo-team',
  ...
  it('cleanupTeamJob preserves state when pane liveness probe is unknown', async () => {
    ...
    const jobId = 'omg-cleanup6';
    const cwd = mkdtempSync(join(tmpdir(), 'omg-team-cli-unknown-probe-'));
    const stateRoot = join(cwd, '.omg', 'state', 'team', 'demo-team');
    mkdirSync(stateRoot, { recursive: true });
    writeFileSync(join(jobsDir, `${jobId}.json`), JSON.stringify({
      status: 'running',
  ...
  it('cleanupTeamJob preserves worktrees and state when worker panes remain alive', async () => {
    ...
    const jobId = 'omg-cleanup4';
    const cwd = mkdtempSync(join(tmpdir(), 'omg-team-cli-live-cleanup-'));
    const stateRoot = join(cwd, '.omg', 'state', 'team', 'demo-team');
    mkdirSync(stateRoot, { recursive: true });
  ...
  it('legacy shorthand start alias supports optional ralph token', async () => {
    ...
    const unref = vi.fn();
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => undefined);
    const cwd = mkdtempSync(join(tmpdir(), 'omg-team-cli-legacy-'));
