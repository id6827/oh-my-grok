/**
 * Tests for `--plugin-dir` capture in `omg` launch.
 *
 * Plan: binary-weaving-mountain — HUD wrapper resolves the active plugin root
 * from `process.env.OMC_PLUGIN_ROOT`, set by the `omg` CLI when the user
...
describe('parsePluginDirArg', () => {
  ...
  it('preserves Windows drive-letter absolute paths on non-Windows hosts', () => {
    const out = parsePluginDirArg(['--plugin-dir', 'C:\\Users\\me\\omg']);
    expect(out).toBe('C:\\Users\\me\\omg');
  });

  it('preserves Windows UNC absolute paths on non-Windows hosts', () => {
    const out = parsePluginDirArg(['--plugin-dir=\\\\server\\share\\omg']);
    expect(out).toBe('\\\\server\\share\\omg');
  });
...
describe('launchCommand → child env propagation (OMC_PLUGIN_ROOT)', () => {
  ...
  beforeEach(() => {
    tmpConfigDir = mkdtempSync(join(tmpdir(), 'omg-pdc-'));
    savedEnv = {
  ...
  it('6. relative --plugin-dir is resolved against the launch CWD', async () => {
    ...
    const knownCwd = realpathSync(mkdtempSync(join(tmpdir(), 'omg-pdc-cwd-')));
    try {
