...
describe('tmux clipboard configuration', () => {
  ...
  it('sets session-scoped clipboard options and appends universal terminal clipboard when missing', () => {
    ...
    configureTmuxClipboardForSession('omg-session', { stripTmux: true, stdio: 'ignore' });

    expect(mocks.tmuxExec).toHaveBeenCalledWith(
      ['set-option', '-t', 'omg-session', 'set-clipboard', 'on'],
      { stripTmux: true, stdio: 'ignore' },
    );
    expect(mocks.tmuxExec).toHaveBeenCalledWith(
      ['show-options', '-t', 'omg-session', '-v', 'terminal-features'],
      { stripTmux: true, stdio: 'ignore' },
    );
    expect(mocks.tmuxExec).toHaveBeenCalledWith(
      ['set-option', '-at', 'omg-session', 'terminal-features', ',*:clipboard'],
      { stripTmux: true, stdio: 'ignore' },
    );
  });
  ...
  it('does not append terminal-features when universal clipboard is already present', () => {
    ...
    configureTmuxClipboardForSession('omg-session');
  ...
  it('supports async tmux launch paths', async () => {
    ...
    await configureTmuxClipboardForSessionAsync('omg-team');

    expect(mocks.tmuxExecAsync).toHaveBeenCalledWith(['set-option', '-t', 'omg-team', 'set-clipboard', 'on'], undefined);
    expect(mocks.tmuxExecAsync).toHaveBeenCalledWith(['set-option', '-at', 'omg-team', 'terminal-features', ',*:clipboard'], undefined);
  });
});
