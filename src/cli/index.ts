...
program
  .command('interop')
  .description('Launch split-pane tmux session with Claude Code (OMG) and Codex (OMX)')
  .addHelpText('after', `
Requirements:
...
Examples:
  $ omg config-stop-callback file --enable --path ${join(getClaudeConfigDir(), 'logs/{date}.md')}
...
Examples:
  ...
  .action(async (options) => {
    if (!options.quiet) {
      console.log(chalk.blue('Oh-My-Grok Update\n'));
    }
...
Examples:
  ...
  .action(async () => {
    ...
    console.log(chalk.blue.bold('\nOh-My-Grok Version Information\n'));
    console.log(chalk.gray('━'.repeat(50)));
    ...
    console.log(chalk.gray('\n━'.repeat(50)));
    console.log(chalk.gray('\nTo check for updates, run: oh-my-grok update --check'));
  });

/**
 * Install command - Install agents and commands (default: ~/.grok/)
 */
program
  .command('install')
  .description('Install OMG agents and commands to Claude Code config directory (default: ~/.grok/)')
  .option('-f, --force', 'Overwrite existing files')
  .option('-q, --quiet', 'Suppress output except for errors')
  .option('--skip-claude-check', 'Skip checking if Claude Code is installed')
  .addHelpText('after', `
Examples:
  $ omg install                  Install to config directory (default: ~/.grok/)
  $ omg install --force          Reinstall, overwriting existing files
  $ omg install --quiet          Silent install for scripts
  $ GROK_CONFIG_DIR=$HOME/.claude-isolated-workspace omg install  Isolated config directory`)
  .action(async (options) => {
    if (!options.quiet) {
      console.log(chalk.blue('╔═══════════════════════════════════════════════════════════╗'));
      console.log(chalk.blue('║         Oh-My-Grok Installer                        ║'));
...
capabilitiesCmd
  ...
  .option('--json', 'Output as JSON')
  .option('--lockfile <path>', 'Lockfile path (default: omg-capabilities.lock.json)')
  .action(async (options) => {
...
const doctorCmd = program
  ...
  .option('--plugin-dir <path>', 'Override OMG plugin root directory (sets OMC_PLUGIN_ROOT)')
...
program
  ...
  .option('--plugin-dir-mode', 'Treat OMG as launched via --plugin-dir at runtime (skip agent/skill copy; HUD + hooks + CLAUDE.md still installed)')
...
Examples:
  ...
  .action(async (options) => {
    if (!options.quiet) {
      console.log(chalk.blue('Oh-My-Grok Setup\n'));
    }
    ...
    if (!options.quiet) {
      ...
      console.log(chalk.gray('Start Claude Code and use /oh-my-grok:omg-setup for interactive setup.'));
    }
  });
...
program
  ...
  .action(async () => {
    ...
    if (result.success) {
      console.log(chalk.green('✓ Oh-My-Grok installed successfully!'));
      console.log(chalk.gray('  Run "oh-my-grok info" to see available agents.'));
      console.log(chalk.yellow('  Run "/omg-default" (project) or "/omg-default-global" (global) in Claude Code.'));
    } else {
      // Don't fail the npm install, just warn
      console.warn(chalk.yellow('⚠ Could not complete OMG setup:'), result.message);
      console.warn(chalk.gray('  Run "oh-my-grok install" manually to complete setup.'));
    }
  });
...
program
  ...
  .option(
    '--preset <name>',
    'Persist HUD preset (minimal|focused|full|opencode|dense) then render',
  )
  .action(async (options) => {
    if (options.preset) {
      ...
      if (!(name in PRESET_CONFIGS)) {
        console.error(
          `Unknown HUD preset "${options.preset}". Expected: ${Object.keys(PRESET_CONFIGS).join(', ')}`,
        );
        process.exitCode = 1;
        return;
      }
      const cfg = applyPreset(name as keyof typeof PRESET_CONFIGS);
