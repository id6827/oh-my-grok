...
describe('MINGW64 escape safety: no "!" in node -e inline scripts (issue #729)', () => {
  ...
  describe('specific regressions (issue #729)', () => {
    ...
    it('hud SKILL.md keeps Unix statusLine guidance portable while preserving Windows-safe paths', () => {
      const content = readFileSync(join(REPO_ROOT, 'skills', 'hud', 'SKILL.md'), 'utf-8');
      expect(content).toContain('"command": "node ${GROK_CONFIG_DIR:-$HOME/.claude}/hud/omg-hud.mjs"');
      expect(content).toContain('"command": "node C:/Users/username/.claude/hud/omg-hud.mjs"');
    ...
    it('hud SKILL.md cleanup step removes only the legacy HUD wrapper filename', () => {
      ...
      const cleanupLine = content
        .split('\n')
        .find(l => l.includes('Removed legacy omg-hud.js') && l.startsWith('node -e'));

      expect(cleanupLine).toBeDefined();
      expect(cleanupLine).toContain("t=p.join(d,'hud','omg-hud.js')");
      expect(cleanupLine).not.toContain("t=p.join(d,'hud','omg-hud.mjs')");
    });
    ...
    it('omg-setup extracts CLAUDE.md version from OMG marker', () => {
      const setupDir = join(REPO_ROOT, 'skills', 'omg-setup');
      const files = [
        join(setupDir, 'SKILL.md'),
        ...readdirSync(join(setupDir, 'phases')).map(f => join(setupDir, 'phases', f)),
      ...
      expect(combined).not.toContain('grep -m1 "^# oh-my-grok"');
    });
    ...
    it('omg-setup phase 2 delegates HUD setup instead of inlining statusLine formatting', () => {
      ...
      expect(content).toContain('Configure `statusLine` in `~/.grok/settings.json`');
      expect(content).not.toContain('Read `~/.grok/settings.json`, then update/add the `statusLine` field.');
