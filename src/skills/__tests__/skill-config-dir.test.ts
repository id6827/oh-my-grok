/**
 * Regression test: skill markdown files must use GROK_CONFIG_DIR
 *
 * Ensures that bash code blocks in skill files never hardcode $HOME/.claude
 * without a ${GROK_CONFIG_DIR:-...} fallback. This prevents skills from
 ...
 * ${GROK_CONFIG_DIR:-$HOME/.claude} pattern.
 */
function findHardcodedHomeClaude(filePath: string): { line: number; text: string }[] {
  ...
  for (const block of blocks) {
    ...
    for (let i = 0; i < lines.length; i++) {
      const line = lines[i];
      // Match $HOME/.claude that is NOT inside ${GROK_CONFIG_DIR:-$HOME/.claude}
      if (/\$HOME\/\.claude/.test(line) && !/\$\{GROK_CONFIG_DIR:-\$HOME\/\.claude\}/.test(line)) {
        violations.push({
 ...
 * without portable notation like [$GROK_CONFIG_DIR|~/.claude].
...
function findHardcodedTildeClaude(filePath: string): { line: number; text: string }[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Allow: portable notation [$GROK_CONFIG_DIR|~/.claude]
    if (/\[\$GROK_CONFIG_DIR\|~\/\.claude\]/.test(line)) continue;
    // Allow: env-var fallback ${GROK_CONFIG_DIR:-...}
    if (/\$\{GROK_CONFIG_DIR:-/.test(line)) continue;
    ...
    // Allow: lines that mention GROK_CONFIG_DIR (explaining the config dir system)
    if (/GROK_CONFIG_DIR/i.test(line)) continue;
    // Allow: glob patterns like ~/.grok/** (permission patterns, not path resolution)
    if (/~\/\.claude\/\*/.test(line)) continue;
...
describe('skill markdown bash blocks must respect GROK_CONFIG_DIR', () => {
    ...
    (_label, filePath) => {
      ...
      if (violations.length > 0) {
        const details = violations
          ...
          .join('\n');
        expect.fail(
          `Found $HOME/.claude without GROK_CONFIG_DIR fallback:\n${details}\n` +
...
describe('skill markdown prose must not use raw ~/.claude (Contract 6, issue #2155 §16)', () => {
  ...
  // To reduce the baseline: fix the skill prose to use [$GROK_CONFIG_DIR|~/.claude] notation,
  ...
  const KNOWN_VIOLATION_BASELINE: Record<string, number> = {
    ...
    'skills/omg-doctor/SKILL.md': 7,
    'skills/omg-setup/SKILL.md': 5,
    'skills/omg-setup/phases/01-install-claude-md.md': 4,
    'skills/omg-setup/phases/02-configure.md': 3,
    'skills/omg-setup/phases/03-integrations.md': 3,
    ...
    (label, filePath) => {
      ...
      if (violations.length > baseline) {
        ...
        expect.fail(
          ...
          `Replace with: [$GROK_CONFIG_DIR|~/.claude] or use \${GROK_CONFIG_DIR:-$HOME/.claude} in code`
        );
      }
    },
  );
