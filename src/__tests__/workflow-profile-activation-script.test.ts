...
function runHook(script: string, prompt: string, cwd: string, configHome: string, transcriptPath = join(cwd, 'claude-config', 'projects', 'workflow-activation-fixture.jsonl'), extraEnv: Record<string, string> = {}) {
  return JSON.parse(execFileSync(NODE, [script], {
    ...
    encoding: 'utf8',
    env: { ...process.env, NODE_ENV: 'test', OMC_SKIP_HOOKS: '', XDG_CONFIG_HOME: configHome, GROK_CONFIG_DIR: join(cwd, 'claude-config'), ...extraEnv },
...
function runHookAsync(script: string, prompt: string, cwd: string, configHome: string, transcriptPath = join(cwd, 'claude-config', 'projects', 'workflow-activation-fixture.jsonl')) {
  return new Promise<{ hookSpecificOutput?: { additionalContext?: string } }>((resolve, reject) => {
    const child = spawn(NODE, [script], {
      cwd,
      env: { ...process.env, NODE_ENV: 'test', OMC_SKIP_HOOKS: '', XDG_CONFIG_HOME: configHome, GROK_CONFIG_DIR: join(cwd, 'claude-config') },
...
function createFixture(cwd = mkdtempSync(join(tmpdir(), 'omg-workflow-activation-'))) {
  const configHome = join(cwd, 'config');
  mkdirSync(join(cwd, '.claude'), { recursive: true });
  mkdirSync(join(configHome, 'claude-omg'), { recursive: true });
  ...
  writeFileSync(join(configHome, 'claude-omg', 'config.jsonc'), `{
  ...
  writeFileSync(join(cwd, '.claude', 'omg.jsonc'), `{
    "autopilot": { "workflows": {
...
function createNestedGitFixture() {
  const parent = mkdtempSync(join(tmpdir(), 'omg-workflow-profile-root-'));
...
describe('workflow profile activation hook fixtures (#3487)', () => {
  ...
  it.each(HOOKS)('ignores profile config above the git root and falls back to the user profile through %s', (script) => {
    ...
    try {
      ...
      writeFileSync(join(parent, '.claude', 'omg.jsonc'), '{ "autopilot": { "workflows": { "release-flow": { "version": 1, "stages": ["ralplan", "execution", "qa"] } } } }');
  ...
  it.each(HOOKS)('does not load profile config above a non-Git workspace boundary through %s', (script) => {
    ...
    try {
      writeFileSync(join(workspace, '.omg-workspace'), '{}');
      mkdirSync(nested, { recursive: true });
      mkdirSync(join(parent, '.claude'), { recursive: true });
      writeFileSync(join(parent, '.claude', 'omg.jsonc'), '{ "autopilot": { "workflows": { "release-flow": { "version": 1, "stages": ["ralplan", "execution", "qa"] } } } }');
      runHook(script, '/autopilot --workflow release-flow ship the release', nested, configHome, transcriptPath, { GROK_CONFIG_DIR: join(workspace, 'claude-config') });
  ...
  it.each(HOOKS)('preserves named workflow state before routing /cancel through %s', (script) => {
    ...
    try {
      ...
      const dependentPath = join(cwd, '.omg', 'state', 'sessions', 'workflow-activation-fixture', 'ralph-state.json');
  ...
  it.each(HOOKS)('reactivates the exact persisted named run through %s', (script) => {
    ...
    try {
      ...
      writeFileSync(join(cwd, '.claude', 'omg.jsonc'), '{ invalid later config');
  ...
  it.each(HOOKS)('resolves project workflow profiles from a literal non-Git working directory through %s', (script) => {
    ...
    try {
      ...
      writeFileSync(join(nested, '.claude', 'omg.jsonc'), JSON.stringify({ autopilot: { workflows: { 'root-only': { version: 1, stages: ['ralplan', 'execution'] } } } }));
      const output = runHook(script, '/autopilot --workflow root-only ship it', nested, configHome, transcriptPath, { GROK_CONFIG_DIR: join(cwd, 'claude-config') });
      expect(output.hookSpecificOutput?.additionalContext).toContain('## PIPELINE STAGE: RALPLAN (Consensus Planning)');
    } finally {
      rmSync(cwd, { recursive: true, force: true });
    }
  ...
  it('preserves foreign shared-home recovery claims and publication temps while the template activates project A', () => {
    ...
    const globalStatePath = join(projectA.cwd, '.omg', 'state', 'autopilot-state.json');
    try {
  ...
  it.each([
    ...
    const { cwd, configHome } = createFixture();
    const statePath = join(cwd, '.omg', 'state', 'sessions', 'workflow-activation-fixture');
  ...
  it.each([
    ...
    const { cwd, configHome } = createFixture();
    try {
      writeFileSync(join(cwd, '.claude', 'omg.jsonc'), JSON.stringify({ autopilot: { workflows: { 'release-flow': { version: 1, stages } } } }));
      for (const script of HOOKS) {
        const output = runHook(script, '/autopilot --workflow release-flow ship it', cwd, configHome);
        expect(output.hookSpecificOutput?.additionalContext).toContain('[AUTOPILOT WORKFLOW ERROR]');
