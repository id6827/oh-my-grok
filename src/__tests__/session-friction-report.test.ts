...
describe('session friction report', () => {
  ...
  beforeEach(() => {
    tempRoot = mkdtempSync(join(tmpdir(), 'omg-session-friction-'));
    claudeDir = join(tempRoot, 'claude');
    process.env.GROK_CONFIG_DIR = claudeDir;
  ...
  it('includes OMG replay artifacts for all-project reports', async () => {
    ...
    expect(session?.sources).toContain('omg-session-replay');
    expect(session?.replayAgentsFailed).toBe(1);
  });

  it('keeps project-local OMG replay artifacts when entries have no cwd', async () => {
    const report = await generateSessionFrictionReport({ workingDirectory: repoRoot, project: 'oh-my-grok' });
