...
describe('plugin-setup.mjs hook command portability', () => {
  ...
  const scriptContent = existsSync(PLUGIN_SETUP_PATH)
    ...
    : '';
  const UNIX_PREFIX =
    'sh "$GROK_PLUGIN_ROOT"/scripts/find-node.sh "$GROK_PLUGIN_ROOT"/scripts/run.cjs ';
  const WINDOWS_PREFIX = 'node "$GROK_PLUGIN_ROOT"/scripts/run.cjs ';
  ...
  function patchCommand(cmd: string, prefix = UNIX_PREFIX): string {
    const findNodePattern =
      /^sh "\$\{GROK_PLUGIN_ROOT\}\/scripts\/find-node\.sh" "\$\{GROK_PLUGIN_ROOT\}\/scripts\/([^"\s]+)"?(.*)$/;
    const currentFindNodePattern =
      /^(?:"\/bin\/sh"|sh) "\$GROK_PLUGIN_ROOT"\/scripts\/find-node\.sh "\$GROK_PLUGIN_ROOT"\/scripts\/run\.cjs "\$GROK_PLUGIN_ROOT"\/scripts\/([^"\s]+)"?(.*)$/;
    const directRunCjsPattern =
      /^node\s+"\$GROK_PLUGIN_ROOT"\/scripts\/run\.cjs\s+"\$GROK_PLUGIN_ROOT"\/scripts\/([^"\s]+)"?(.*)$/;
    const absNodePattern =
      /^"([^"]*\/node|[A-Za-z]:\\[^"]*\\node(?:\.exe)?)"\s+"\$GROK_PLUGIN_ROOT"\/scripts\/run\.cjs\s+"\$GROK_PLUGIN_ROOT"\/scripts\/([^"\s]+)"?(.*)$/;

    const m = cmd.match(currentFindNodePattern) ?? cmd.match(findNodePattern) ?? cmd.match(directRunCjsPattern);
    if (m) {
      return `${prefix}"$GROK_PLUGIN_ROOT"/scripts/${m[1]}${m[2]}`;
    }
    const absNodeMatch = cmd.match(absNodePattern);
    if (absNodeMatch) {
      return `${prefix}"$GROK_PLUGIN_ROOT"/scripts/${absNodeMatch[2]}${absNodeMatch[3]}`;
    }
    return cmd;
  }
  ...
  it('leaves the canonical sh+find-node+run.cjs command unchanged', () => {
    const canonical =
      `${UNIX_PREFIX}"$GROK_PLUGIN_ROOT"/scripts/keyword-detector.mjs`;
    expect(patchCommand(canonical)).toBe(canonical);
  });

  it('normalizes legacy sh "${GROK_PLUGIN_ROOT}/..." form to the canonical prefix', () => {
    const legacy =
      'sh "${GROK_PLUGIN_ROOT}/scripts/find-node.sh" "${GROK_PLUGIN_ROOT}/scripts/keyword-detector.mjs"';
  ...
  it('normalizes bare "node run.cjs" form (node on PATH) to the find-node bootstrap', () => {
    const bare =
      'node "$GROK_PLUGIN_ROOT"/scripts/run.cjs "$GROK_PLUGIN_ROOT"/scripts/session-start.mjs';
    const result = patchCommand(bare);
    expect(result).toBe(
      `${UNIX_PREFIX}"$GROK_PLUGIN_ROOT"/scripts/session-start.mjs`,
    );
  });
  ...
  it('self-heals an absolute node path baked in at publish time', () => {
    const absolute =
      '"/opt/hostedtoolcache/node/20.0.0/x64/bin/node" "$GROK_PLUGIN_ROOT"/scripts/run.cjs "$GROK_PLUGIN_ROOT"/scripts/keyword-detector.mjs';
  ...
  it('keeps generated SessionEnd hooks native-Windows safe without sh', () => {
    const sessionEnd =
      'node "$GROK_PLUGIN_ROOT"/scripts/run.cjs "$GROK_PLUGIN_ROOT"/scripts/session-end.mjs';
    const wikiSessionEnd =
      'node "$GROK_PLUGIN_ROOT"/scripts/run.cjs "$GROK_PLUGIN_ROOT"/scripts/wiki-session-end.mjs';
  ...
  it('does not rewrite the source hooks manifest when run from a repository checkout', () => {
    ...
    const tempRoot = mkdtempSync(join(tmpdir(), 'omg-plugin-setup-source-hooks-'));
  ...
  it('normalizes current sh find-node commands to node run.cjs on Windows', () => {
    const current =
      'sh "$GROK_PLUGIN_ROOT"/scripts/find-node.sh "$GROK_PLUGIN_ROOT"/scripts/run.cjs "$GROK_PLUGIN_ROOT"/scripts/session-end.mjs';
  ...
  it.runIf(process.platform !== 'win32')('executes a Unix hook command with minimal PATH by resolving Volta-managed node', () => {
    const tempRoot = mkdtempSync(join(tmpdir(), 'omg-min-path-hook-'));
    try {
      ...
      const command = `${UNIX_PREFIX}"$GROK_PLUGIN_ROOT"/scripts/keyword-detector.mjs --smoke`;

      execFileSync('/bin/sh', ['-c', command], {
        env: {
          HOME: tempHome,
          PATH: tempBin,
          GROK_PLUGIN_ROOT: PACKAGE_ROOT,
  ...
  it.runIf(process.platform !== 'win32')('prefers concrete nvm node over a stale executable shim on PATH', () => {
    const tempRoot = mkdtempSync(join(tmpdir(), 'omg-min-path-nvm-'));
    try {
      const tempHome = join(tempRoot, 'home');
      const tempBin = join(tempRoot, 'bin');
      ...
      const command = `${UNIX_PREFIX}"$GROK_PLUGIN_ROOT"/scripts/session-end.mjs`;
      execFileSync('/bin/sh', ['-c', command], {
        env: {
          HOME: tempHome,
  ...
  it.runIf(process.platform !== 'win32')('prefers concrete nvm node over a stale stored nodeBinary shim', () => {
    const tempRoot = mkdtempSync(join(tmpdir(), 'omg-stored-shim-'));
    try {
      const tempHome = join(tempRoot, 'home');
      const tempBin = join(tempRoot, 'bin');
      const asdfBin = join(tempHome, '.asdf', 'shims');
      ...
      writeFileSync(join(claudeDir, '.omg-config.json'), JSON.stringify({ nodeBinary: staleShim }));
  ...
  it('keeps the Windows hook command direct-node and shell-wrapper free', () => {
    const command = patchCommand(
      `${UNIX_PREFIX}"$GROK_PLUGIN_ROOT"/scripts/session-end.mjs`,
      WINDOWS_PREFIX,
    );

    expect(command).toBe('node "$GROK_PLUGIN_ROOT"/scripts/run.cjs "$GROK_PLUGIN_ROOT"/scripts/session-end.mjs');
