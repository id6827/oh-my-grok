...
/**
 * Regression tests for run.cjs graceful fallback when GROK_PLUGIN_ROOT
 * points to a stale/deleted/broken plugin cache directory.
 *
 * See: https://github.com/Yeachan-Heo/oh-my-grok/issues/1007
 */
describe('run.cjs — graceful fallback for stale plugin paths', () => {
  ...
  beforeEach(() => {
    ...
    fakeCacheBase = join(tmpDir, 'plugins', 'cache', 'omg', 'oh-my-grok');
  ...
  it('exits 0 when target script does not exist (stale GROK_PLUGIN_ROOT)', () => {
  ...
  it('resolves target through symlinked version directory', () => {
    ...
    const result = runCjs(target, {
      GROK_PLUGIN_ROOT: symlinkVersion,
    });
  ...
  it('exits 0 when no GROK_PLUGIN_ROOT is set and target is missing', () => {
          ...
          Stop: [
            {
              ...
              hooks: [
                {
                  type: 'command',
                  command: 'node "$GROK_PLUGIN_ROOT"/scripts/run.cjs "$GROK_PLUGIN_ROOT"/scripts/slow-stop-hook.cjs',
                  timeout: 2,
                },
              ],
            },
          ...
          UserPromptSubmit: [
            {
              ...
              hooks: [
                {
                  type: 'command',
                  command: 'node "$GROK_PLUGIN_ROOT"/scripts/run.cjs "$GROK_PLUGIN_ROOT"/scripts/prompt-ten.cjs',
                  timeout: 10,
                },
                {
                  type: 'command',
                  command: 'node "$GROK_PLUGIN_ROOT"/scripts/run.cjs "$GROK_PLUGIN_ROOT"/scripts/prompt-fifteen.cjs',
                  timeout: 15,
                },
              ],
            },
          ...
          Stop: [
            {
              ...
              hooks: [
                {
                  type: 'command',
                  command: 'node "$GROK_PLUGIN_ROOT"/scripts/run.cjs "$GROK_PLUGIN_ROOT"/scripts/non-prompt-slow.cjs',
                  timeout: 2,
                },
              ],
            },
          ...
          UserPromptSubmit: [
            {
              ...
              hooks: [
                {
                  type: 'command',
                  command: 'node "$GROK_PLUGIN_ROOT"/scripts/run.cjs "$GROK_PLUGIN_ROOT"/scripts/prompt-debug-slow.cjs',
                  timeout: 1,
                },
              ],
            },
...
describe('run.cjs trusted UserPromptSubmit Worker selection', () => {
  ...
  beforeEach(() => {
    tmpDir = mkdtempSync(join(tmpdir(), 'omg-trusted-run-cjs-'));
  });
    ...
    writeFileSync(join(root, 'hooks', 'hooks.json'), JSON.stringify({
      hooks: {
        [event]: [{ matcher: '', hooks: Object.keys(scripts).map(name => ({
          type: 'command',
          command: `node "$GROK_PLUGIN_ROOT"/scripts/run.cjs "$GROK_PLUGIN_ROOT"/scripts/${name}`,
          timeout,
        })) }],
      },
    }));
  ...
  it('uses a Worker only for an exact canonical prompt script below the configured trusted root', () => {
    ...
    const result = run(target, { GROK_PLUGIN_ROOT: root });
  ...
  it('rejects same-basename outside-root, extra-argument, and nonprompt candidates to the generic child path', () => {
    ...
    expect(run(outside, { GROK_PLUGIN_ROOT: root })).toMatchObject({ status: 0, stdout: 'child' });
    expect(run(join(root, 'scripts', 'keyword-detector.mjs'), { GROK_PLUGIN_ROOT: root }, ['extra']))
    ...
    expect(run(nonPromptTarget, { GROK_PLUGIN_ROOT: nonPromptRoot })).toMatchObject({ status: 0, stdout: 'child' });
  });
  ...
  it('rejects a lexical trusted-root path that escapes through a symlink', () => {
    ...
    expect(run(join(root, 'scripts', 'keyword-detector.mjs'), { GROK_PLUGIN_ROOT: root }))
      .toMatchObject({ status: 0, stdout: 'child' });
  });
  ...
  it('trusts only the explicitly selected canonical stale-cache sibling root', () => {
    ...
    const result = run(join(staleRoot, 'scripts', 'keyword-detector.mjs'), { GROK_PLUGIN_ROOT: staleRoot });
  ...
  it('terminates synchronous and async handle hangs fail-open without late output', () => {
    ...
    const quiet = run(target, { GROK_PLUGIN_ROOT: root, HANG_KIND: 'sync' });
    expect(quiet).toMatchObject({ status: 0, stdout: '', stderr: '' });
    expect(readFileSync(startedMarker, 'utf-8')).toBe('sync');

    const debug = run(target, { GROK_PLUGIN_ROOT: root, HANG_KIND: 'async', OMC_DEBUG_HOOKS: '1' });
