...
describe('release generation', () => {
  it('extracts a deduped PR set from squash and merge subjects', () => {
    const prNumbers = extractPullRequestNumbers([
      ...
      'Merge pull request #2146 from Yeachan-Heo/issue-2143-omg-launch-followup',
  ...
  it('keeps non-conventional PRs in other changes and renders exact PR counts', () => {
    const pullRequests = [
      ...
      { number: '2146', title: 'fix: follow up #2143 with explicit overwrite choice + omg launch profile', author: 'Yeachan-Heo', headRefName: 'issue-2143-omg-launch-followup' },
  ...
  it('assembles a single custom release body with compare link and contributors', () => {
    ...
    expect(body).toContain('CLI-dependent skill paths such as `ask`, `ccg`, and CLI-backed `team` require the `omg` CLI');
    expect(body).toContain('npm install -g oh-my-grok@4.10.2');
    expect(body).toContain('/plugin marketplace update omg');
    expect(body).toContain('https://github.com/Yeachan-Heo/oh-my-grok/compare/v4.10.1...v4.10.2');
  ...
  it('enforces the release publication boundary around one exact archive', () => {
    ...
    expect(workflow).toContain(
      'node scripts/release-boundary.mjs assert-npm-absent --package oh-my-grok --version "$VERSION"',
    );
    expect(workflow).toContain(
    ...
    expect(workflow).toContain('"$SMOKE_PREFIX/node_modules/.bin/omg" --help');
    expect(workflow).toContain(
      '"$SMOKE_PREFIX/node_modules/.bin/omg-cli" team api --help',
    );
    ...
    expect(workflow).toContain('test -f "$SMOKE_PACKAGE_ROOT/skills/omg-setup/SKILL.md"');
    expect(workflow).toContain('test -f "$SMOKE_PACKAGE_ROOT/skills/omg-setup/phases/01-install-claude-md.md"');
    expect(workflow).toContain('cd "$SMOKE_PROJECT"');
    expect(workflow).not.toContain('working-directory: "$SMOKE_PROJECT"');
    expect(workflow).toContain('bash -c \'bash "${OMC_SETUP_PLUGIN_ROOT:-${GROK_PLUGIN_ROOT}}/scripts/setup-claude-md.sh" local\'');
    expect(workflow).toContain('cmp "$SMOKE_PACKAGE_ROOT/docs/CLAUDE.md" "$SMOKE_PROJECT/.claude/CLAUDE.md"');
    expect(workflow).toContain('cmp "$SMOKE_PACKAGE_ROOT/skills/omg-reference/SKILL.md" "$SMOKE_PROJECT/.claude/skills/omg-reference/SKILL.md"');
    ...
    expect(workflow).toContain(
      'npm install --ignore-scripts --no-audit --no-fund --prefix "$VERIFICATION_PREFIX" "oh-my-grok@$VERSION"',
    );
    expect(workflow).toContain(
      'npm audit signatures --json --include-attestations --prefix "$VERIFICATION_PREFIX" > "$AUDIT_JSON"',
    );
    expect(workflow).toContain(
      'node scripts/release-boundary.mjs verify-registry --package oh-my-grok --version "$VERSION" --tag "$GITHUB_REF_NAME" --sha "$GITHUB_SHA" --evidence "$EVIDENCE_JSON" --tarball "$FINAL_TARBALL" --provenance required --audit "$AUDIT_JSON"',
    );
    expect(workflow).toContain(
      'node scripts/release-boundary.mjs verify-registry --package oh-my-grok --version "$VERSION" --tag "$GITHUB_REF_NAME" --sha "$GITHUB_SHA" --evidence "$EVIDENCE_JSON" --tarball "$FINAL_TARBALL" --provenance sigstore-fallback --publish-log npm-publish.log',
    );
    ...
    const fallbackVerification = workflow.indexOf(
      'node scripts/release-boundary.mjs verify-registry --package oh-my-grok --version "$VERSION" --tag "$GITHUB_REF_NAME" --sha "$GITHUB_SHA" --evidence "$EVIDENCE_JSON" --tarball "$FINAL_TARBALL" --provenance sigstore-fallback',
    );
    ...
    const recoveryPack = recoveryJob.indexOf(
      'npm pack --ignore-scripts --pack-destination "$RECOVERY_ARCHIVE_DIR" --silent "oh-my-grok@$VERSION"',
    );
    ...
    const recoveryInstall = recoveryJob.indexOf(
      'npm install --ignore-scripts --no-audit --no-fund --prefix "$RECOVERY_PREFIX" "oh-my-grok@$VERSION"',
    );
    ...
    const recoveryRequiredVerification = recoveryJob.indexOf(
      'node scripts/release-boundary.mjs verify-registry --package oh-my-grok --version "$VERSION" --tag "$RECOVERY_TAG" --sha "$RECOVERY_SHA" --evidence "$RECOVERY_EVIDENCE_JSON" --tarball "$RECOVERY_TARBALL" --provenance required --audit "$RECOVERY_AUDIT_JSON"',
    );
