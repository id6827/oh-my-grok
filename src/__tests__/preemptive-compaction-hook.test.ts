...
function makeTempDir(): string {
  const dir = mkdtempSync(join(tmpdir(), 'omg-preemptive-hook-'));
  tempDirs.push(dir);
  return dir;
}
  ...
  it('warns when transcript usage crosses the configured threshold', () => {
    ...
    expect(result).toEqual({
      ...
      hookSpecificOutput: {
        ...
        additionalContext:
          '[OMG WARNING] Context at 72% (warning threshold: 70%). Plan a /compact soon to preserve room for the next large tool output.',
      },
    });
  });
  ...
  it('escalates to a critical warning even when a warning cooldown is active', () => {
    ...
    expect(escalated).toEqual({
      ...
      hookSpecificOutput: {
        hookEventName: 'PostToolUse',
        additionalContext:
          '[OMG CRITICAL] Context at 91% (critical threshold: 90%). Run /compact now before continuing with more tools or agent fan-out.',
      },
    });
  });
