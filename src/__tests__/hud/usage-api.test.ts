...
describe('getUsage routing', () => {
  ...
  it('uses the raw ~-prefixed GROK_CONFIG_DIR value for Keychain service lookup', async () => {
    process.env.GROK_CONFIG_DIR = '~/.claude-personal';

    const oneHourFromNow = Date.now() + 60 * 60 * 1000;
    const execFileMock = vi.mocked(childProcess.execFileSync);
    const username = os.userInfo().username;
    const expectedService = expectedServiceName(process.env.GROK_CONFIG_DIR);
  ...
  it('uses a different Keychain service when GROK_CONFIG_DIR is already expanded', async () => {
    process.env.GROK_CONFIG_DIR = '/Users/test/.claude-personal';
