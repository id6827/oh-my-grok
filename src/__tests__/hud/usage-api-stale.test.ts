...
const GROK_CONFIG_DIR = '/tmp/test-claude';
const CACHE_PATH = `${GROK_CONFIG_DIR}/plugins/oh-my-grok/.usage-cache-zai.json`;
const CACHE_DIR = `${GROK_CONFIG_DIR}/plugins/oh-my-grok`;

function createFsMock(initialFiles: Record<string, string>) {
  const files = new Map(Object.entries(initialFiles));
  const directories = new Set<string>([GROK_CONFIG_DIR, CACHE_DIR]);
...
function setupMocks(fsModule: ReturnType<typeof createFsMock>['fsModule'], httpStatus: number, httpBody: string) {
  vi.doMock('../../utils/config-dir.js', () => ({
    getClaudeConfigDir: () => GROK_CONFIG_DIR,
  }));
