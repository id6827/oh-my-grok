...
describe('readStdinCache — env-less reader fallback to most recent session cache', () => {
  ...
  it('resolves the fallback directory through the same OMG_STATE_DIR helper as writers', () => {
    // Regression: the env-less fallback previously assembled the sessions
    // directory from `join(root, '.omg', 'state', 'sessions')` directly,
    // which bypasses `OMG_STATE_DIR`-backed centralized state and made
    ...
    const prevStateDir = process.env.OMG_STATE_DIR;
    process.env.OMG_STATE_DIR = centralRoot;
    try {
      // Writer pinned to a session id: must land under OMG_STATE_DIR/...,
      ...
      if (prevStateDir === undefined) {
        ...
        process.env.OMG_STATE_DIR = prevStateDir;
      }
