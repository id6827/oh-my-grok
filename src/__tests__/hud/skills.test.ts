...
describe('renderSkills', () => {
  ...
  describe('namespaced skill names', () => {
    it('displays only last segment for namespaced skills (oh-my-grok:plan)', () => {
      const lastSkill: SkillInvocation = { name: 'oh-my-grok:plan', timestamp: new Date() };
      const result = renderSkills(inactiveUltrawork, inactiveRalph, lastSkill);
      expect(result).toContain('skill:plan');
      expect(result).not.toContain('oh-my-grok');
    });

    it('displays only last segment for namespaced skills with args', () => {
      const lastSkill: SkillInvocation = { name: 'oh-my-grok:autopilot', args: 'build app', timestamp: new Date() };
      const result = renderSkills(inactiveUltrawork, inactiveRalph, lastSkill);
