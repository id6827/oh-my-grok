...
describe('purgeStalePluginCacheVersions', () => {
  ...
  it('handles multiple marketplaces and plugins', () => {
    ...
    const active2 = join(cacheDir, 'omg/oh-my-grok/4.3.0');
    ...
    mockedReadFileSync.mockReturnValue(JSON.stringify({
      ...
      plugins: {
        ...
        'oh-my-grok@omg': [{ installPath: active2 }],
      },
    }));

    mockedReaddirSync.mockImplementation((p, _opts?) => {
      const ps = String(p);
      if (ps === cacheDir) return [dirent('official'), dirent('omg')] as any;
      ...
      if (ps.endsWith('omg')) return [dirent('oh-my-grok')] as any;
      if (ps.endsWith('oh-my-grok')) return [dirent('4.3.0')] as any;
      return [] as any;
    });
  ...
  it('does nothing when all cache versions are active', () => {
    const cacheDir = '/mock/.claude/plugins/cache';
    const active = join(cacheDir, 'omg/oh-my-grok/4.3.0');
    ...
    mockedReadFileSync.mockReturnValue(JSON.stringify({
      version: 2,
      plugins: {
        'oh-my-grok@omg': [{ installPath: active }],
      },
    }));
  ...
  it('replaces stale version dir with symlink to active version in same namespace', () => {
    // Scenario: GROK_PLUGIN_ROOT=4.14.4 in a running session; 4.14.5 installed;
    // purge runs after grace period.  4.14.4 must become a symlink, not disappear.
    const cacheDir = '/mock/.claude/plugins/cache';
    const activeVersion = join(cacheDir, 'omg/oh-my-grok/4.14.5');
    const staleVersion = join(cacheDir, 'omg/oh-my-grok/4.14.4');
    ...
    mockedReadFileSync.mockReturnValue(JSON.stringify({
      version: 2,
      plugins: {
        'oh-my-grok@omg': [{ installPath: activeVersion, version: '4.14.5' }],
      },
    }));
    ...
    mockedReaddirSync.mockImplementation((p, _opts?) => {
      ...
      if (ps === cacheDir) return [dirent('omg')] as any;
      if (ps.endsWith('omg')) return [dirent('oh-my-grok')] as any;
      if (ps.endsWith('oh-my-grok')) return [dirent('4.14.4'), dirent('4.14.5')] as any;
      return [] as any;
    });
  ...
  it('deletes stale version dir when no active version exists in namespace', () => {
    ...
    const cacheDir = '/mock/.claude/plugins/cache';
    const staleVersion = join(cacheDir, 'omg/plugin/1.0.0');

    mockedExistsSync.mockReturnValue(true);
    mockedReadFileSync.mockReturnValue(JSON.stringify({
      version: 2,
      plugins: {
        // installPath is outside the omg/plugin namespace
  ...
  it('skips version directory entries where isDirectory() returns false (existing symlinks)', () => {
    ...
    mockedReadFileSync.mockReturnValue(JSON.stringify({
      version: 2,
      plugins: {
        'oh-my-grok@omg': [{ installPath: activeVersion }],
      },
    }));
