...
describe('omg config-stop-callback tag options', () => {
  it('updates telegram tagList options and preserves existing config fields', () => {
    ...
    const configPath = join(homeDir, '.claude', '.omg-config.json');
  ...
  it('uses GROK_CONFIG_DIR for the default file callback path', () => {
    const homeDir = mkdtempSync(join(tmpdir(), 'omg-cli-stop-callback-home-'));
    const claudeConfigDir = join(homeDir, '.claude-isolated-workspace');
    const configPath = join(claudeConfigDir, '.omg-config.json');
    ...
    const result = spawnSync(process.execPath, ['--import', 'tsx', CLI_ENTRY, 'config-stop-callback', 'file', '--enable'], {
      ...
      env: {
        ...process.env,
        HOME: homeDir,
        GROK_CONFIG_DIR: claudeConfigDir,
      },
      encoding: 'utf-8',
    });
