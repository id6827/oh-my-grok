 ...
 * returns the submodule's own root (a submodule is a complete git repo), so OMG
...
describe('submodule state anchoring (issue #3349)', () => {
  ...
  beforeAll(() => {
    tempDir = realpathSync(mkdtempSync(join(tmpdir(), 'omg-submodule-')));
  ...
  it('getOmcRoot from inside a submodule anchors .omg/ to the superproject root', () => {
    ...
    try {
      ...
      if (prev !== undefined) process.env.OMG_STATE_DIR = prev;
      clearWorktreeCache();
    }
  });
  ...
  // resolving state. Under OMG_STATE_DIR the resolved root becomes the
  ...
  it('resolveToWorktreeRoot preserves submodule identity under OMG_STATE_DIR (no climb)', () => {
    if (!gitAvailable) return;
    clearWorktreeCache();
    const prev = process.env.OMG_STATE_DIR;
    process.env.OMG_STATE_DIR = join(tempDir, 'central');
    try {
      // OMG_STATE_DIR set: identity-bearing root stays at the submodule.
      expect(resolveToWorktreeRoot(submodulePath)).toBe(submodulePath);
    } finally {
      if (prev === undefined) delete process.env.OMG_STATE_DIR;
      else process.env.OMG_STATE_DIR = prev;
      clearWorktreeCache();
    }
  });
  ...
  it('centralized .omg for a submodule session does not merge into the parent superproject', () => {
    if (!gitAvailable) return;
    clearWorktreeCache();
    const prev = process.env.OMG_STATE_DIR;
    const central = join(tempDir, 'central');
    process.env.OMG_STATE_DIR = central;
    try {
