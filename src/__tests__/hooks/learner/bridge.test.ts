  ...
  describe("findSkillFiles", () => {
    ...
    it("should treat symlinked project roots as within boundary", () => {
      ...
      const linkedProjectRoot = join(
        tmpdir(),
        `omg-bridge-link-${Date.now()}-${Math.random().toString(16).slice(2)}`,
      );
...
Mixed trigger instructions`,
  ...
  describe("Session Cache", () => {
    ...
    it("does not write project-local .omg when OMG_STATE_DIR is set", () => {
      const centralizedDir = join(tmpdir(), `omg-state-dir-${Date.now()}`);
      mkdirSync(centralizedDir, { recursive: true });
      const previousOmcStateDir = process.env.OMG_STATE_DIR;
      process.env.OMG_STATE_DIR = centralizedDir;
      try {
        markSkillsInjected(
          "omg-state-dir-test",
        ...
        if (previousOmcStateDir === undefined) {
          ...
          process.env.OMG_STATE_DIR = previousOmcStateDir;
        }
