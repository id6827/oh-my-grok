...
function createCommittedPluginFixture() {
  const root = mkdtempSync(join(tmpdir(), 'omg-committed-plugin-'));
  tempRoots.push(root);
...
function createPluginFixture(claudeMdContent: string) {
  const root = mkdtempSync(join(tmpdir(), 'omg-setup-claude-md-'));
  tempRoots.push(root);
...
describe('setup-claude-md.sh committed plugin shipping surface (issue #3476)', () => {
  ...
  it('runs local setup from a source-only tracked plugin checkout without a consumer build', () => {
    ...
    expect(readFileSync(join(fixture.projectRoot, '.claude', 'skills', 'omg-reference', 'SKILL.md'), 'utf-8'))
      .toBe(readFileSync(join(fixture.pluginRoot, 'skills', 'omg-reference', 'SKILL.md'), 'utf-8'));
  });
  ...
  for (const scenario of ['missing', 'stale'] as const) {
    it(`fails before local mutation when the committed coordinator is ${scenario}`, () => {
      ...
      if (scenario === 'missing') {
        ...
        writeFileSync(join(fixture.pluginRoot, 'docs', 'CLAUDE.md'), '<!-- OMG:START -->\n<!-- OMG:VERSION:stale -->\n# stale\n<!-- OMG:END -->\n');
      }
  ...
  it('uses one active cache root for the coordinator, canonical source, and engine version', () => {
    const root = mkdtempSync(join(tmpdir(), 'omg-committed-root-coherence-'));
    tempRoots.push(root);

    const cacheBase = join(root, '.claude', 'plugins', 'cache', 'omg', 'oh-my-grok');
    ...
    writeFileSync(join(staleRoot, 'docs', 'CLAUDE.md'), '<!-- OMG:START -->\n<!-- OMG:VERSION:stale -->\n# stale root\n<!-- OMG:END -->\n');
    mkdirSync(projectRoot, { recursive: true });
    mkdirSync(homeRoot, { recursive: true });
    ...
    expect(installed).toContain(`<!-- OMG:VERSION:${activeHandshake.engineVersion} -->`);
  });
});


describe('setup-claude-md.sh (issue #3442)', () => {
  it('installs the canonical docs/CLAUDE.md content with OMG markers', () => {
...
`);
    ...
    const installedSkillPath = join(fixture.projectRoot, '.claude', 'skills', 'omg-reference', 'SKILL.md');
...
`);
  ...
  it('refuses to install a canonical source that lacks OMG markers', () => {
    const fixture = createPluginFixture(`# oh-my-grok (OMG) v9.9.9 Summary
...
`);
    ...
    const excludePath = join(fixture.projectRoot, '.git', 'info', 'exclude');
    writeFileSync(excludePath, `# BEGIN OMG local artifacts
!.omg/
.omg/*
!.omg/skills/
!.omg/skills/**
...
`);
    ...
    expect(`${result.stdout}
${result.stderr}`).toContain('Updated OMG git exclude for local OMX artifacts');
  });
...
`);
  ...
  it('uses GROK_CONFIG_DIR for global setup targets and plugin verification', () => {
    const fixture = createPluginFixture(`<!-- OMG:START -->
<!-- OMG:VERSION:9.9.9 -->
...
`);
    ...
    writeFileSync(join(configDir, 'settings.json'), JSON.stringify({ plugins: ['oh-my-grok'] }));
    ...
    expect(existsSync(join(configDir, 'skills', 'omg-reference', 'SKILL.md'))).toBe(true);
...
`);
    ...
    expect(result.status).toBe(0);
    expect(`${result.stdout}\n${result.stderr}`).not.toContain('legacy OMG hook entries');
  });

  it('warns when settings hooks reference a legacy OMG hook command', () => {
    const fixture = createPluginFixture(`<!-- OMG:START -->
<!-- OMG:VERSION:9.9.9 -->
...
`);
    ...
    expect(result.status).toBe(0);
    expect(`${result.stdout}\n${result.stderr}`).toContain('legacy OMG hook entries');
  });
...
`);
    ...
    expect(result.status).toBe(0);
    expect(output).toContain('legacy OMG hook entries');
...
`);
    ...
    expect(baseClaude).toContain('<!-- OMG:START -->');
    expect(baseClaude).toContain('<!-- OMG:END -->');
    expect(baseClaude).toContain('<!-- User customizations -->');
    expect(baseClaude).toContain('# User CLAUDE');
    expect(existsSync(join(configDir, 'CLAUDE-omg.md'))).toBe(false);
  });
...
`);
    ...
    const baseClaude = readFileSync(join(configDir, 'CLAUDE.md'), 'utf-8');
    const companionClaude = readFileSync(join(configDir, 'CLAUDE-omg.md'), 'utf-8');

    expect(baseClaude).toContain('# User CLAUDE');
    expect(baseClaude).toContain('Keep my base config.');
    expect(baseClaude).toContain('<!-- OMG:IMPORT:START -->');
    expect(baseClaude).toContain('@CLAUDE-omg.md');
    expect(baseClaude).toContain('<!-- OMG:IMPORT:END -->');
    expect(baseClaude).not.toContain('<!-- OMG:START -->');

    expect(companionClaude).toContain('<!-- OMG:START -->');
    expect(companionClaude).toContain('<!-- OMG:END -->');
    expect(companionClaude).toContain('<!-- OMG:VERSION:9.9.9 -->');
...
`);
    ...
    const baseClaude = readFileSync(join(configDir, 'CLAUDE.md'), 'utf-8');
    expect(baseClaude.match(/<!-- OMG:IMPORT:START -->/g)).toHaveLength(1);
    expect(baseClaude.match(/@CLAUDE-omg\.md/g)).toHaveLength(1);
    expect(readFileSync(join(configDir, 'CLAUDE-omg.md'), 'utf-8')).toContain('<!-- OMG:VERSION:9.9.9 -->');
  });
...
`);
    ...
    expect(first.status).toBe(0);
    expect(existsSync(join(configDir, 'CLAUDE-omg.md'))).toBe(true);
    expect(readFileSync(join(configDir, 'CLAUDE.md'), 'utf-8')).toContain('<!-- OMG:IMPORT:START -->');
    ...
    // CLAUDE.md must have OMG markers inline, not an import block
    const baseClaude = readFileSync(join(configDir, 'CLAUDE.md'), 'utf-8');
    expect(baseClaude).toContain('<!-- OMG:START -->');
    expect(baseClaude).toContain('<!-- OMG:END -->');
    expect(baseClaude).not.toContain('<!-- OMG:IMPORT:START -->');
    expect(baseClaude).not.toContain('@CLAUDE-omg.md');
...
`);
    ...
    symlinkSync(realTarget, join(configDir, 'CLAUDE-omg.md'));
...
describe('setup-claude-md.sh stale GROK_PLUGIN_ROOT resolution', () => {
  it('does not prefer a newer cache directory when it is missing required plugin assets', () => {
    const root = mkdtempSync(join(tmpdir(), 'omg-stale-invalid-newer-cache-'));
    tempRoots.push(root);
    ...
    writeFileSync(join(homeRoot, '.claude', 'settings.json'), JSON.stringify({ plugins: ['oh-my-grok'] }));
    ...
    const result = spawnSync('bash', [join(oldVersion, 'scripts', 'setup-claude-md.sh'), 'local'], {
      ...
      env: {
        ...process.env,
        HOME: homeRoot,
        GROK_CONFIG_DIR: join(homeRoot, '.claude'),
      },
      encoding: 'utf-8',
    });
  ...
  it('ignores non-semver cache directories when selecting latest fallback version', () => {
    ...
    mkdirSync(join(oldVersion, 'scripts', 'lib'), { recursive: true });
    copyFileSync(CONFIG_DIR_HELPER, join(oldVersion, 'scripts', 'lib', 'config-dir.sh'));
    writeFileSync(join(oldVersion, 'docs', 'CLAUDE.md'), `<!-- OMG:START -->\n<!-- OMG:VERSION:4.8.2 -->\n# Old\n<!-- OMG:END -->\n`);
    buildCoordinatorFixture(oldVersion, readFileSync(join(oldVersion, 'docs', 'CLAUDE.md'), 'utf-8'), '4.8.2');

    mkdirSync(join(newVersion, 'docs'), { recursive: true });
    writeFileSync(join(newVersion, 'docs', 'CLAUDE.md'), `<!-- OMG:START -->\n<!-- OMG:VERSION:4.9.0 -->\n# New\n<!-- OMG:END -->\n`);
  ...
  it('prefers newer cache version when installed_plugins.json points to an existing but stale older version', () => {
    const root = mkdtempSync(join(tmpdir(), 'omg-stale-json-old-version-'));
    tempRoots.push(root);
    ...
    writeFileSync(
      ...
      JSON.stringify({ plugins: ['oh-my-grok'] }),
    );
