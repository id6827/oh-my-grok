...
describe('update-check cache path', () => {
  it('uses the active Claude config dir as the canonical OMG cache root', () => {
    process.env.GROK_CONFIG_DIR = join('/tmp', 'omg-custom-claude');

    expect(getUpdateCheckCachePath()).toBe(
      join('/tmp', 'omg-custom-claude', '.omg', 'update-check.json'),
    );
  });
  ...
  it('keeps the hook updater writer and HUD reader on the shared helper path', () => {
    ...
    for (const source of [hudSource, hookSource, templateSource]) {
      expect(source).not.toMatch(/join\(homedir\(\),\s*['"]\.omg['"],\s*['"]update-check\.json['"]\)/);
    }
  });
});
