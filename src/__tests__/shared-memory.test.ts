...
describe('Shared Memory', () => {
  ...
  beforeEach(() => {
    ...
    tildeConfigDir = join(homedir(), `.omg-test-shared-memory-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  describe('isSharedMemoryEnabled', () => {
    ...
    it('should read config from the active GROK_CONFIG_DIR', () => {
      const claudeConfigDir = join(testDir, 'claude-config');
      mkdirSync(claudeConfigDir, { recursive: true });
      writeFileSync(join(claudeConfigDir, '.omg-config.json'), JSON.stringify({
        agents: {
          sharedMemory: {
            enabled: false,
          },
    ...
    it('should expand ~-prefixed GROK_CONFIG_DIR values', () => {
      mkdirSync(tildeConfigDir, { recursive: true });
      writeFileSync(join(tildeConfigDir, '.omg-config.json'), JSON.stringify({
        agents: {
          sharedMemory: {
            enabled: false,
          },
