...
function runUntilClose(
  ...
  return new Promise((resolve) => {
    ...
    const child = spawn(process.execPath, [RUN_CJS, script], {
      cwd,
      env: { ...process.env, ...extraEnv, GROK_CONFIG_DIR: join(cwd, '.claude') },
...
function configureDeferredAdapters(cwd: string): void {
  ...
  writeFileSync(join(configDir, '.omg-config.json'), JSON.stringify({
...
describe('SessionEnd run.cjs process exit regressions (#3477)', () => {
  ...
  function createProject(): string {
    const cwd = mkdtempSync(join(tmpdir(), 'omg-session-end-process-exit-'));
    tempDirs.push(cwd);
  ...
  it.skipIf(!HAS_GENERATED_DIST)('wiki-session-end exits without waiting for a live wiki lock', async () => {
    ...
    writeFileSync(join(cwd, '.omg', '.omg-config.json'), JSON.stringify({ wiki: { autoCapture: true } }));
