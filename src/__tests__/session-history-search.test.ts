...
describe('session history search', () => {
  ...
  beforeEach(() => {
    ...
    tildeClaudeDir = join(homedir(), `.omg-session-search-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  it('uses a ~-prefixed GROK_CONFIG_DIR for transcript discovery', async () => {
    process.env.GROK_CONFIG_DIR = `~/${basename(tildeClaudeDir)}`;
  ...
  it('encodes a Windows drive path the same way Claude Code names its project dir', () => {
    ...
    // matches Claude Code's actual project dir (e.g. ~/.grok/projects/C--Users-me-proj).
