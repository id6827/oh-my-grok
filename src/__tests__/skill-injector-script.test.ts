...
function isSkillDiscoveryPath(value) {
  ...
  return path === projectDir + '/.omg/skills' || path.includes('/.omg/skills/') || path.endsWith('/skills/omg-learned');
}
...
Write concise release notes.`);
  ...
  it('injects a matching learned skill through the trusted run.cjs Worker path', () => {
    ...
    writeFileSync(join(pluginRoot, 'hooks', 'hooks.json'), JSON.stringify({
      hooks: {
        UserPromptSubmit: [{ matcher: '', hooks: [{
          type: 'command',
          command: 'node "$GROK_PLUGIN_ROOT"/scripts/run.cjs "$GROK_PLUGIN_ROOT"/scripts/skill-injector.mjs',
          timeout: 15,
        }] }],
      },
    }));
    ...
    const result = spawnSync(NODE, [join(scriptsDir, 'run.cjs'), join(scriptsDir, 'skill-injector.mjs')], {
      ...
      env: { ...process.env, GROK_PLUGIN_ROOT: pluginRoot, DISABLE_OMC: '', OMC_SKIP_HOOKS: '' },
      timeout: 15000,
    });
