...
async function loadInstallerWithEnv(claudeConfigDir: string, homeDir: string) {
  vi.resetModules();
  process.env.GROK_CONFIG_DIR = claudeConfigDir;
...
function writeEnabledPluginSettings(claudeConfigDir: string): void {
  writeFileSync(
    ...
    JSON.stringify({ plugins: ['oh-my-grok'] }, null, 2)
  );
}
...
function writeMinimallyCompletePluginPayload(pluginRoot: string): void {
  ...
  writeFileSync(join(pluginRoot, 'commands', 'omg-setup.md'), 'Read skills/omg-setup/SKILL.md.\n');
  ...
  writeFileSync(join(pluginRoot, 'package.json'), JSON.stringify({ name: 'oh-my-grok', version: '4.10.2' }, null, 2));
}
...
describe('installer bundled + standalone skill sync', () => {
  ...
  beforeEach(() => {
    ...
    originalClaudeConfigDir = process.env.GROK_CONFIG_DIR;
  ...
  it('installs standalone slash skills into ~/.grok/skills during legacy install', async () => {
    ...
    expect(result.installedSkills).toEqual(expect.arrayContaining([
      ...
      'team/SKILL.md',
      'ultrawork/SKILL.md',
      'omg-reference/SKILL.md',
      'omg-plan/SKILL.md',
    ]));

    for (const skillName of ['autopilot', 'ralph', 'ralplan', 'team', 'ultrawork', 'omg-reference', 'omg-plan']) {
  ...
  it('installs bundled skills when no enabled OMG plugin is configured', async () => {
    const pluginRoot = join(tempRoot, 'plugin-cache', 'oh-my-grok', '4.10.2');
    mkdirSync(join(pluginRoot, 'skills', 'ralph'), { recursive: true });
    writeFileSync(join(pluginRoot, 'skills', 'ralph', 'SKILL.md'), 'name: ralph\n');
    ...
    expect(result.installedSkills).toContain('omg-plan/SKILL.md');

    for (const skillName of ['omg-reference', 'ralph', 'team']) {
      const installedSkillPath = join(claudeConfigDir, 'skills', skillName, 'SKILL.md');
      expect(existsSync(installedSkillPath)).toBe(true);
      expect(readFileSync(installedSkillPath, 'utf-8')).toContain(`name: ${skillName}`);
    }

    expect(existsSync(join(claudeConfigDir, 'skills', 'omg-setup', 'phases', '04-welcome.md'))).toBe(true);
  });
