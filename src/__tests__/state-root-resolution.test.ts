/**
 * Regression tests for issue #2532: centralized OMG_STATE_DIR state-root resolution.
 *
 * Verifies that:
 *   1. Default behavior (no OMG_STATE_DIR) is unchanged — state lives in {dir}/.omg/
 *   2. session-start.mjs reads session state from the custom OMG_STATE_DIR location
 *   3. persistent-mode.cjs (stop hook) reads mode state from the custom OMG_STATE_DIR
 ...
 *      skill-active-state through the centralized OMG_STATE_DIR location
 */
...
function buildHookEnv(extraEnv: Record<string, string> = {}): Record<string, string> {
  ...
  // Remove OMG_STATE_DIR from parent env so only extraEnv controls it.
  delete env.OMG_STATE_DIR;
  return { ...env, GROK_PLUGIN_ROOT: REPO_ROOT, ...extraEnv };
}
...
/**
 * Compute the centralized .omg root path for a given project dir and state dir.
 * Temporarily sets OMG_STATE_DIR so getOmcRoot() returns the centralized path.
 */
function getCentralizedOmcRoot(projectDir: string, stateDir: string): string {
  const prev = process.env.OMG_STATE_DIR;
  try {
    ...
    if (prev === undefined) {
      ...
      process.env.OMG_STATE_DIR = prev;
    }
    clearWorktreeCache();
  }
}
...
describe('OMG_STATE_DIR state-root resolution (issue #2532)', () => {
  let tempDir: string;
  let fakeProject: string;
  let fakeStateDir: string;

  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'omg-state-root-'));
  ...
  // ────────────────────────────────────────────────────────────────────────────
  // 1. Default state dir (no OMG_STATE_DIR)
  // ────────────────────────────────────────────────────────────────────────────

  it('session-start reads ralph state from default .omg path when OMG_STATE_DIR is not set', () => {
  ...
  // ────────────────────────────────────────────────────────────────────────────
  // 2. Custom OMG_STATE_DIR — session-start
  // ────────────────────────────────────────────────────────────────────────────

  it('session-start reads ralph state from centralized path when OMG_STATE_DIR is set', () => {
  ...
  it('session-start reads ultrawork state from centralized path when OMG_STATE_DIR is set', () => {
  ...
  it('session-start does NOT restore state when OMG_STATE_DIR is set but state is only in default .omg', () => {
    const sessionId = 'test-session-only-default';
    // Place state ONLY in the default .omg location
    ...
    writeFileSync(
      ...
      JSON.stringify({
        active: true,
        session_id: sessionId,
        prompt: 'Should not restore from default when OMG_STATE_DIR is set',
        iteration: 1,
        max_iterations: 5,
      }),
    );

    // Run with OMG_STATE_DIR pointing elsewhere — centralized location is empty
    const output = runHook(
      SESSION_START,
      { hook_event_name: 'SessionStart', session_id: sessionId, cwd: fakeProject },
      { OMG_STATE_DIR: fakeStateDir },
  ...
  // ────────────────────────────────────────────────────────────────────────────
  // 3. Custom OMG_STATE_DIR — stop hook (persistent-mode.cjs)
  // ────────────────────────────────────────────────────────────────────────────

  it('stop hook blocks when active ralph state is in default .omg path (baseline)', () => {
  ...
  it('stop hook blocks when active ralph state is in centralized OMG_STATE_DIR path', () => {
    ...
    // No .omg in fakeProject — active state ONLY in centralized dir
    const output = runHook(
      STOP_HOOK,
  ...
  it('stop hook does NOT block when OMG_STATE_DIR is set but state is only in default .omg', () => {
    ...
    // Run stop hook with OMG_STATE_DIR — centralized path is empty → no block
    const output = runHook(
      STOP_HOOK,
      { hook_event_name: 'Stop', session_id: sessionId, cwd: fakeProject },
      { OMG_STATE_DIR: fakeStateDir },
  ...
  //    from the same centralized OMG_STATE_DIR location as sibling hooks.
  // ────────────────────────────────────────────────────────────────────────────

  it('pre-tool-enforcer injects [TEAM ROUTING REQUIRED] when team-state lives in default .omg (baseline)', () => {
  ...
  it('pre-tool-enforcer injects [TEAM ROUTING REQUIRED] when team-state lives in centralized OMG_STATE_DIR', () => {
    ...
    // No .omg in fakeProject — active team-state ONLY in centralized dir
    const output = runHook(
      PRE_TOOL_ENFORCER,
      {
        hook_event_name: 'PreToolUse',
  ...
  it('pre-tool-enforcer ignores stale team-state in default .omg when OMG_STATE_DIR is set', () => {
    ...
    // Run with OMG_STATE_DIR pointing elsewhere — centralized path is empty → no redirect
    const output = runHook(
      PRE_TOOL_ENFORCER,
      {
        hook_event_name: 'PreToolUse',
  ...
  it('pre-tool-enforcer writes skill-active-state into the centralized OMG_STATE_DIR path', () => {
    ...
    runHook(
      PRE_TOOL_ENFORCER,
      {
        hook_event_name: 'PreToolUse',
        tool_name: 'Skill',
        // `skill` needs a non-'none' protection level. The OMG-prefixed `skill`
        // slash-command maps to 'light' protection, which triggers the write.
        tool_input: { skill: 'oh-my-grok:skill' },
        session_id: sessionId,
        cwd: fakeProject,
      },
      { OMG_STATE_DIR: fakeStateDir },
