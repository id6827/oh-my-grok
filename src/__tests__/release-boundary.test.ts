...
function packageManifest(gitHead?: string): Record<string, unknown> {
  return {
    ...
    ...(gitHead ? { gitHead } : {}),
    bin: {
      'oh-my-grok': 'bin/oh-my-grok.js',
      omg: 'bin/oh-my-grok.js',
...
function releaseTarball(gitHead = SHA, extraEntries: TarEntry[] = [], readme = '# fixture\n'): Buffer {
  return makeTarball([
    { path: 'package/package.json', content: `${JSON.stringify(packageManifest(gitHead), null, 2)}\n` },
    {
      ...
      content: JSON.stringify({ name: 'oh-my-grok', version: VERSION }),
    },
    {
      path: 'package/.claude-plugin/marketplace.json',
      content: JSON.stringify({
        version: VERSION,
        plugins: [{ name: 'oh-my-grok', version: VERSION, source: './' }],
      }),
    },
    {
      ...
      content: JSON.stringify({
        mcpServers: {
          omg: { command: 'node', args: ['${GROK_PLUGIN_ROOT}/bridge/mcp-server.cjs'] },
        },
      }),
    },
    { path: 'package/bin/oh-my-grok.js', content: '#!/usr/bin/env node\n', mode: 0o755 },
  ...
  return {
    ...
    predicate: {
      buildDefinition: {
        ...
        externalParameters: {
          workflow: {
            repository: 'https://github.com/Yeachan-Heo/oh-my-grok',
            path: '.github/workflows/release.yml',
            ref: `refs/tags/${TAG}`,
          },
        },
        resolvedDependencies: [{
          uri: `git+https://github.com/Yeachan-Heo/oh-my-grok@refs/tags/${TAG}`,
          digest: { gitCommit: SHA },
        }],
      },
      runDetails: {
        builder: { id: 'https://github.com/actions/runner/github-hosted' },
        metadata: { invocationId: 'https://github.com/Yeachan-Heo/oh-my-grok/actions/runs/123' },
      },
    },
    ...overrides,
  };
...
function createTriggerRepository(): { root: string; sha: string } {
  ...
  writeFileSync(join(root, '.claude-plugin', 'plugin.json'), JSON.stringify({ name: 'oh-my-grok', version: VERSION }));
  writeFileSync(join(root, '.claude-plugin', 'marketplace.json'), JSON.stringify({
    version: VERSION,
    plugins: [{ name: 'oh-my-grok', version: VERSION }],
  }));
  writeFileSync(join(root, 'docs', 'CLAUDE.md'), `<!-- OMG:VERSION:${VERSION} -->\n`);
  writeFileSync(join(root, 'CHANGELOG.md'), `# oh-my-grok v${VERSION}: fixture\n`);
...
describe('release-boundary.mjs', () => {
  ...
  it('requires an annotated tag, exact checkout SHA, version parity, and committed release notes', () => {
    ...
    writeFileSync(join(root, 'docs', 'CLAUDE.md'), '<!-- OMG:VERSION:4.15.3 -->\n');
  ...
  it('asserts the final archive surfaces and records tamper-evident byte and content manifests', async () => {
    ...
    const missingCoordinatorPath = writeTarball(root, 'missing-coordinator.tgz', makeTarball([
      ...
      { path: 'package/.claude-plugin/plugin.json', content: JSON.stringify({ name: 'oh-my-grok', version: VERSION }) },
      { path: 'package/.claude-plugin/marketplace.json', content: JSON.stringify({ version: VERSION, plugins: [{ name: 'oh-my-grok', version: VERSION, source: './' }] }) },
      { path: 'package/.mcp.json', content: JSON.stringify({ mcpServers: { omg: { command: 'node', args: ['${GROK_PLUGIN_ROOT}/bridge/mcp-server.cjs'] } } }) },
      { path: 'package/bin/oh-my-grok.js', content: '#!/usr/bin/env node\n', mode: 0o755 },
      { path: 'package/bridge/cli.cjs', content: 'module.exports = {};\n' },
      { path: 'package/bridge/mcp-server.cjs', content: 'module.exports = {};\n' },
      { path: 'package/bridge/runtime-cli.cjs', content: 'module.exports = {};\n' },
