...
describe('tmux-detector', () => {
  ...
  describe('analyzePaneContent', () => {
    ...
    it('should detect OMG HUD rate-limit pane without Claude branding', () => {
      const content = `
        ● You've hit your session limit · resets 12pm (Asia/Tokyo)
        ─────────────────────────────────────────────
        ❯
        ─────────────────────────────────────────────
          [OMG#4.15.1L] | Model: Opus 4.8 | 5h:100% wk:14% | thinking | session:80m | ctx:14%
    ...
    it('should not treat copied HUD snippets plus rate-limit text as OMG pane evidence', () => {
      const content = `
        $ cat notes.txt
    ...
    it('should not treat copied full HUD block followed by rate-limit output as OMG pane evidence', () => {
      const content = `
        $ cat copied-hud.txt
        [OMG#4.15.1L] | Model: Opus 4.8 | 5h:100% wk:14% | thinking | session:80m | ctx:14%
        ⏵⏵ auto mode on (shift+tab to cycle) · ← for agents
    ...
    it('should not treat copied full HUD block without shell prompt as OMG pane evidence', () => {
      const content = `
        copied from saved terminal output:
        unrelated API response: Error: rate limit exceeded
        [OMG#4.15.1L] | Model: Opus 4.8 | 5h:100% wk:14% | thinking | session:80m | ctx:14%
    ...
    it('should not treat copied HUD chrome without shell prompt as OMG pane evidence', () => {
      const content = `
        copied from saved terminal output:
        unrelated API response: Error: rate limit exceeded
        ─────────────────────────────────────────────
    ...
    it('should not treat copied HUD chrome from cat output as OMG pane evidence', () => {
      const content = `
        $ cat copied-hud.txt
        unrelated API response: Error: rate limit exceeded
        ─────────────────────────────────────────────
    ...
    it('should not treat search output containing HUD markers as OMG pane evidence', () => {
      const content = `
        $ rg "OMG#|shift\\+tab|rate limit" src
        src/__tests__/rate-limit-wait/tmux-detector.test.ts
        *152cz|          [OMG#4.15.1L] | Model: Opus 4.8 | 5h:100% wk:14% | thinking | session:80m | ctx:14%
    ...
    it('should not treat test output containing shift-tab HUD help as OMG pane evidence', () => {
      const content = `
    ...
    it('should not flag plain shell output without Claude or OMG evidence', () => {
      const content = `
        $ npm run build
    ...
    it('should not treat model labels alone as Claude or OMG evidence', () => {
      const content = `
        $ echo "Model: Opus 4.8"
        Model: Opus 4.8
        Error: rate limit exceeded
