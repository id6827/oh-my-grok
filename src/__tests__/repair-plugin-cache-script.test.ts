...
function writePluginRoot(root: string, version: string): void {
  ...
  mkdirSync(join(root, 'skills', 'omg-setup'), { recursive: true });
  mkdirSync(join(root, 'docs'), { recursive: true });
  writeFileSync(join(root, 'hooks', 'hooks.json'), '{}\n');
  writeFileSync(join(root, 'skills', 'omg-setup', 'SKILL.md'), '# setup\n');
  writeFileSync(join(root, 'docs', 'CLAUDE.md'), `<!-- OMG:VERSION:${version} -->\n`);
}
...
describe('repair-plugin-cache.mjs', () => {
  it('rewrites stale installed_plugins.json and keeps old cache path as a symlink fallback', () => {
    const root = mkdtempSync(join(tmpdir(), 'omg-repair-plugin-cache-'));
    tempRoots.push(root);
    ...
    writeFileSync(join(configDir, 'plugins', 'installed_plugins.json'), JSON.stringify({
      version: 2,
      plugins: {
        'oh-my-grok@omg': [{ installPath: oldRoot, version: '4.11.6', enabled: true }],
      },
    }, null, 2));

    const result = spawnSync(process.execPath, [SCRIPT_PATH], {
      env: { ...process.env, GROK_CONFIG_DIR: configDir, OMC_REPAIR_PLUGIN_CACHE_PLATFORM: 'linux' },
      encoding: 'utf-8',
    });
    ...
    expect(registry.plugins['oh-my-grok@omg'][0]).toMatchObject({
      installPath: newRoot,
      version: '4.14.1',
      enabled: true,
    });
  ...
  it('repairs a registry entry whose old cache path was already deleted', () => {
    const root = mkdtempSync(join(tmpdir(), 'omg-repair-missing-cache-'));
    tempRoots.push(root);
    ...
    writePluginRoot(newRoot, '4.14.1');
    writeFileSync(join(configDir, 'plugins', 'installed_plugins.json'), JSON.stringify({
      'oh-my-grok@omg': [{ installPath: oldRoot, version: '4.11.6' }],
    }, null, 2));
    ...
    expect(existsSync(join(oldRoot, 'hooks', 'hooks.json'))).toBe(true);
    const registry = JSON.parse(readFileSync(join(configDir, 'plugins', 'installed_plugins.json'), 'utf-8'));
    expect(registry['oh-my-grok@omg'][0]).toMatchObject({
      installPath: newRoot,
      version: '4.14.1',
    });
  });
  ...
  it.runIf(process.platform !== 'win32')('repairs Unix cache hooks from direct node to the find-node bootstrap', () => {
    const root = mkdtempSync(join(tmpdir(), 'omg-repair-unix-hooks-'));
    tempRoots.push(root);
    ...
    const result = spawnSync(process.execPath, [SCRIPT_PATH], {
      env: { ...process.env, GROK_CONFIG_DIR: configDir },
      encoding: 'utf-8',
    });
  ...
  it.runIf(process.platform !== 'win32')('repairs every bundled direct-node hook command to find-node on Unix/macOS', () => {
    const root = mkdtempSync(join(tmpdir(), 'omg-repair-unix-bundled-hooks-'));
    tempRoots.push(root);
  ...
  it('repairs Windows cache hooks from find-node to direct node', () => {
    const root = mkdtempSync(join(tmpdir(), 'omg-repair-win-hooks-'));
    tempRoots.push(root);
    ...
    writeFileSync(join(pluginRoot, 'hooks', 'hooks.json'), JSON.stringify({
      hooks: {
        SessionEnd: [{
          ...
          hooks: [{
            type: 'command',
            command: 'sh "$GROK_PLUGIN_ROOT"/scripts/find-node.sh "$GROK_PLUGIN_ROOT"/scripts/run.cjs "$GROK_PLUGIN_ROOT"/scripts/session-end.mjs',
          }],
        }],
      },
    }, null, 2));
    ...
    const result = spawnSync(process.execPath, [SCRIPT_PATH], {
      env: { ...process.env, GROK_CONFIG_DIR: configDir, OMC_REPAIR_PLUGIN_CACHE_PLATFORM: 'win32' },
      encoding: 'utf-8',
    });
  ...
  it('setup instructions delegate cache resolution and retain phase repair without unsafe deletion', () => {
    const setupSkill = readFileSync(join(REPO_ROOT, 'skills', 'omg-setup', 'SKILL.md'), 'utf-8');
    const phase = readFileSync(join(REPO_ROOT, 'skills', 'omg-setup', 'phases', '02-configure.md'), 'utf-8');
