 ...
 * Two core contracts:
 *   1. Never hardcode paths — use getClaudeConfigDir() or GROK_CONFIG_DIR env var
 *   2. Never install to root ~/.claude when GROK_CONFIG_DIR is set to a custom path
 *
...
describe('Contract 1: no join(homedir()...".claude") outside canonical helpers', () => {
  ...
  const EXCLUDED_FUNCTIONS = [
    ...
    'prepareOmcLaunchConfigDir', // entry-point with its own GROK_CONFIG_DIR || fallback
  ];
...
// Issue #2155 §11-13 — scripts with inline $HOME/.claude without GROK_CONFIG_DIR guard
...
describe('Contract 2: no unguarded $HOME/.claude in shell/script files', () => {
  ...
  // The safe pattern: ${GROK_CONFIG_DIR:-$HOME/.claude}
  const SAFE_PATTERN = /\$\{GROK_CONFIG_DIR:-\$HOME\/\.claude\}/;
  ...
  it('has no $HOME/.claude without ${GROK_CONFIG_DIR:-...} guard in scripts', () => {
    if (violations.length > 0) {
      const details = violations
        .map(v => `  ${v.file}:${v.line}: ${v.text}`)
        .join('\n');
      expect.fail(
        `Found $HOME/.claude without GROK_CONFIG_DIR guard:\n${details}\n\n` +
        `Replace with: \${GROK_CONFIG_DIR:-$HOME/.claude}`
      );
    }
  });
});
...
describe('Contract 2b: setup jq writes are guarded against truncation', () => {
  const SETUP_MUTATION_FILES = [
    join(REPO_ROOT, 'skills', 'omg-setup', 'phases', '02-configure.md'),
    join(REPO_ROOT, 'skills', 'omg-setup', 'phases', '03-integrations.md'),
...
describe('Contract 5: no hardcoded ~/.claude in LLM-consumed artifacts', () => {
  ...
  // Match ~/.claude NOT inside portable notation [$GROK_CONFIG_DIR|~/.claude]
  // or ${GROK_CONFIG_DIR:-...} pattern
  const TILDE_CLAUDE_PATTERN = /~\/\.claude/;
  const SAFE_PORTABLE = /\[\$GROK_CONFIG_DIR\|~\/\.claude\]/;
  const SAFE_ENV_FALLBACK = /\$\{GROK_CONFIG_DIR:-/;
  ...
  it('agents/*.md have no unguarded ~/.claude references', () => {
    ...
    if (violations.length > 0) {
      ...
      expect.fail(
        ...
        `Use [$GROK_CONFIG_DIR|~/.claude] notation in LLM-consumed artifacts.`
      );
    }
  });
  ...
  it('docs/CLAUDE.md (the installed template) has no unguarded ~/.claude references', () => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (TILDE_CLAUDE_PATTERN.test(line) && !SAFE_PORTABLE.test(line) && !SAFE_ENV_FALLBACK.test(line)) {
        ...
        if (/default.*~\/\.claude/i.test(line) || /fallback.*~\/\.claude/i.test(line)) continue;
        if (/GROK_CONFIG_DIR/i.test(line)) continue;
        // Skip glob/permission patterns like ~/.grok/** (describes allowed paths, not path resolution)
...
// ── Contract 9: hooks/hooks.json commands use $GROK_PLUGIN_ROOT, no absolute paths ──
...
describe('Contract 9: hooks/hooks.json portability', () => {
  ...
  it('all hook commands reference $GROK_PLUGIN_ROOT', () => {
    ...
    if (violations.length > 0) {
      ...
      expect.fail(
        `Found hook commands not using $GROK_PLUGIN_ROOT:\n${details}\n\n` +
        `All plugin hook commands must reference $GROK_PLUGIN_ROOT for portability.`
      );
    }
  });
...
// ── Contract 10: Setup installer manages stale OMG-created files ─────────────
// User requirement: setup cleanup stale ~/.grok/skills and ~/.grok/agents created by OMG

describe('Contract 10: installer manages stale OMG-created agents and skills', () => {
  ...
  it('syncBundledSkillDefinitions overwrites existing OMG skills (force copy)', () => {
  ...
  it('OMG agent filenames are all lowercase kebab-case .md files', () => {
  ...
  it('OMG skill directories match a consistent naming pattern', () => {
    const skillsDir = join(REPO_ROOT, 'skills');
    const skillDirs = readdirSync(skillsDir, { withFileTypes: true })
      .filter(d => d.isDirectory() && existsSync(join(skillsDir, d.name, 'SKILL.md')));
...
describe('OMG setup Ralph Ruby dependency guidance (issue #2969)', () => {
  it('checks Ruby during setup with product-facing Ralph remediation', () => {
    const phasePath = join(REPO_ROOT, 'skills', 'omg-setup', 'phases', '02-configure.md');
