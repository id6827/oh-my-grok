...
describe('setup-progress.sh', () => {
  it('writes setup completion metadata to GROK_CONFIG_DIR', () => {
    const root = mkdtempSync(join(tmpdir(), 'omg-setup-progress-'));
    tempRoots.push(root);
    ...
    const configPath = join(configDir, '.omg-config.json');
  ...
  it('fails without jq and preserves existing setup config', () => {
    const root = mkdtempSync(join(tmpdir(), 'omg-setup-progress-no-jq-'));
    tempRoots.push(root);
