...
describe('Claude Code compatibility for OMG-authored user skills', () => {
  let tempDir: string;
  let originalClaudeConfigDir: string | undefined;

  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'omg-user-skill-compat-'));
  ...
  it('reproduces that nested omg-learned skills are invisible to flat Claude Code lookup until compat sync runs', async () => {
    const nestedSkillPath = join(tempDir, 'skills', 'omg-learned', 'expert-review', 'SKILL.md');
  ...
  it('keeps new user skills in omg-learned/<name>.md and exposes a flat Claude Code SKILL.md shim', async () => {
    ...
    expect(result.path).toBe(join(tempDir, 'skills', 'omg-learned', 'expert-review.md'));
  ...
  it('uses a symlink shim when supported so edits to the omg-learned source remain visible', async () => {
    const nestedSkillPath = join(tempDir, 'skills', 'omg-learned', 'expert-review', 'SKILL.md');
    mkdirSync(join(nestedSkillPath, '..'), { recursive: true });
    writeFileSync(nestedSkillPath, '---\nname: expert-review\ntriggers: [expert-review]\n---\n\nBefore edit.\n');
