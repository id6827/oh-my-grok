...
} from '../hooks/omg-orchestrator/index.js';
import type { AuditEntry } from '../hooks/omg-orchestrator/audit.js';
...
describe('delegation-enforcement-levels', () => {
  const savedConfigDir = process.env.GROK_CONFIG_DIR;

  beforeEach(() => {
    vi.clearAllMocks();
    clearEnforcementCache();
    // Ensure tests use the mocked homedir, not a custom GROK_CONFIG_DIR
  ...
  afterEach(() => {
    if (savedConfigDir !== undefined) {
      process.env.GROK_CONFIG_DIR = savedConfigDir;
    } else {
      delete process.env.GROK_CONFIG_DIR;
    }
  });
  ...
  describe('isAllowedPath', () => {
    ...
    it('returns true for absolute paths under GROK_CONFIG_DIR', () => {
      const originalConfigDir = process.env.GROK_CONFIG_DIR;
      process.env.GROK_CONFIG_DIR = '/custom/claude-config';
      try {
    ...
    it('returns true for absolute paths under a ~-prefixed GROK_CONFIG_DIR', () => {
      const originalConfigDir = process.env.GROK_CONFIG_DIR;
