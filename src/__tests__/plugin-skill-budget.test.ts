...
describe('plugin skill context budget gate (issues #2943, #2986)', () => {
  ...
  it('compacts installed plugin SKILL.md files while archiving full on-demand skill bodies', () => {
    const tempRoot = mkdtempSync(join(tmpdir(), 'omg-plugin-skill-budget-'));
    try {
      ...
      for (const skillDir of allSkillDirs) {
        ...
        expect(shim, `${skillDir} shim should expose runtime body override`).toContain('omg-full-body:');
        expect(shim, `${skillDir} shim should prefer plugin root env vars`).toContain(
          `\${GROK_PLUGIN_ROOT:-\${OMC_PLUGIN_ROOT}}/skill-bodies/${skillDir}/SKILL.md`,
        );
  ...
  it('uses platform-safe containment for archived full-body skill paths', () => {
    const winRoot = 'C:\\Users\\me\\.claude\\plugins\\cache\\omg\\oh-my-grok\\4.13.7';
  ...
  it('materializes declared plugin command wrappers into cache sync targets', () => {
    const tempRoot = mkdtempSync(join(tmpdir(), 'omg-plugin-commands-cache-'));
    try {
      const sourceRoot = join(tempRoot, 'source');
      const targetRoot = join(tempRoot, 'cache', 'omg', 'oh-my-grok', '4.14.1');
      ...
      writeFileSync(join(sourceRoot, 'commands', 'omg-setup.md'), 'Read skills/omg-setup/SKILL.md and pass $ARGUMENTS.\n');
      ...
      writeFileSync(join(sourceRoot, 'package.json'), JSON.stringify({ name: 'oh-my-grok', version: '4.14.1' }));
