...
function runPreToolEnforcerWithEnv(
  ...
  const stdout = execFileSync(process.execPath, [SCRIPT_PATH], {
    ...
    env: {
      ...process.env,
      HOME: homeDir,
      GROK_CONFIG_DIR: join(homeDir, '.claude'),
      NODE_ENV: 'test',
      DISABLE_OMC: '',
      OMC_SKIP_HOOKS: '',
...
describe('pre-tool-enforcer fallback gating (issue #970)', () => {
  ...
  it('preserves Agent spawn warnings for real subagent delegation', () => {
    ...
    const hookSpecificOutput = output.hookSpecificOutput as Record<string, unknown>;
    expect(output.continue).toBe(true);
    expect(String(hookSpecificOutput.additionalContext)).toContain('Spawning agent: oh-my-grok:executor');
  });
  ...
  it('allows tier alias "sonnet" through when OMC_SUBAGENT_MODEL is set and forceInherit is enabled', () => {
    const output = runPreToolEnforcerWithEnv(
      {
        tool_name: 'Agent',
        toolInput: { subagent_type: 'oh-my-grok:architect', model: 'sonnet' },
        cwd: tempDir,
  ...
  it('allows tier alias "opus" via ANTHROPIC_DEFAULT_OPUS_MODEL without OMC_SUBAGENT_MODEL', () => {
    const output = runPreToolEnforcerWithEnv(
      {
        tool_name: 'Agent',
        toolInput: { subagent_type: 'oh-my-grok:architect', model: 'opus' },
        cwd: tempDir,
  ...
  it('allows tier alias "haiku" via ANTHROPIC_DEFAULT_HAIKU_MODEL without OMC_SUBAGENT_MODEL', () => {
    const output = runPreToolEnforcerWithEnv(
      {
        tool_name: 'Agent',
        toolInput: { subagent_type: 'oh-my-grok:executor', model: 'haiku' },
        cwd: tempDir,
  ...
  it('allows tier alias "fable" via ANTHROPIC_DEFAULT_FABLE_MODEL without OMC_SUBAGENT_MODEL (issue #3246)', () => {
    const output = runPreToolEnforcerWithEnv(
      {
        tool_name: 'Agent',
        toolInput: { subagent_type: 'oh-my-grok:architect', model: 'fable' },
        cwd: tempDir,
  ...
  it('resolves tier alias "fable" via CLAUDE_CODE_BEDROCK_FABLE_MODEL (issue #3246)', () => {
    const output = runPreToolEnforcerWithEnv(
      {
        tool_name: 'Agent',
        toolInput: { subagent_type: 'oh-my-grok:executor', model: 'fable' },
        cwd: tempDir,
  ...
  it.each([
    ...
    const output = runPreToolEnforcerWithEnv(
      {
        tool_name: 'Agent',
        toolInput: { subagent_type: 'oh-my-grok:executor', model: tier },
        cwd: tempDir,
        session_id: sessionId,
      },
      {
  ...
  it('blocks tier alias when proxy ANTHROPIC_DEFAULT_*_MODEL is only whitespace', () => {
    const output = runPreToolEnforcerWithEnv(
      {
        tool_name: 'Agent',
        toolInput: { subagent_type: 'oh-my-grok:executor', model: 'sonnet' },
        cwd: tempDir,
    ...
    const output = runPreToolEnforcerWithEnv(
      {
        ...
        toolInput: { subagent_type: 'oh-my-grok:executor', model: 'sonnet' },
        cwd: tempDir,
        session_id: 'session-tier-omg-model-fallback',
      },
      {
        OMC_ROUTING_FORCE_INHERIT: 'true',
        OMC_SUBAGENT_MODEL: '',
  ...
  it('blocks tier alias when only OMC_MODEL_* is set (not a CC-side routing proof)', () => {
    // OMC_MODEL_* proves OMG-bridge routing, not CC model resolution. Without a CC-native
    ...
    const output = runPreToolEnforcerWithEnv(
      {
        ...
        toolInput: { subagent_type: 'oh-my-grok:executor', model: 'sonnet' },
        cwd: tempDir,
        session_id: 'session-tier-omg-model-only',
      },
      {
        OMC_ROUTING_FORCE_INHERIT: 'true',
        OMC_SUBAGENT_MODEL: '',
    ...
    const output = runPreToolEnforcerWithEnv(
      {
        tool_name: 'Agent',
        toolInput: {
          subagent_type: 'oh-my-grok:critic',
  ...
  it('blocks tier alias when OMC_SUBAGENT_MODEL has a [1m] extended-context suffix', () => {
    const output = runPreToolEnforcerWithEnv(
      {
        tool_name: 'Agent',
        toolInput: { subagent_type: 'oh-my-grok:executor', model: 'opus' },
        cwd: tempDir,
  ...
  it('still blocks bare Anthropic model ID even when OMC_SUBAGENT_MODEL is set', () => {
    const output = runPreToolEnforcerWithEnv(
      {
        tool_name: 'Agent',
        toolInput: { subagent_type: 'oh-my-grok:executor', model: 'claude-sonnet-4-6' },
        cwd: tempDir,
    ...
    const output = runPreToolEnforcerWithEnv(
      {
        tool_name: 'Agent',
        toolInput: {
          subagent_type: 'oh-my-grok:../docs/CLAUDE',
          description: 'Some task',
          prompt: 'Do something',
        },
        cwd: tempDir,
  ...
  it('falls back to script-relative agents dir when GROK_PLUGIN_ROOT points to a non-existent path and allows shipped tier aliases', () => {
    const output = runPreToolEnforcerWithEnv(
      {
        tool_name: 'Agent',
        toolInput: {
      ...
      },
      {
        OMC_ROUTING_FORCE_INHERIT: 'true',
        OMC_SUBAGENT_MODEL: 'global.anthropic.claude-sonnet-4-6',
        GROK_PLUGIN_ROOT: '/nonexistent/path/that/does/not/exist',
      },
    );
  ...
  it('falls back to script-relative agents dir when GROK_PLUGIN_ROOT/agents exists but lacks the specific agent file, and allows shipped tier aliases', () => {
    // GROK_PLUGIN_ROOT/agents/ exists (non-empty check passes) but does not contain critic.md
    ...
    const output = runPreToolEnforcerWithEnv(
      {
        tool_name: 'Agent',
        toolInput: {
          subagent_type: 'oh-my-grok:body-hr-agent',
          description: 'Some task',
          prompt: 'Do something',
        },
        cwd: tempDir,
    ...
    const output = runPreToolEnforcerWithEnv(
      {
        tool_name: 'Agent',
        toolInput: {
          subagent_type: 'oh-my-grok:body-model-agent',
          description: 'Some task',
          prompt: 'Do something',
        },
        cwd: tempDir,
    ...
    const output = runPreToolEnforcerWithEnv(
      {
        tool_name: 'Agent',
        toolInput: {
          subagent_type: 'oh-my-grok:quoted-model-agent',
          description: 'Review spec',
          prompt: 'Review this spec',
        },
        cwd: tempDir,
    ...
    const output = runPreToolEnforcerWithEnv(
      {
        tool_name: 'Agent',
        toolInput: {
          subagent_type: 'oh-my-grok:bedrock-quoted-agent',
    ...
    const output = runPreToolEnforcerWithEnv(
      {
        tool_name: 'Agent',
        toolInput: {
          subagent_type: 'oh-my-grok:bom-agent',
    ...
    const output = runPreToolEnforcerWithEnv(
      {
        tool_name: 'Agent',
        toolInput: {
          subagent_type: 'oh-my-grok:nonexistent-agent-xyz',
          description: 'Some task',
          prompt: 'Do something',
        },
        cwd: tempDir,
...
describe('pre-tool-enforcer force-agent-delegation enforcement', () => {
  ...
  it('blocks the call that crosses the threshold and surfaces the configured deny message', () => {
    const denyMessage =
      'Too many Reads — spawn Agent(subagent_type=\'oh-my-grok:explore\', model=\'haiku\'). Bypass: ALLOW_RAW_READ=1.';
    writeDelegationConfig([
      {
        pattern: 'Read',
...
describe('pre-tool-enforcer agents.<name>.model injection (issue #3242)', () => {
  ...
  beforeEach(() => {
    ...
    mkdirSync(join(xdgConfigHome, 'claude-omg'), { recursive: true });
  });
  ...
  function writeUserConfig(jsonc: string): void {
    writeFileSync(join(xdgConfigHome, 'claude-omg', 'config.jsonc'), jsonc);
  }
  ...
  it('injects configured model via updatedInput for native Task calls without a model param', () => {
    ...
    const output = run({
      tool_name: 'Task',
      toolInput: { subagent_type: 'oh-my-grok:explore', prompt: 'x', description: 'find files' },
  ...
  it('does not inject when no per-agent override is configured', () => {
    ...
    const output = run({
      tool_name: 'Task',
      toolInput: { subagent_type: 'oh-my-grok:architect', prompt: 'x', description: 'design' },
  ...
  it('preserves an explicit model param and does not inject', () => {
    ...
    const output = run({
      tool_name: 'Task',
      toolInput: { subagent_type: 'oh-my-grok:explore', model: 'opus', prompt: 'x', description: 'd' },
  ...
  it('normalizes full Claude model IDs to a CC tier alias', () => {
    ...
    const output = run({
      tool_name: 'Task',
      toolInput: { subagent_type: 'oh-my-grok:executor', prompt: 'x', description: 'd' },
  ...
  it('lets project config override user config', () => {
    ...
    const output = run({
      tool_name: 'Task',
      toolInput: { subagent_type: 'oh-my-grok:explore', prompt: 'x', description: 'd' },
  ...
  it('resolves deprecated subagent aliases to the canonical config key', () => {
    ...
    const output = run({
      tool_name: 'Task',
      toolInput: { subagent_type: 'oh-my-grok:reviewer', prompt: 'x', description: 'd' },
