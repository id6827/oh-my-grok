...
const REPOSITORY = 'Yeachan-Heo/oh-my-grok';
...
describe('generated-artifact base-owned authorization decision', () => {
  it('contains the exact independently derived #3537 closure and allows only its exact positive case', () => {
    ...
    expect(exactAuthorization).toMatchObject({
      ...
      generatedDelta: {
        count: 199,
        sha256: '3cc92a253062a554f8583405a0f1a9655a31e8e2b5d08e25c2bc6dcb1eebdd94',
      },
    });
  ...
  it('allows ordinary contributor pull requests with no generated changes and no authorization entry', () => {
    ...
    input.event.pull_request.head.repo.full_name = 'contributor/oh-my-grok';
    input.event.pull_request.user.login = 'contributor';
    input.event.pull_request.author_association = 'CONTRIBUTOR';
    input.livePull.head.repo.full_name = 'contributor/oh-my-grok';
  ...
  it('requires separate main runtime/workflow and explicit event-base provenance before every decision', () => {
    ...
    expectDenied(input => {
      input.environment.githubWorkflowRef = `attacker/oh-my-grok/.github/workflows/generated-artifact-authorization.yml@refs/heads/main`;
    ...
    expectDenied(input => {
      input.repositoryMetadata.full_name = 'attacker/oh-my-grok';
  ...
  it('rejects generated changes from forks and non-owner contributors', () => {
    expectDenied(input => {
      input.event.pull_request.head.repo.full_name = 'fork/oh-my-grok';
      input.livePull.head.repo.full_name = 'fork/oh-my-grok';
    }, 'fork');
    expectDenied(input => {
      input.event.pull_request.user.login = 'contributor';
      input.livePull.user.login = 'contributor';
