...
describe('HUD Windows Compatibility', () => {
  ...
  describe('Cross-Platform Plugin Cache Path (#670)', () => {
    it('getPluginCacheBase should return path with correct segments', () => {
      ...
      expect(normalized).toContain('plugins/cache/omg/oh-my-grok');
    });

    it('getPluginCacheBase should use platform-native separators', () => {
      const cachePath = getPluginCacheBase();
      // On Windows: backslashes, on Unix: forward slashes
      expect(cachePath).toContain(`plugins${sep}cache${sep}omg${sep}oh-my-grok`);
    });
    ...
    it('shared HUD wrapper template should respect GROK_CONFIG_DIR for plugin cache base', () => {
      const templatePath = join(packageRoot, 'scripts', 'lib', 'hud-wrapper-template.txt');
      const content = readFileSync(templatePath, 'utf-8');
    ...
    it('omg-doctor skill should use cross-platform Node.js commands', () => {
      ...
      expect(content).toContain("p.join(d,'plugins','cache','omg','oh-my-grok')");
      expect(content).not.toContain('ls ~/.grok/CLAUDE-*.md');
      expect(content).toContain("find \"${GROK_CONFIG_DIR:-$HOME/.claude}\" -maxdepth 1 -type f -name 'CLAUDE-*.md' -print 2>/dev/null");
    });
    ...
    it('hud skill should normalize statusLine command paths to forward slashes', () => {
      ...
      expect(content).not.toContain('"command": "node C:\\Users\\username\\.claude\\hud\\omg-hud.mjs"');
    });
    ...
    it('usage-api should use path.join with separate segments', () => {
      ...
      expect(content).toContain("'plugins', 'oh-my-grok', `.usage-cache-${source}.json`");
    });
  });
});
