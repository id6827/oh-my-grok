...
// (it always resolves the path relative to its own __dirname, ignoring GROK_CONFIG_DIR).
...
describe('HUD marketplace resolution', () => {
  it('omg-hud.mjs converts absolute HUD paths to file URLs before dynamic imports', () => {
    ...
    expect(existsSync(join(configDir, '.omg-config.json'))).toBe(true);
  ...
  it('omg-hud.mjs surfaces dynamic import errors from OMC_PLUGIN_ROOT HUD paths', () => {
    ...
    expect(normalized).toContain('[OMG HUD] HUD import failed from');
  ...
  it('omg-hud.mjs loads a global npm install outside a Node project via npm prefix resolution', () => {
    ...
    const npmPackageRoot = join(npmRoot, 'oh-my-grok');
  ...
  it('omg-hud.mjs loads the published npm package name before the branded fallback', () => {
    ...
    const npmPackageRoot = join(configDir, 'node_modules', 'oh-my-grok');
    const npmHudDir = join(npmPackageRoot, 'dist', 'hud');
    mkdirSync(npmHudDir, { recursive: true });
    writeFileSync(join(npmPackageRoot, 'package.json'), '{"type":"module"}\n');
    writeFileSync(
