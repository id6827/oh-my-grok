...
function writeCompletePluginPayload(root: string): void {
  ...
  writePluginFile(
    join(root, 'commands', 'omg-setup.md'),
    'Read skills/omg-setup/SKILL.md and pass $ARGUMENTS.\n',
  );
  writePluginFile(
...
describe('installer legacy agent sync gating (issue #1502)', () => {
  ...
  it('skips recreating ~/.grok/agents when installed plugin agent files already exist', async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify(
        {
          plugins: {
            'oh-my-grok@omg': [{ installPath: pluginInstallPath }],
          },
        },
        null,
        2,
