...
describe('Installer Constants', () => {
    ...
    it('should not have any commands/*.md files that redirect to their own skill name', () => {
      ...
      for (const file of files) {
        ...
        const skillInvokePattern = new RegExp(
          `/oh-my-grok:${commandName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`,
          'i'
        );
  ...
  describe('VERSION', () => {
    ...
    it('should keep docs/CLAUDE.md version marker in sync with package version', () => {
      const versionMatch = CLAUDE_MD_CONTENT.match(/<!-- OMG:VERSION:([^\s]*?) -->/);
  ...
  describe('extractOmcVersionFromClaudeMd()', () => {
    it('prefers the OMG version marker', () => {
      const content = `<!-- OMG:VERSION:4.7.7 -->
# oh-my-grok - Intelligent Multi-Agent Orchestration`;
    ...
    it('falls back to legacy heading versions', () => {
      const content = '# oh-my-grok v4.6.0 - Intelligent Multi-Agent Orchestration';
  ...
  describe('File Paths', () => {
    it('should define valid directory paths', () => {
      expect(AGENTS_DIR).toBe(join(GROK_CONFIG_DIR, 'agents'));
      expect(COMMANDS_DIR).toBe(join(GROK_CONFIG_DIR, 'commands'));
      expect(SKILLS_DIR).toBe(join(GROK_CONFIG_DIR, 'skills'));
      expect(HOOKS_DIR).toBe(join(GROK_CONFIG_DIR, 'hooks'));
    });
  ...
  describe('Plugin Detection', () => {
    ...
    beforeEach(() => {
      ...
      originalEnv = process.env.GROK_PLUGIN_ROOT;
    });

    afterEach(() => {
      // Restore original env var
      if (originalEnv !== undefined) {
        process.env.GROK_PLUGIN_ROOT = originalEnv;
      } else {
        delete process.env.GROK_PLUGIN_ROOT;
      }
    });

    it('should return false when GROK_PLUGIN_ROOT is not set', () => {
      delete process.env.GROK_PLUGIN_ROOT;
    ...
    it('should detect plugin context from environment variable', () => {
      process.env.GROK_PLUGIN_ROOT = '/any/path';
      expect(isRunningAsPlugin()).toBe(true);
    });
  });
  ...
  describe('Project-Scoped Plugin Detection', () => {
    ...
    it('should return false for global plugin installation', () => {
      // Global plugins are under ~/.grok/plugins/
      process.env.GROK_PLUGIN_ROOT = join(GROK_CONFIG_DIR, 'plugins', 'cache', 'omg', 'oh-my-grok', '3.9.0');
      expect(isProjectScopedPlugin()).toBe(false);
    });
    ...
    it('should handle trailing slashes in paths', () => {
      process.env.GROK_PLUGIN_ROOT = join(GROK_CONFIG_DIR, 'plugins', 'cache', 'omg') + '/';
      expect(isProjectScopedPlugin()).toBe(false);
    });
  });
