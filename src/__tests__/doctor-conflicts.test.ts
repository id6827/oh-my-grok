...
function resetTestDirs(): void {
  TEST_CLAUDE_DIR = mkdtempSync(join(tmpdir(), 'omg-doctor-conflicts-claude-'));
...
function writeCanonicalOmcReferenceSkill(content = '# Canonical omg-reference skill\n'): string {
  const skillPath = join(TEST_DIRS.builtinSkillsDir, 'omg-reference', 'SKILL.md');
  mkdirSync(join(TEST_DIRS.builtinSkillsDir, 'omg-reference'), { recursive: true });
...
function writePluginRoot(root: string, content: string): void {
  ...
  mkdirSync(join(root, 'skills', 'omg-reference'), { recursive: true });
  writeFileSync(join(root, 'docs', 'CLAUDE.md'), '<!-- OMG:START -->\n# OMG\n<!-- OMG:END -->\n');
  writeFileSync(join(root, 'skills', 'omg-reference', 'SKILL.md'), content);
}
...
vi.mock('../features/builtin-skills/skills.js', () => ({
  ...
  listBuiltinSkillNames: ({ includeAliases }: { includeAliases?: boolean } = {}) => {
    const names = ['autopilot', 'ralph', 'ultrawork', 'plan', 'team', 'cancel', 'note', 'omg-reference'];
    if (includeAliases) {
      return [...names, 'psm'];
    }
    return names;
...
describe('doctor-conflicts: hook ownership classification', () => {
  ...
  beforeEach(() => {
    ...
    process.env.GROK_CONFIG_DIR = TEST_CLAUDE_DIR;
    process.env.CLAUDE_MCP_CONFIG_PATH = join(TEST_CLAUDE_DIR, '..', '.claude.json');
    process.env.OMC_HOME = join(TEST_PROJECT_DIR, '.omg-home');
  ...
  it('warns on native Windows when a plugin cache hooks manifest still contains sh/find-node commands', () => {
    const pluginRoot = mkdtempSync(join(tmpdir(), 'omg-doctor-win-plugin-'));
      ...
      writeFileSync(join(pluginRoot, 'hooks', 'hooks.json'), JSON.stringify({
        hooks: {
          Stop: [{
            hooks: [{
              type: 'command',
              command: 'sh "$GROK_PLUGIN_ROOT"/scripts/find-node.sh "$GROK_PLUGIN_ROOT"/scripts/run.cjs "$GROK_PLUGIN_ROOT"/scripts/persistent-mode.mjs',
            }],
          }],
          SessionEnd: [{
            hooks: [{
              type: 'command',
              command: 'node "$GROK_PLUGIN_ROOT"/scripts/run.cjs "$GROK_PLUGIN_ROOT"/scripts/session-end.mjs',
            }],
          }],
        },
      }));
      process.env.GROK_PLUGIN_ROOT = pluginRoot;
  ...
  it('warns on native Windows for stale installed plugin manifest even when settings hooks are clean', () => {
    const pluginRoot = mkdtempSync(join(tmpdir(), 'omg-doctor-win-installed-plugin-'));
    const originalPlatform = Object.getOwnPropertyDescriptor(process, 'platform');
    ...
    try {
      ...
      writeFileSync(join(pluginRoot, 'hooks', 'hooks.json'), JSON.stringify({
        hooks: {
          PostToolUse: [{
            hooks: [{
              type: 'command',
              command: 'sh "$GROK_PLUGIN_ROOT"/scripts/find-node.sh "$GROK_PLUGIN_ROOT"/scripts/run.cjs "$GROK_PLUGIN_ROOT"/scripts/post-tool-verifier.mjs',
            }],
          }],
        },
      }));
      ...
      writeFileSync(join(TEST_CLAUDE_DIR, 'plugins', 'installed_plugins.json'), JSON.stringify({
        plugins: {
          'oh-my-grok': [{ installPath: pluginRoot }],
        },
      }));
      Object.defineProperty(process, 'platform', { value: 'win32', configurable: true });
  ...
  it('does not warn on native Windows when plugin hooks already use direct node run.cjs commands', () => {
    const pluginRoot = mkdtempSync(join(tmpdir(), 'omg-doctor-win-plugin-clean-'));
    const originalPlatform = Object.getOwnPropertyDescriptor(process, 'platform');
      ...
      writeFileSync(join(pluginRoot, 'hooks', 'hooks.json'), JSON.stringify({
        hooks: {
          Stop: [{
            hooks: [{
              type: 'command',
              command: 'node "$GROK_PLUGIN_ROOT"/scripts/run.cjs "$GROK_PLUGIN_ROOT"/scripts/persistent-mode.mjs',
            }],
          }],
        },
      }));
...
describe('doctor-conflicts: CLAUDE.md companion file detection (issue #1101)', () => {
  ...
  it('detects OMG markers in main CLAUDE.md', () => {
    writeFileSync(join(TEST_CLAUDE_DIR, 'CLAUDE.md'), '<!-- OMG:START -->\n# OMG Config\n<!-- OMG:END -->\n');
  ...
  it('detects OMG markers in companion file when main CLAUDE.md lacks them', () => {
  ...
  it('detects companion file reference in CLAUDE.md', () => {
    writeFileSync(join(TEST_CLAUDE_DIR, 'CLAUDE.md'), '@CLAUDE-omg.md\n');
    const status = checkClaudeMdStatus();
    expect(status).not.toBeNull();
    expect(status!.hasMarkers).toBe(false);
  ...
  it('inspects an orphan active companion when main CLAUDE.md is absent', () => {
    const activePath = join(TEST_CLAUDE_DIR, 'CLAUDE-omg.md');
  ...
  it('aggregates main, active, referenced, and generic companions in deterministic order', () => {
    ...
    writeFileSync(activePath, '<!-- OMG:START -->\nactive\n<!-- OMG:END -->\n');
    writeFileSync(referencedPath, '<!-- OMG:START -->\nreferenced\n<!-- OMG:END -->\n');
  ...
  it('includes aggregated analyzer findings in JSON and formatted reports', () => {
    const mainPath = join(TEST_CLAUDE_DIR, 'CLAUDE.md');
    writeFileSync(mainPath, '<!-- OMG:END -->\n');
    const report = runConflictCheck();
...
describe('doctor-conflicts: legacy skills collision check (issue #1101)', () => {
  ...
  it('does NOT flag setup-installed omg-reference fallback when it matches the bundled skill (issue #2992)', () => {
    ...
    const skillsDir = join(TEST_CLAUDE_DIR, 'skills');
    mkdirSync(join(skillsDir, 'omg-reference'), { recursive: true });
    writeFileSync(join(skillsDir, 'omg-reference', 'SKILL.md'), canonicalContent);

    const collisions = checkLegacySkills();
    expect(collisions).toHaveLength(0);
  });

  it('does NOT flag setup-installed omg-reference fallback when setup resolved a newer active cache root (issue #2992)', () => {
    const oldContent = '# Old omg-reference skill\n';
    const newerContent = '# Newer setup-installed omg-reference skill\n';
    const cacheBase = join(TEST_PROJECT_DIR, 'plugin-cache', 'oh-my-grok');
    ...
    mkdirSync(join(TEST_CLAUDE_DIR, 'plugins'), { recursive: true });
    writeFileSync(join(TEST_CLAUDE_DIR, 'plugins', 'installed_plugins.json'), JSON.stringify({
      'oh-my-grok@omg': [{ installPath: oldPluginRoot, version: '4.8.2' }],
    }));
    const skillsDir = join(TEST_CLAUDE_DIR, 'skills');
    mkdirSync(join(skillsDir, 'omg-reference'), { recursive: true });
    writeFileSync(join(skillsDir, 'omg-reference', 'SKILL.md'), newerContent);

    const collisions = checkLegacySkills();
    expect(collisions).toHaveLength(0);
  });

  it('does NOT flag setup-installed omg-reference fallback when it matches GROK_PLUGIN_ROOT (issue #2992)', () => {
    const currentContent = '# Current omg-reference skill\n';
    const sessionContent = '# Session root omg-reference skill\n';
    ...
    process.env.GROK_PLUGIN_ROOT = sessionPluginRoot;
    const skillsDir = join(TEST_CLAUDE_DIR, 'skills');
    mkdirSync(join(skillsDir, 'omg-reference'), { recursive: true });
    writeFileSync(join(skillsDir, 'omg-reference', 'SKILL.md'), sessionContent);

    const collisions = checkLegacySkills();
    expect(collisions).toHaveLength(0);
  });

  it('flags user-modified omg-reference fallback content as a real collision (issue #2992)', () => {
    writeCanonicalOmcReferenceSkill('# Canonical omg-reference skill\n');
    const skillsDir = join(TEST_CLAUDE_DIR, 'skills');
    mkdirSync(join(skillsDir, 'omg-reference'), { recursive: true });
    writeFileSync(join(skillsDir, 'omg-reference', 'SKILL.md'), '# Modified omg-reference skill\n');

    const collisions = checkLegacySkills();
    expect(collisions).toHaveLength(1);
    expect(collisions[0].name).toBe('omg-reference');
  });

  it('still flags non-contract omg-reference.md legacy files (issue #2992)', () => {
    writeCanonicalOmcReferenceSkill();
    const skillsDir = join(TEST_CLAUDE_DIR, 'skills');
    mkdirSync(skillsDir, { recursive: true });
    writeFileSync(join(skillsDir, 'omg-reference.md'), '# Legacy omg-reference markdown file\n');
  ...
  it('reports no conflicts for the setup-installed omg-reference fallback (issue #2992)', () => {
    const canonicalContent = writeCanonicalOmcReferenceSkill();
    const skillsDir = join(TEST_CLAUDE_DIR, 'skills');
    mkdirSync(join(skillsDir, 'omg-reference'), { recursive: true });
    writeFileSync(join(skillsDir, 'omg-reference', 'SKILL.md'), canonicalContent);
...
describe('doctor-conflicts: config known fields (issue #1499)', () => {
  ...
  it('does not flag legitimate config keys from current writers and readers', () => {
    writeFileSync(join(TEST_CLAUDE_DIR, '.omg-config.json'), JSON.stringify({
...
describe('doctor-conflicts: workspace marker check (Wave F.2)', () => {
  ...
  beforeEach(() => {
    ...
    process.env.CLAUDE_MCP_CONFIG_PATH = join(TEST_CLAUDE_DIR, '..', '.claude.json');
    cwdSpy = vi.spyOn(process, 'cwd').mockReturnValue(TEST_PROJECT_DIR);
    savedOmcStateDir = process.env.OMG_STATE_DIR;
    delete process.env.OMG_STATE_DIR;
    tempDir = mkdtempSync(join(tmpdir(), 'omg-ws-marker-test-'));
  });
  ...
  afterEach(() => {
    ...
    if (savedOmcStateDir === undefined) {
      ...
      process.env.OMG_STATE_DIR = savedOmcStateDir;
    }
    for (const dir of [TEST_CLAUDE_DIR, TEST_PROJECT_DIR]) {
      if (dir && existsSync(dir)) {
        rmSync(dir, { recursive: true, force: true });
  ...
  it('reports markerRoot null when no .omg-workspace marker exists', () => {
  ...
  it('reports markerRoot when .omg-workspace marker is present', () => {
    writeFileSync(join(tempDir, '.omg-workspace'), '{}');
    cwdSpy.mockReturnValue(tempDir);
    const status = checkWorkspaceMarker();
  ...
  it('reports stateDirEnvSet when OMG_STATE_DIR is set', () => {
    process.env.OMG_STATE_DIR = '/some/centralized/state';
    cwdSpy.mockReturnValue(tempDir);
    const status = checkWorkspaceMarker();
  ...
  it('emits precedenceConflict when both OMG_STATE_DIR and .omg-workspace are active', () => {
    writeFileSync(join(tempDir, '.omg-workspace'), '{}');
    process.env.OMG_STATE_DIR = '/centralized/override';
    cwdSpy.mockReturnValue(tempDir);
    const status = checkWorkspaceMarker();
    expect(status.markerRoot).toBe(tempDir);
    expect(status.stateDirEnvSet).toBe(true);
