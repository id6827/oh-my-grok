...
import { install, isProjectScopedPlugin, checkNodeVersion, GROK_CONFIG_DIR } from '../installer/index.js';
import {
...
describe('auto-update reconciliation', () => {
  beforeEach(() => {
    ...
    mockedReaddirSync.mockImplementation((path: Parameters<typeof readdirSync>[0], options?: Parameters<typeof readdirSync>[1]) => {
      ...
      if (normalized.endsWith('/commands')) {
        return options && typeof options === 'object' && 'withFileTypes' in options
          ? [{ name: 'omg-setup.md', isFile: () => true, isDirectory: () => false }] as any
          : ['omg-setup.md'] as any;
      }
    ...
    mockedReadFileSync.mockImplementation((path: Parameters<typeof readFileSync>[0]) => {
      ...
      if (normalized.endsWith('/.claude-plugin/plugin.json')) {
        return JSON.stringify({ name: 'oh-my-grok', commands: './commands/', skills: ['./skills/plan/'] });
      }
  ...
  it('syncs active plugin cache roots and logs when copy occurs', () => {
    ...
    const activeRoot = join(GROK_CONFIG_DIR, 'plugins', 'cache', 'omg', 'oh-my-grok', '4.1.5');
    ...
    mockedReadFileSync.mockImplementation((path: Parameters<typeof readFileSync>[0]) => {
      ...
      if (normalized.endsWith('/plugins/installed_plugins.json')) {
        return JSON.stringify({
          plugins: {
            'oh-my-grok': [{ installPath: activeRoot }],
          },
        });
      }
      return '';
  ...
  it('fails reconciliation when active plugin cache repair reports validation errors', () => {
    const activeRoot = join(GROK_CONFIG_DIR, 'plugins', 'cache', 'omg', 'oh-my-grok', '4.14.1');
  ...
  it('updates installed_plugins.json to the new cache version after plugin cache sync succeeds', () => {
    const cacheRoot = join(GROK_CONFIG_DIR, 'plugins', 'cache', 'omg', 'oh-my-grok');
    ...
    mockedReadFileSync.mockImplementation((path: Parameters<typeof readFileSync>[0]) => {
      ...
      if (normalized === '/usr/lib/node_modules/oh-my-grok/package.json') {
      ...
      if (normalized.endsWith('/plugins/installed_plugins.json')) {
        return JSON.stringify({
          version: 2,
          plugins: {
            'oh-my-grok@omg': [{ installPath: oldRoot, version: '4.14.0', enabled: true }],
          },
        });
      }
      return '';
    ...
    mockedExistsSync.mockImplementation((path: Parameters<typeof existsSync>[0]) => {
      const normalized = String(path).replace(/\\/g, '/');
      return normalized.endsWith('/plugins/cache/omg/oh-my-grok')
        || normalized.endsWith('/plugins/installed_plugins.json')
        || normalized.startsWith(join(cacheRoot, '4.14.1').replace(/\\/g, '/'))
        || normalized.startsWith('/usr/lib/node_modules/oh-my-grok');
    });
  ...
  it('preserves Windows-style installPath separators when rewriting installed_plugins.json', () => {
    const cacheRoot = join(GROK_CONFIG_DIR, 'plugins', 'cache', 'omg', 'oh-my-grok');
    const oldWindowsRoot = 'C:\\Users\\bellman\\.claude\\plugins\\cache\\omg\\oh-my-grok\\4.14.0';
    const newWindowsRoot = 'C:\\Users\\bellman\\.claude\\plugins\\cache\\omg\\oh-my-grok\\4.14.1';
    ...
    mockedReadFileSync.mockImplementation((path: Parameters<typeof readFileSync>[0]) => {
      ...
      if (normalized === 'C:/Users/bellman/AppData/Roaming/npm/node_modules/oh-my-grok/package.json') {
        return JSON.stringify({ version: '4.14.1' });
      }
      if (normalized.endsWith('/plugins/installed_plugins.json')) {
        return JSON.stringify({
          version: 2,
          plugins: {
            'oh-my-grok@omg': [{ installPath: oldWindowsRoot, version: '4.14.0' }],
          },
        });
      }
      return '';
    ...
    mockedExistsSync.mockImplementation((path: Parameters<typeof existsSync>[0]) => {
      ...
      return normalized === cacheRoot.replace(/\\/g, '/')
        || normalized.endsWith('/plugins/installed_plugins.json')
        || normalized.startsWith(join(cacheRoot, '4.14.1').replace(/\\/g, '/'))
        || normalized.startsWith('C:/Users/bellman/AppData/Roaming/npm/node_modules/oh-my-grok');
    });
    ...
    expect(result.errors).toEqual([]);
    const written = String(mockedWriteFileSync.mock.calls.find(([path]) => String(path).includes('installed_plugins.json.tmp-'))?.[1]);
    expect(JSON.parse(written).plugins['oh-my-grok@omg'][0]).toMatchObject({
      version: '4.14.1',
    ...
    mockedReadFileSync.mockImplementation((path: Parameters<typeof readFileSync>[0]) => {
      ...
      if (normalized.endsWith('/plugins/installed_plugins.json')) {
        return JSON.stringify({
          version: 2,
          plugins: {
            'oh-my-grok@omg': [{ installPath: join(cacheRoot, '4.14.0'), version: '4.14.0' }],
          },
        });
      }
      return '';
  ...
  it('syncs the plugin cache directory when cache root exists', () => {
    ...
    mockedExistsSync.mockImplementation((path: Parameters<typeof existsSync>[0]) => {
      ...
      if (normalized.startsWith('/usr/lib/node_modules/oh-my-grok/')) {
        return normalized.endsWith('/dist')
          ...
          || normalized.endsWith('/commands/omg-setup.md')
    ...
    expect(mockedCpSync).toHaveBeenCalledWith(
      '/usr/lib/node_modules/oh-my-grok/dist',
      `${versionedCacheRoot}/dist`,
      expect.objectContaining({ recursive: true, force: true }),
    );
    expect(mockedCpSync).toHaveBeenCalledWith(
      '/usr/lib/node_modules/oh-my-grok/package.json',
      `${versionedCacheRoot}/package.json`,
      expect.objectContaining({ recursive: true, force: true }),
    );
    expect(mockedCpSync).toHaveBeenCalledWith(
      '/usr/lib/node_modules/oh-my-grok/bridge',
  ...
  it('handles plugin cache sync errors non-fatally', () => {
    ...
    expect(consoleWarnSpy).toHaveBeenCalledWith(
      `[omg update] Plugin cache sync warning: Failed to sync dist to ${versionedCacheRoot}: copy failed`,
    );
  });
  ...
  it('only blocks standalone update inside an active plugin session', () => {
    ...
    process.env.GROK_PLUGIN_ROOT = '/tmp/.claude/plugins/cache/omg/oh-my-grok/4.1.5';
    expect(shouldBlockStandaloneUpdateInCurrentSession()).toBe(false);
  ...
  it('dedupes plugin roots and ignores missing targets during sync', () => {
    const consoleLogSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
    const activeRoot = join(GROK_CONFIG_DIR, 'plugins', 'cache', 'omg', 'oh-my-grok', '4.1.5');
    const staleRoot = join(GROK_CONFIG_DIR, 'plugins', 'cache', 'omg', 'oh-my-grok', '4.1.4');
    process.env.GROK_PLUGIN_ROOT = activeRoot;
  ...
  it('allows standalone update when GROK_PLUGIN_ROOT is inherited without an active Claude session', async () => {
    const pluginRoot = join(GROK_CONFIG_DIR, 'plugins', 'cache', 'omg', 'oh-my-grok', '4.1.5');
    const cacheRoot = join(GROK_CONFIG_DIR, 'plugins', 'cache', 'omg', 'oh-my-grok');
    ...
    mockedExecSync.mockImplementation((command: string) => {
      if (command === 'npm install -g oh-my-grok@latest') {
        return '';
      }
      if (command === 'npm root -g') {
        return '/usr/lib/node_modules\n';
    ...
    expect(result.success).toBe(true);
    expect(mockedExecSync).toHaveBeenCalledWith('npm install -g oh-my-grok@latest', expect.any(Object));
  });
  ...
  it('restores global Claude Code when npm removes an existing global install during update', async () => {
    ...
    mockedExistsSync.mockImplementation((path: Parameters<typeof existsSync>[0]) => {
      ...
      if (normalized.endsWith('/plugins/cache/omg/oh-my-grok')) {
        return false;
      }
      return true;
    });
    ...
    try {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith('[omg update] Restoring global @anthropic-ai/claude-code@1.2.3 after npm update...');
      expect(consoleLogSpy).toHaveBeenCalledWith('[omg update] Restored global @anthropic-ai/claude-code');
    } finally {
  ...
  it('runs reconciliation as part of performUpdate without plugin hook reinjection', async () => {
    ...
    process.env.OMC_UPDATE_RECONCILE = '1';
    process.env.GROK_PLUGIN_ROOT = join(
      GROK_CONFIG_DIR,
      'plugins',
      'cache',
      'omg',
  ...
  it('skips marketplace auto-sync when the marketplace clone has local modifications', async () => {
    ...
    expect(mockedExecFileSync).toHaveBeenCalledWith(
      'git',
      ['-C', expect.stringContaining('/plugins/marketplaces/omg'), 'status', '--porcelain', '--untracked-files=normal'],
      expect.any(Object)
    );
  ...
  it('skips marketplace auto-sync when the marketplace clone has local commits', async () => {
    ...
    expect(mockedExecFileSync).toHaveBeenCalledWith(
      'git',
      ['-C', expect.stringContaining('/plugins/marketplaces/omg'), 'rev-list', '--left-right', '--count', 'HEAD...origin/main'],
      expect.any(Object)
    );
    expect(mockedExecFileSync).not.toHaveBeenCalledWith(
      'git',
  ...
  it('fast-forwards a clean marketplace clone when origin/main is ahead', async () => {
    ...
    expect(mockedExecFileSync).toHaveBeenCalledWith(
      'git',
      ['-C', expect.stringContaining('/plugins/marketplaces/omg'), 'merge', '--ff-only', 'origin/main'],
      expect.any(Object)
    );
    expect(mockedExecFileSync).not.toHaveBeenCalledWith(
      'git',
  ...
  it('re-execs with omg.cmd on Windows and persists metadata after reconciliation', async () => {
    ...
    expect(result.success).toBe(true);
    expect(mockedExecSync).toHaveBeenCalledWith('npm install -g oh-my-grok@latest', expect.objectContaining({
      windowsHide: true,
    }));
