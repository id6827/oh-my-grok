 ...
 *
 * The script's cleanup block scans ~/.grok/plugins/cache/omg/oh-my-grok/
 ...
 * "Cannot find module" errors when a running session's GROK_PLUGIN_ROOT
...
describe('session-start.mjs — plugin cache cleanup uses symlinks', () => {
  ...
  beforeEach(() => {
    ...
    fakeCacheBase = join(fakeHome, '.claude', 'plugins', 'cache', 'omg', 'oh-my-grok');
    ...
    try {
      const result = execFileSync(NODE, [SCRIPT_PATH], {
        ...
        env: {
          ...
          GROK_CONFIG_DIR: join(fakeHome, '.claude'), // Override to use fake home
          GROK_PLUGIN_ROOT: join(fakeCacheBase, '4.4.3'),
          ...env,
        },
        timeout: 15000,
      });
  ...
  it('keeps explicit external plugin roots authoritative for update checks', () => {
    ...
    const output = runSessionStart({ GROK_PLUGIN_ROOT: externalRoot });
    const parsed = JSON.parse(output);

    expect(parsed.systemMessage).toContain('[OMG UPDATE AVAILABLE]');
  ...
  it('uses latest managed cache version for stale managed cache roots', () => {
    ...
    const output = runSessionStart({ GROK_PLUGIN_ROOT: join(fakeCacheBase, '4.14.4') });
    const parsed = JSON.parse(output);
