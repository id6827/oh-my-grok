...
- doctor -> /oh-my-grok:omg-doctor with remaining args
- mcp -> /oh-my-grok:mcp-setup with remaining args
- otherwise -> /oh-my-grok:omg-setup with remaining args`
    );
    ...
    expect(result.replacementText).toContain('doctor -> /oh-my-grok:omg-doctor with remaining args');
...
Compatibility body`
  ...
  it('discovers workspace-local Claude Code skills from .claude/skills before OMG compatibility skills', async () => {
...
Deep interview body`
  ...
  it('routes /ccg advisor asks through the plugin bridge inside an active Claude session when GROK_PLUGIN_ROOT is set', async () => {
    ...
    expect(result.success).toBe(true);
    expect(result.replacementText).toContain('`node "$GROK_PLUGIN_ROOT"/bridge/cli.cjs ask codex "<codex prompt>"`');
    expect(result.replacementText).toContain('`node "$GROK_PLUGIN_ROOT"/bridge/cli.cjs ask antigravity "<antigravity prompt>"`');
    expect(result.replacementText).not.toContain('`omg ask codex "<codex prompt>"`');
    expect(result.replacementText).not.toContain('`omg ask antigravity "<antigravity prompt>"`');
  });
});
