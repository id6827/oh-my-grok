...
describe('resolveTranscriptPath', () => {
  ...
  it('resolves worktree-encoded transcript path to original project path', () => {
    // Simulate: ~/.grok/projects/-Users-user-project/<session>.jsonl (real)
    ...
    // ~/.grok/projects/-Users-user-project--claude-worktrees-refactor/<session>.jsonl
  ...
  it('resolves worktree paths with complex worktree names', () => {
    ...
    const worktreePath = join(
      tempDir,
      'projects',
      '-home-bellman-Workspace-myproject--claude-worktrees-home-bellman-Workspace-omg-worktrees-fix-issue-1094',
      'session-uuid.jsonl',
    );
  ...
  it('resolves via a .claude/worktrees CWD using the OS-native separator (Strategy 2)', () => {
    ...
    const origClaudeConfigDir = process.env.GROK_CONFIG_DIR;
    const fakeClaudeDir = join(tempDir, 'fake-claude');
    process.env.GROK_CONFIG_DIR = fakeClaudeDir;
    try {
      ...
      if (origClaudeConfigDir === undefined) {
        ...
        process.env.GROK_CONFIG_DIR = origClaudeConfigDir;
      }
    }
  });
  ...
  describe('native git worktree fallback', () => {
    ...
    beforeEach(() => {
      // Save and override GROK_CONFIG_DIR so Strategy 3 finds our fake projects dir
      origClaudeConfigDir = process.env.GROK_CONFIG_DIR;
      ...
      // Simulate ~/.grok/projects/ with a transcript at the main repo's encoded path
