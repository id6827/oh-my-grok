...
describe("loadConfig() — auto-forceInherit for non-standard providers", () => {
  ...
  it("does NOT auto-enable forceInherit for non-Claude OMG tier env vars", () => {
  ...
  it("does NOT auto-enable forceInherit when direct Claude CLAUDE_MODEL beats stale OMG tier env vars", () => {
    process.env.CLAUDE_MODEL = "claude-sonnet-5";
    process.env.OMC_MODEL_MEDIUM = "glm-5.1:cloud";
    const config = loadConfig();
    expect(config.routing?.forceInherit).toBe(false);
  ...
  it("does NOT auto-enable forceInherit when direct Claude ANTHROPIC_MODEL beats stale OMG tier env vars", () => {
...
describe("startup context compaction", () => {
  it("compacts only OMG-style guidance in loadContextFromFiles while preserving key sections", () => {
    ...
    try {
      ...
      const omcGuidance = `# oh-my-grok - Intelligent Multi-Agent Orchestration
...
- verify this stays
  ...
  it("caps aggregated context across multiple files", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "omg-loader-context-aggregate-"));
    ...
    try {
      ...
      const largeSection = `# oh-my-grok - Intelligent Multi-Agent Orchestration
...
- verify
  ...
  it("caps very large OMG guidance after preserving high-value sections", () => {
    const largeOmc = `# oh-my-grok - Intelligent Multi-Agent Orchestration
...
- verify this stays before truncation
    ...
    expect(compacted).toContain("OMG startup guidance truncated");
  });
...
describe("company context configuration", () => {
  ...
  it("loads company context overrides from project config", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "omg-company-context-"));
...
describe("team.roleRouting (Option E)", () => {
  ...
  it("merges per-role file overrides into team.roleRouting", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "omg-team-routing-"));
    try {
      const claudeDir = join(tempDir, ".claude");
      require("node:fs").mkdirSync(claudeDir, { recursive: true });
      writeFileSync(
  ...
  it("accepts cursor as team defaultAgentType and executor roleRouting provider", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "omg-team-routing-cursor-"));
    try {
      const claudeDir = join(tempDir, ".claude");
      require("node:fs").mkdirSync(claudeDir, { recursive: true });
      writeFileSync(
  ...
  it("rejects cursor for non-executor team roleRouting providers", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "omg-team-routing-cursor-reviewer-"));
    try {
      const claudeDir = join(tempDir, ".claude");
      require("node:fs").mkdirSync(claudeDir, { recursive: true });
      writeFileSync(
  ...
  it("OMC_TEAM_ROLE_OVERRIDES env wins over file config", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "omg-team-routing-env-"));
    try {
      const claudeDir = join(tempDir, ".claude");
      require("node:fs").mkdirSync(claudeDir, { recursive: true });
      writeFileSync(
  ...
  it("rejects invalid provider value with descriptive error", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "omg-team-bad-provider-"));
    try {
      const claudeDir = join(tempDir, ".claude");
      require("node:fs").mkdirSync(claudeDir, { recursive: true });
      writeFileSync(
  ...
  it("rejects orchestrator.provider override (orchestrator is pinned to claude)", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "omg-team-orch-pin-"));
    try {
      const claudeDir = join(tempDir, ".claude");
      require("node:fs").mkdirSync(claudeDir, { recursive: true });
      writeFileSync(
  ...
  it("rejects unknown agent name", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "omg-team-bad-agent-"));
    try {
      const claudeDir = join(tempDir, ".claude");
      require("node:fs").mkdirSync(claudeDir, { recursive: true });
      writeFileSync(
  ...
  it("accepts 'reviewer' alias and preserves the raw key for later alias-aware resolution", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "omg-team-alias-"));
    try {
      const claudeDir = join(tempDir, ".claude");
      require("node:fs").mkdirSync(claudeDir, { recursive: true });
      writeFileSync(
  ...
  it("rejects unsupported team.ops.defaultAgentType values", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "omg-team-default-agent-type-"));
    try {
      const claudeDir = join(tempDir, ".claude");
      require("node:fs").mkdirSync(claudeDir, { recursive: true });
      writeFileSync(
  ...
  it("rejects unknown role with descriptive error", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "omg-team-bad-role-"));
    try {
      const claudeDir = join(tempDir, ".claude");
      require("node:fs").mkdirSync(claudeDir, { recursive: true });
      writeFileSync(
...
describe("delegation routing deprecation warnings", () => {
  ...
  it("warns when project config uses deprecated delegation role provider", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "omg-delegation-routing-warning-"));
...
describe("loadConfig() — autopilot team worker config", () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), "omg-autopilot-config-"));
    process.chdir(tempDir);
  });
...
describe("loadConfig() — autopilot.workflows", () => {
  ...
  const writeProjectConfig = (content: string) => {
    require("node:fs").mkdirSync(join(tempDir, ".claude"), { recursive: true });
    writeFileSync(join(tempDir, ".claude", "omg.jsonc"), content);
  };

  const writeUserConfig = (content: string) => {
    const path = join(configHome, "claude-omg");
    require("node:fs").mkdirSync(path, { recursive: true });
    writeFileSync(join(path, "config.jsonc"), content);
  };

  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), "omg-workflow-config-"));
