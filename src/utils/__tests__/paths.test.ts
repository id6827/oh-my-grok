...
describe('cross-platform path utilities', () => {
  ...
  describe('global OMG path helpers', () => {
    ...
    it('should use XDG config root for global OMG config on Linux', () => {
      ...
      expect(getGlobalOmcConfigRoot()).toBe('/custom/config/omg');
      expect(getGlobalOmcConfigPath('config.json')).toBe('/custom/config/omg/config.json');
    });
    ...
    it('should use XDG state root for global OMG state on Linux', () => {
      ...
      expect(getGlobalOmcStateRoot()).toBe('/custom/state/omg');
      expect(getGlobalOmcStatePath('daemon.json')).toBe('/custom/state/omg/daemon.json');
    });

    it('should keep OMC_HOME authoritative for config and state roots', () => {
      Object.defineProperty(process, 'platform', { value: 'linux' });
      process.env.OMC_HOME = '/override/omg';
      process.env.XDG_CONFIG_HOME = '/custom/config';
      process.env.XDG_STATE_HOME = '/custom/state';

      expect(getGlobalOmcConfigRoot()).toBe('/override/omg');
      expect(getGlobalOmcStateRoot()).toBe('/override/omg/state');
    });
