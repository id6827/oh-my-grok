...
const EXTRA_STATE_ONLY_MODES = ['ralplan', 'omg-teams', 'skill-active'] as const;
...
function getLegacyStateFileCandidates(mode: StateToolMode, root: string): string[] {
  ...
  if (mode === 'autopilot') candidates.push(join(homedir(), '.omg', 'state', 'autopilot-state.json'));
...
export const stateListActiveTool: ToolDefinition<{
  ...
  description: 'List all currently active modes. By default, scopes to the current session (OMG_SESSION_ID). Pass all:true to list active modes across all sessions.',
  annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
  schema: {
    workingDirectory: z.string().optional().describe('Working directory (defaults to cwd)'),
    session_id: z.string().optional().describe('Explicit session ID to scope the listing. Overrides OMG_SESSION_ID when provided.'),
