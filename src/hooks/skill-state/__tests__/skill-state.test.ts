...
describe('skill-state', () => {
  ...
  describe('getSkillProtection', () => {
    ...
    it('strips oh-my-grok: prefix', () => {
      expect(getSkillProtection('oh-my-grok:plan')).toBe('medium');
      expect(getSkillProtection('oh-my-grok:ralph')).toBe('none');
    });
    ...
    it('returns none for project custom skills with same name as OMG skills (issue #1581)', () => {
      // rawSkillName without oh-my-grok: prefix → project custom skill
    ...
    it('returns protection for OMG skills when rawSkillName has prefix', () => {
      expect(getSkillProtection('plan', 'oh-my-grok:plan')).toBe('medium');
      expect(getSkillProtection('deepinit', 'oh-my-grok:deepinit')).toBe('heavy');
    });

    it('returns none for other plugin skills with rawSkillName', () => {
      // ouroboros:plan, claude-mem:make-plan etc. should not get OMG protection
  ...
  describe('writeSkillActiveState', () => {
    ...
    it('strips namespace prefix from skill name', () => {
      const state = writeSkillActiveState(tempDir, 'oh-my-grok:plan', 'session-1');
      expect(state!.skill_name).toBe('plan');
    });
    ...
    it('writes state for OMG skills when rawSkillName has prefix', () => {
      const state = writeSkillActiveState(tempDir, 'plan', 'session-1', 'oh-my-grok:plan');
      expect(state).not.toBeNull();
      expect(state!.skill_name).toBe('plan');
      expect(state!.max_reinforcements).toBe(5);
    });
    ...
    it('does not overwrite when mcp-setup is invoked inside omg-setup (canonical nesting scenario)', () => {
      writeSkillActiveState(tempDir, 'omg-setup', 'session-1');
      const child = writeSkillActiveState(tempDir, 'mcp-setup', 'session-1');
      expect(child).toBeNull();
      expect(readSkillActiveState(tempDir, 'session-1')!.skill_name).toBe('omg-setup');
    });
  ...
  describe('checkSkillActiveState', () => {
    ...
    it('nesting-aware clear: child completion preserves parent state, parent completion clears it', () => {
      // Simulates the full omg-setup → mcp-setup lifecycle including
      ...
      // "Verify stop hook no longer blocks after omg-setup completes with nested mcp-setup"

      // 1. Parent skill (omg-setup) starts
      writeSkillActiveState(tempDir, 'omg-setup', 'session-1');
      expect(readSkillActiveState(tempDir, 'session-1')!.skill_name).toBe('omg-setup');
      ...
      expect(parentState!.skill_name).toBe('omg-setup');
      ...
      expect(stopCheck.skillName).toBe('omg-setup');
      ...
      const completingParent = 'omg-setup';
  ...
  describe('upsertWorkflowSkillSlot — pure helper', () => {
    ...
    it('strips oh-my-grok: prefix from skill name', () => {
      const state = upsertWorkflowSkillSlot(emptySkillActiveStateV2(), 'oh-my-grok:ralph', {
        session_id: 's1',
        mode_state_path: 'r.json',
        initialized_mode: 'ralph',
        initialized_state_path: '',
      ...
      expect(state.active_skills['oh-my-grok:ralph']).toBeUndefined();
    });
