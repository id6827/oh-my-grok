 ...
 * #!/bin/bash
 * INPUT=$(cat)
 * echo "$INPUT" | node ~/.grok/omg/hook-bridge.mjs --hook=keyword-detector
 * ```
 */
...
import { formatOmcCliInvocation } from "../utils/omg-cli-rendering.js";
...
async function reconcileAbandonedSessionStarts(directory: string, currentSessionId?: string): Promise<void> {
  ...
  for (const sessionId of entries) {
    ...
    // Deliberately narrow: clear only OMG session-scoped mode/mission state.
...
function isExplicitAskSlashInvocation(promptText: string): boolean {
  return /^\s*\/(?:oh-my-grok:)?ask\s+(?:claude|codex|gemini|antigravity|agy|grok|cursor)\b/i.test(promptText);
}
...
- Prefer a tier alias: \`model: "sonnet"\`, \`model: "opus"\`, \`model: "haiku"\`, or \`model: "fable"\` (Claude Fable 5, above Opus). OMG's pre-tool enforcer resolves these to provider-safe IDs when one of these env vars is set: \`ANTHROPIC_DEFAULT_SONNET_MODEL\` (and siblings \`ANTHROPIC_DEFAULT_OPUS_MODEL\` / \`ANTHROPIC_DEFAULT_HAIKU_MODEL\` / \`ANTHROPIC_DEFAULT_FABLE_MODEL\`), \`CLAUDE_CODE_BEDROCK_SONNET_MODEL\` (and siblings \`CLAUDE_CODE_BEDROCK_OPUS_MODEL\` / \`CLAUDE_CODE_BEDROCK_HAIKU_MODEL\` / \`CLAUDE_CODE_BEDROCK_FABLE_MODEL\`), or \`OMC_SUBAGENT_MODEL\`.
...
When the session model has no \`[1m]\` suffix, omitting \`model\` is safe UNLESS a custom sub-agent definition pins a bare Anthropic model ID (e.g. \`model: claude-sonnet-4-6\` in agent frontmatter). When resolver env vars are configured, the enforcer will deny that call with tier-alias guidance; when they are absent, the call is not denied by the enforcer but will fail at the provider. Either way, custom sub-agents should pin tier aliases (not bare Anthropic IDs) in their frontmatter. Shipped OMG agents already do this and are unaffected.
 ...
 * Used to distinguish OMG built-in skills (prefixed with 'oh-my-grok:')
...
async function processPostToolUse(input: HookInput): Promise<HookOutput> {
  ...
  if (toolName === "skill") {
    ...
    // When a parent skill (e.g. omg-setup) invokes a child skill (e.g. mcp-setup),
...
// Default to false when argv is missing (library import / node -e) so importing
// hooks never starts the CLI.
function isMainModule(): boolean {
  ...
  try {
    ...
    // Bundled CJS / path normalization: match basename of bridge entry
    return /(?:^|[/\\])(?:bridge|hook-bridge)\.(?:[cm]?js|ts)$/.test(entry)
      && import.meta.url.includes("/hooks/bridge.");
  } catch {
    return false;
  }
}
