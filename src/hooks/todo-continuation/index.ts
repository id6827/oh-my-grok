...
/**
 * TERMINOLOGY:
 * - "Task" (capitalized): New Claude Code Task system (~/.grok/tasks/)
 * - "todo" (lowercase): Legacy todo system (~/.grok/todos/)
 ...
 * tool schema. The file structure ~/.grok/tasks/{sessionId}/{taskId}.json is inferred
...
export function isExplicitCancelCommand(context?: StopContext): boolean {
  ...
  if (prompt) {
    const slashCancelPattern = /^\/(?:oh-my-grok:)?cancel(?:\s+--force)?\s*$/i;
  ...
  if (toolName.includes('skill') && toolInput && typeof toolInput.skill === 'string') {
    ...
    if (skill === 'oh-my-grok:cancel' || skill.endsWith(':cancel')) {
      return true;
    }
  }
 ...
 *
 * NOTE: This path (~/.grok/tasks/{sessionId}/) is inferred from Claude Code's
