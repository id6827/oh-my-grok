...
describe('SessionEnd action runner', () => {
  it('observes a fast child exit before publishing an arm or duplicate action', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'omg-action-runner-'));
    ...
    expect(existsSync(join(directory, '.omg', 'state', 'session-end-jobs', 'runs', 'job-id', 'foreground-cleanup', '1', 'runner', 'arm.json'))).toBe(false);
  });
