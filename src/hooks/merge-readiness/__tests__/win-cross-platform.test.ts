...
function makeRepo(): string {
  const dir = mkdtempSync(join(tmpdir(), "omg-mr-win-"));
  git(["init"], dir);
...
describe("merge-readiness Windows cross-platform contract", () => {
  ...
  it("resolves .omg root with the platform-native separator under a worktree", () => {
    const omcRoot = getOmcRoot(tempDir);
    expect(isAbsolute(omcRoot)).toBe(true);
    // .omg root terminates with the native separator + ".omg" (backslash on win32,
  ...
  it("resolves the worktree root from a subdirectory (not a subdirectory itself)", () => {
    ...
    // Must climb to the git worktree root, never the nested subdir - prevents .omg
