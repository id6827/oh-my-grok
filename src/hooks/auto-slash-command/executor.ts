...
const GROK_CONFIG_DIR = getClaudeConfigDir();
...
export function discoverAllCommands(): CommandInfo[] {
  const userCommandsDir = join(GROK_CONFIG_DIR, 'commands');
  ...
  const userSkillsDir = join(GROK_CONFIG_DIR, 'skills');
  ...
  // Priority: project commands > user commands > project Claude Code skills > project OMG skills > project compatibility skills > user skills > builtin skills
  const prioritized = [
    ...projectCommands,
    ...userCommands,
    ...projectClaudeSkills,
...
export function executeSlashCommand(parsed: ParsedSlashCommand): ExecuteResult {
  ...
  if (!command) {
    return {
      success: false,
      error: `Command "/${parsed.command}" not found. Available commands are in ${GROK_CONFIG_DIR}/commands/ or .claude/commands/`,
    };
  }
