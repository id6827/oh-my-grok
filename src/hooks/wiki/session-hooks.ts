...
function pageHasCaptureKey(page: ReturnType<typeof readPage>, captureKey: string): boolean {
  return page?.content.includes(`<!-- omg-wiki-capture:${captureKey} -->`) ?? false;
}

function logHasCaptureKey(root: string, captureKey: string): boolean {
  return readLog(root)?.includes(`omg-wiki-capture:${captureKey}`) ?? false;
}
...
/**
 * Load wiki config from .omg-config.json.
 * Returns defaults if config doesn't exist or wiki section is missing.
 */
function loadWikiConfig(root: string): WikiConfig {
  try {
    const configPath = join(getOmcRoot(root), '.omg-config.json');
    // Try active Claude config too
    const activeConfigPath = join(getClaudeConfigDir(), '.omg-config.json');
    ...
    withWikiLock(intent.root, () => {
      ...
      if (!logHasCaptureKey(intent.root, captureKey)) {
        appendLogUnsafe(intent.root, {
          ...
          summary: `Auto-captured session log for ${intent.sessionId} (omg-wiki-capture:${captureKey})`,
        });
      }
