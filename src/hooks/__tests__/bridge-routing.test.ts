...
Read src/hooks/bridge.ts first.`,
    ...
    it('does not activate ultrawork state for explanatory reference follow-up prose', async () => {
      ...
      try {
        ...
        const keywordResult = await processHook('keyword-detector', {
          sessionId,
          prompt: 'OMG Ultrawork = "special ops". how much would it cost?',
          directory: tempDir,
        });
...
$ ultrawork search the codebase`,
    ...
    it.each([
      ...
      '/omg:autopilot --workflow release-flow ship the release',
      '/autopilot --workflow',
      '/autopilot --workflow=release-flow ship the release',
    ...
    it('preserves existing autopilot state for a named invocation on an unsupported runtime', async () => {
      ...
      try {
        ...
        const result = await processHook('keyword-detector', {
          sessionId,
          prompt: '/omg:autopilot --workflow release-flow ship the release',
          directory: tempDir,
        });
    ...
    it('should activate ralph and linked ultrawork when Skill tool invokes ralph', async () => {
      ...
      try {
        ...
        const input: HookInput = {
          sessionId,
          toolName: 'Skill',
          toolInput: { skill: 'oh-my-grok:ralph' },
          directory: tempDir,
        };
    ...
    it('activates ralplan state when Skill tool invokes ralplan directly', async () => {
      ...
      try {
        ...
        const result = await processHook('pre-tool-use', {
          sessionId,
          toolName: 'Skill',
          toolInput: { skill: 'oh-my-grok:ralplan' },
          directory: tempDir,
        });
    ...
    it('arms ralplan startup state and init context for explicit /ralplan invoke in UserPromptSubmit', async () => {
      ...
      try {
        ...
        const result = await processHook('keyword-detector', {
          sessionId,
          prompt: '/oh-my-grok:ralplan issue #2622',
          directory: tempDir,
        });
        ...
        expect(hookSpecificOutput.additionalContext).toContain('/oh-my-grok:ralplan issue #2622');
    ...
    it('seeds workflow slot for explicit /deep-interview slash invocation in UserPromptSubmit', async () => {
      ...
      try {
        ...
        const result = await processHook('keyword-detector', {
          sessionId,
          prompt: '/oh-my-grok:deep-interview explore auth flows',
          directory: tempDir,
        });
    ...
    it('seeds workflow slot when Skill tool invokes oh-my-grok:deep-interview', async () => {
      ...
      try {
        ...
        const result = await processHook('pre-tool-use', {
          sessionId,
          toolName: 'Skill',
          toolInput: { skill: 'oh-my-grok:deep-interview' },
          directory: tempDir,
        });
    ...
    it('seeds workflow slot when Skill tool invokes oh-my-grok:self-improve', async () => {
      ...
      try {
        ...
        const result = await processHook('pre-tool-use', {
          sessionId,
          toolName: 'Skill',
          toolInput: { skill: 'oh-my-grok:self-improve' },
          directory: tempDir,
        });
  ...
  describe('Regression #858 — snake_case fields reach handlers after normalization', () => {
    ...
    it('subagent start/stop: normalized optional fields survive routing lifecycle', async () => {
      ...
      try {
        ...
        const trackingPath = join(tempDir, '.omg', 'state', 'sessions', 'test-session-858-subagent', 'subagent-tracking-state.json');
