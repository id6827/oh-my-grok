 ...
 * GROK_PLUGIN_ROOT segments so that
...
describe('patchHooksJsonForWindows', () => {
  ...
  it('replaces sh+find-node.sh with the run.cjs wrapper for a simple script', () => {
    ...
    expect(cmd).toBe('node "$GROK_PLUGIN_ROOT"/scripts/run.cjs "$GROK_PLUGIN_ROOT"/scripts/keyword-detector.mjs');
  });

  it('replaces current portable sh+find-node+run.cjs commands with the run.cjs wrapper', () => {
    const original = makeHooksJson([
      'sh "$GROK_PLUGIN_ROOT"/scripts/find-node.sh "$GROK_PLUGIN_ROOT"/scripts/run.cjs "$GROK_PLUGIN_ROOT"/scripts/keyword-detector.mjs',
    ]);
    writeFileSync(hooksJsonPath, JSON.stringify(original, null, 2));
  ...
  it('replaces current quoted /bin/sh+find-node+run.cjs commands with the run.cjs wrapper', () => {
    const original = makeHooksJson([
      '"/bin/sh" "$GROK_PLUGIN_ROOT"/scripts/find-node.sh "$GROK_PLUGIN_ROOT"/scripts/run.cjs "$GROK_PLUGIN_ROOT"/scripts/keyword-detector.mjs',
    ]);
    writeFileSync(hooksJsonPath, JSON.stringify(original, null, 2));
  ...
  it('preserves trailing arguments for current quoted /bin/sh+find-node+run.cjs commands', () => {
    const original = makeHooksJson([
      '"/bin/sh" "$GROK_PLUGIN_ROOT"/scripts/find-node.sh "$GROK_PLUGIN_ROOT"/scripts/run.cjs "$GROK_PLUGIN_ROOT"/scripts/subagent-tracker.mjs start',
    ]);
    writeFileSync(hooksJsonPath, JSON.stringify(original, null, 2));
    ...
    const patched = JSON.parse(readFileSync(hooksJsonPath, 'utf-8'));
    const cmd = patched.hooks.UserPromptSubmit[0].hooks[0].command;
    expect(cmd).toBe('node "$GROK_PLUGIN_ROOT"/scripts/run.cjs "$GROK_PLUGIN_ROOT"/scripts/subagent-tracker.mjs start');
  });

  it('preserves trailing arguments (e.g. subagent-tracker start)', () => {
    const original = makeHooksJson([
      'sh "${GROK_PLUGIN_ROOT}/scripts/find-node.sh" "${GROK_PLUGIN_ROOT}/scripts/subagent-tracker.mjs" start',
    ]);
    writeFileSync(hooksJsonPath, JSON.stringify(original, null, 2));
  ...
  it('is idempotent — already-patched commands are not double-modified', () => {
    const already = makeHooksJson([
      'node "$GROK_PLUGIN_ROOT"/scripts/run.cjs "$GROK_PLUGIN_ROOT"/scripts/keyword-detector.mjs',
    ]);
            ...
            hooks: [
              {
                type: 'command',
                command:
                  'sh "${GROK_PLUGIN_ROOT}/scripts/find-node.sh" "${GROK_PLUGIN_ROOT}/scripts/session-start.mjs"',
              },
            ],
          },
        ],
