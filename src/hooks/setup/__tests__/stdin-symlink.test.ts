 ...
 * when OMG upgrades to a new version. Uses safe replace strategy:
...
describe('ensureStdinSymlink', () => {
  ...
  beforeEach(() => {
    ...
    pluginRoot = mkdtempSync(join(tmpdir(), 'omg-stdin-'));
    ...
    configDir = mkdtempSync(join(tmpdir(), 'omg-config-'));
  ...
  it('uses CLAUDE_CONFIG_DIR when set', () => {
    ...
    const customConfigDir = mkdtempSync(join(tmpdir(), 'omg-custom-config-'));
