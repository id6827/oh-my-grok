...
\`\`\`text
/omg-teams ${agentSpec} "<implementation task from ${planPath}>"
\`\`\`

Requested worker types: ${requested}. Keep these CLI workers executor-style only: implementation, file edits, build/test fixes, and other plan execution tasks. Keep reviewer, critic, security-review, validation verdict, and final approval roles on the native Claude/OMG reviewer agents unless this repository explicitly adds safe role support for that CLI worker type.${cursorGuidance}`;
}
...
${useCliTeamRuntime ? `1. **Launch CLI executor workers** with \`omg team\` or \`/omg-teams\` using the requested agent types.
...
Task(subagent_type="oh-my-grok:executor", model="haiku", prompt="...")

// For standard implementation (feature, multiple methods)
Task(subagent_type="oh-my-grok:executor", model="sonnet", prompt="...")

// For complex work (architecture, debugging, refactoring)
Task(subagent_type="oh-my-grok:executor", model="opus", prompt="...")
\`\`\`
