...
Summarize the failure mode only.`);
      ...
      expect(result).not.toContain('Skill: oh-my-grok:ralph');
...
Why did this happen?`);
      ...
      expect(result).not.toContain('Skill: oh-my-grok:autopilot');
    });
  ...
  describe('detectKeywordsWithType', () => {
    ...
    describe('ultrawork keyword', () => {
      ...
      it('should NOT detect explanatory comparison prose from issue #2474', () => {
        const result = detectKeywordsWithType(`🦌 DeerFlow vs ⚡ OMG Ultrawork - 완전 비교!
...
OMG Ultrawork = "특수부대 작전 반"
...
결론: "순식간에 많은 작업" → OMG Ultrawork ⚡
이런대화가 한번이라면 몇번할수있을까 오픈라우터 20달러 결제기준 api로`);
        expect(result).toEqual([]);
      });

      it('should NOT detect quoted follow-up references after a bad activation', () => {
        const result = detectKeywordsWithType('The article said "OMG Ultrawork", but why is the answer the same?');
        expect(result).toEqual([]);
      });
      ...
      it('should NOT detect single-mode explanatory definitions followed by an unrelated question', () => {
        const result = detectKeywordsWithType('OMG Ultrawork = "special ops". how much would it cost?');
        expect(result).toEqual([]);
      });

      it('should still detect explicit activation after a single-mode explanatory definition', () => {
        const result = detectKeywordsWithType(
          'OMG Ultrawork = "special ops". then use ultrawork on issue #2474 in src/hooks/keyword-detector/index.ts',
        );
...
This article argues that fake popularity signals damage trust in open source.`;
  ...
  describe('intent-pattern guards: file paths and code blocks (spec h)', () => {
    ...
    it('inline backtick `/oh-my-grok:ralph` does NOT detect ralph', () => {
      const result = detectKeywordsWithType('run `/oh-my-grok:ralph` if needed');
      expect(result.find((r) => r.type === 'ralph')).toBeUndefined();
    });
  ...
  // -------------------------------------------------------------------------
  // Unified prefix detector (spec g) — /skill, /omg:skill, /oh-my-grok:skill
  // all seed the same canonical state (T3 implementation required)
  // -------------------------------------------------------------------------

  describe('unified prefix detector: /omg: and /oh-my-grok: forms (spec g)', () => {
    it('/omg:ralph fix auth detects ralph', () => {
      const result = detectKeywordsWithType('/omg:ralph fix auth');
      expect(result.find((r) => r.type === 'ralph')).toBeDefined();
    });

    it('/oh-my-grok:ralph fix auth detects ralph', () => {
      const result = detectKeywordsWithType('/oh-my-grok:ralph fix auth');
      expect(result.find((r) => r.type === 'ralph')).toBeDefined();
    });

    it('/omg:autopilot implement feature detects autopilot', () => {
      const result = detectKeywordsWithType('/omg:autopilot implement feature');
      expect(result.find((r) => r.type === 'autopilot')).toBeDefined();
    });

    it('/omg:ultrawork search codebase detects ultrawork', () => {
      const result = detectKeywordsWithType('/omg:ultrawork search codebase');
      expect(result.find((r) => r.type === 'ultrawork')).toBeDefined();
    });
  ...
  describe('parseExplicitWorkflowSlashInvocation — parser unit tests (spec g)', () => {
    ...
    it('parses /omg:ralph and normalizes skill name', () => {
      const result = parseExplicitWorkflowSlashInvocation('/omg:ralph debug this');
      expect(result).not.toBeNull();
      expect(result!.skill).toBe('ralph');
    });

    it('parses /oh-my-grok:ralph and normalizes skill name', () => {
      const result = parseExplicitWorkflowSlashInvocation('/oh-my-grok:ralph debug this');
      expect(result).not.toBeNull();
      expect(result!.skill).toBe('ralph');
    });
    ...
    it('all three prefix forms produce the same skill name for autopilot', () => {
      ...
      const omg = parseExplicitWorkflowSlashInvocation('/omg:autopilot go');
      const full = parseExplicitWorkflowSlashInvocation('/oh-my-grok:autopilot go');
      expect(bare!.skill).toBe('autopilot');
      expect(omg!.skill).toBe('autopilot');
