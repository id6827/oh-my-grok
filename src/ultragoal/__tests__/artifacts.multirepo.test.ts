 ...
 *
 * Companion to artifacts.test.ts. Verifies that when a .omg-workspace marker
...
describe('ultragoal artifacts — multi-repo workspace anchor', () => {
  it('writes artifacts to workspace anchor .omg/ when .omg-workspace marker is in a parent dir', async () => {
    const workspaceRoot = await mkdtemp(join(tmpdir(), 'omg-multirepo-anchor-'));
    try {
      // Drop workspace marker so getOmcRoot() anchors to workspaceRoot
      writeFileSync(join(workspaceRoot, '.omg-workspace'), '{}');
  ...
  it('sibling sub-repos share one workspace .omg/ when rooted at the same .omg-workspace', async () => {
    const workspaceRoot = await mkdtemp(join(tmpdir(), 'omg-multirepo-sibling-'));
    try {
      writeFileSync(join(workspaceRoot, '.omg-workspace'), '{}');
  ...
  it('full lifecycle (start → checkpoint) resolves through workspace anchor', async () => {
    const workspaceRoot = await mkdtemp(join(tmpdir(), 'omg-multirepo-lifecycle-'));
    try {
      writeFileSync(join(workspaceRoot, '.omg-workspace'), '{}');
