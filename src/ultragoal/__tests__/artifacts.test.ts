...
async function withTempRepo<T>(run: (cwd: string) => Promise<T>): Promise<T> {
  const cwd = await mkdtemp(join(tmpdir(), 'omg-ultragoal-'));
  try {
...
describe('ultragoal artifacts', () => {
  ...
  it('reconciles completed task-scoped Claude snapshot to finish exploded aggregate ultragoal bookkeeping', async () => {
    await withTempRepo(async (cwd) => {
      const taskObjective = 'Fix the mismatch between Claude immutable completed /goal snapshots and OMG ultragoal checkpoint reconciliation.';
      await createUltragoalPlan(cwd, {
        brief: taskObjective,
  ...
  describe('multi-repo workspace anchor', () => {
    it('writes artifacts to the workspace anchor .omg/ when .omg-workspace marker exists in a parent dir', async () => {
      const workspaceRoot = await mkdtemp(join(tmpdir(), 'omg-workspace-anchor-'));
      try {
