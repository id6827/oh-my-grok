 ...
 * inside a .omg-workspace multi-repo layout, interop state must land at the
...
describe('shared-state workspace-marker path resolution', () => {
  ...
  beforeEach(() => {
    // Build fixture:
    //   A/              ← workspace root (contains .omg-workspace marker)
    //   A/sub/          ← child git repo (git init'd)
    workspaceRoot = mkdtempSync(join(tmpdir(), 'omg-workspace-'));
