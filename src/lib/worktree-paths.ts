 ...
 * the OMG anchor regardless of git status — enables multi-repo workspaces
 ...
 *
 * Resolution order in getOmcRoot(): OMG_STATE_DIR > workspace marker > git > cwd.
 */
export const WORKSPACE_MARKER = '.omg-workspace';
...
export function findWorkspaceRoot(startDir?: string): string | null {
  ...
  while (true) {
    // Stop before scanning $HOME (or above) so a stray ~/.omg-workspace does
 ...
 * outermost superproject so `.omg/` anchors to the monorepo root rather than
 * the submodule (#3349).
 *
 ...
 * literal `.omg` base; resolving such a subdir up to the repo root would
...
export function warnSiblingRetrofit(workspaceAnchor: string, sessionId?: string): void {
  ...
  process.stderr.write(
    `[omg] workspace-retrofit warning: .omg-workspace anchor found at ${workspaceAnchor}\n` +
...
export function getProjectIdentifier(worktreeRoot?: string): string {
  ...
  // project identifier is a state *identity* (used for OMG_STATE_DIR centralized
  ...
  // below sees the unclimbed root so an inner `.omg-workspace` marker is honored.
...
export function isValidTranscriptPath(transcriptPath: string): boolean {
  ...
  // Allowed: [$GROK_CONFIG_DIR|~/.claude], ~/.omg/..., system temp dir
...
export function resolveToWorktreeRoot(directory?: string): string {
  ...
  // OMG_STATE_DIR, the centralized-state *identity* (getProjectIdentifier).
  ...
  // So when OMG_STATE_DIR is set — where on-disk placement is moot and identity
  ...
  const resolveRoot =
    process.env.OMG_STATE_DIR || process.env.OMC_STATE_DIR
      ? getGitTopLevel
      : getWorktreeRoot;
  if (directory) {
 ...
 *   ~/.grok/projects/-path-to-project--claude-worktrees-X/<session>.jsonl
 *
 * But the actual transcript lives at the original project's path:
 *   ~/.grok/projects/-path-to-project/<session>.jsonl
 *
