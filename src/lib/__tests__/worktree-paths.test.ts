...
try {
  const probe = join(tmpdir(), `omg-symlink-probe-${process.pid}`);
  const probeTarget = join(tmpdir(), `omg-symlink-target-${process.pid}`);
...
describe('worktree-paths', () => {
  ...
  describe('getOmcRoot with OMC_STATE_DIR (Issue #1014)', () => {
    it('should return default .omg path when OMC_STATE_DIR is not set', () => {
      delete process.env.OMC_STATE_DIR;
      const result = getOmcRoot(TEST_DIR);
      expect(result).toBe(join(TEST_DIR, '.omg'));
    });
  ...
  describe('workspace marker (.omg-workspace)', () => {
    ...
    it('getOmcRoot anchors to marker dir when marker exists in cwd', () => {
      ...
      fs.writeFileSync(join(workspaceDir, '.omg-workspace'), '');
      clearWorktreeCache();
      const result = getOmcRoot(workspaceDir);
      expect(result).toBe(join(workspaceDir, '.omg'));
    });
    ...
    it('getProjectIdentifier honors explicit id from marker', () => {
      const fs = require('node:fs');
      fs.writeFileSync(
        join(workspaceDir, '.omg-workspace'),
  ...
  describe('workspace marker edge cases', () => {
    ...
    beforeEach(() => {
      clearWorktreeCache();
      rootA = resolve(mkdtempSync(join(tmpdir(), 'omg-ws-edge-A-')));
    });
    ...
    it('nested markers: inner workspace wins over outer', () => {
      // Structure: rootA/.omg-workspace  AND  rootA/B/.omg-workspace
      ...
      writeFileSync(join(rootA, '.omg-workspace'), '');
      writeFileSync(join(innerB, '.omg-workspace'), '');
    ...
    it('OMC_STATE_DIR overrides workspace marker: getOmcRoot returns centralized path', () => {
      const stateDir = mkdtempSync(join(tmpdir(), 'omg-state-override-'));
      try {
        // Drop a workspace marker — without OMC_STATE_DIR it would steer to rootA/.omg
        writeFileSync(join(rootA, '.omg-workspace'), '');
        clearWorktreeCache();
        ...
        // Must use centralized path, not the workspace-anchored .omg
    ...
    it('invalid JSON in marker: readWorkspaceMarkerConfig returns {} gracefully', () => {
      writeFileSync(join(rootA, '.omg-workspace'), '{ not valid json !!!');
    ...
    it.skipIf(process.platform === 'win32' && !canSymlink)(
      ...
      () => {
        const realDir = mkdtempSync(join(tmpdir(), 'omg-ws-real-'));
        const linkDir = join(tmpdir(), `omg-ws-link-${process.pid}`);
        try {
          writeFileSync(join(realDir, '.omg-workspace'), '');
  ...
  describe('warnSiblingRetrofit + clearSiblingRetrofitWarnings', () => {
    ...
    beforeEach(() => {
      anchorDir = resolve(mkdtempSync(join(tmpdir(), 'omg-sibling-anchor-')));
  ...
  describe('resolveSessionStatePaths', () => {
    ...
    beforeEach(() => {
      workDir = resolve(mkdtempSync(join(tmpdir(), 'omg-ssp-')));
      clearWorktreeCache();
    });
    ...
    it('no sessionId: sessionScoped is empty string, effectiveRead and effectiveWrite equal legacy', () => {
      ...
      const expectedLegacy = join(workDir, '.omg', 'state', 'ralph-state.json');
    ...
    it('with sessionId: effectiveWrite is the session-scoped path', () => {
      ...
      const expectedSession = join(workDir, '.omg', 'state', 'sessions', sessionId, 'ultrawork-state.json');
    ...
    it('effectiveRead === sessionScoped after session file is created', () => {
      ...
      const sessionScoped = join(workDir, '.omg', 'state', 'sessions', sessionId, 'ralph-state.json');
      mkdirSync(join(workDir, '.omg', 'state', 'sessions', sessionId), { recursive: true });
  ...
  describe('findWorkspaceRoot home-boundary', () => {
    ...
    beforeEach(() => {
      ...
      fakeHome = resolve(mkdtempSync(join(tmpdir(), 'omg-fakehome-')));
    ...
    it('marker EXACTLY at home dir is NOT returned (null)', () => {
      writeFileSync(join(fakeHome, '.omg-workspace'), '');
      clearWorktreeCache();
    ...
    it('marker BELOW home IS found', () => {
      ...
      writeFileSync(join(projectDir, '.omg-workspace'), '');
      clearWorktreeCache();
