 ...
 *
 * Single entry point for all OMG security settings.
 ...
 *
 * 2. Config file (.claude/omg.jsonc or ~/.config/claude-omg/config.jsonc)
...
function loadSecurityFromConfigFiles(): Partial<SecurityConfig> {
  const paths = [
    ...
    join(getConfigDir(), "claude-omg", "config.jsonc"),
  ];
