...
const DEFAULT_REPO_URL = 'https://github.com/Yeachan-Heo/oh-my-grok';
...
export function generateChangelog(
  ...
  let md = `# oh-my-grok v${version}: ${title}\n\n`;
  md += `## Release Notes\n\n${summary}\n`;
...
export function generateReleaseBody(
  ...
  body += `\n### Install / Update\n\n`;
  body += 'The npm CLI and the Claude Code marketplace/plugin are separate install tracks, not either/or replacements. Update whichever track you use; if you have both installed, update both. CLI-dependent skill paths such as `ask`, `ccg`, and CLI-backed `team` require the `omg` CLI from the npm package.\n\n';
  body += '**CLI / runtime:**\n\n';
  body += '```bash\n';
  body += `npm install -g oh-my-grok@${version}\n`;
  body += '```\n\n';
  body += '**Claude Code plugin:**\n\n';
  body += '```text\n';
  body += '/plugin marketplace update omg\n';
  body += '```\n';
