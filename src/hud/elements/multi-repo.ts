 ...
 *
 * Two modes:
 *  - Marker present (`.omg-workspace` at cwd): show
...
function countActiveSessions(cwd: string): number {
  ...
  // so getOmcRoot resolves to <cwd>/.omg. Route through the canonical
  // helper so OMG_STATE_DIR and OMC_DISABLE_MULTIREPO are honored.
...
export function detectMultiRepo(cwd?: string): MultiRepoInfo | null {
  ...
  try {
    ...
    const hasMarker = existsSync(join(key, '.omg-workspace'));
 ...
 *   multi-repo detected — create .omg-workspace to enable shared state
 */
export function renderMultiRepo(cwd?: string): string | null {
  ...
  if (!info.hasMarker) {
    return (
      ...
      dim(' — run: ') +
      cyan(`echo {} > "${info.parentName}/.omg-workspace"`) +
