...
import { readAutopilotStateForHud } from '../omg-state.js';
...
function workflowState(overrides: Record<string, unknown> = {}): Record<string, unknown> {
  ...
  const transcriptRoot = '/tmp/omg-autopilot-profile-transcripts';
...
describe('autopilot workflow profile observability', () => {
  ...
  it('renders a verified profile with bounded name, selected stage, and progress', () => {
    ...
    const directory = mkdtempSync(join(tmpdir(), 'omg-autopilot-profile-'));
  ...
  it('bounds and redacts Stop-facing runtime insight fields', () => {
    const directory = mkdtempSync(join(process.cwd(), '.tmp-omg-runtime-insight-profile-'));
    directories.push(directory);
    writeHudState({
