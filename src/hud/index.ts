 ...
 *
 * Statusline command that visualizes oh-my-grok state.
 ...
 * Helps users verify HUD setup after omg-setup.
 */
function showDiagnostic(): void {
  ...
  console.log(`[OMG] HUD v${version} | preset: ${preset}`);
  ...
  if (!hudExists || !statusLineOk) {
    console.log("  Run /oh-my-grok:hud setup to fix.");
  } else {
...
async function main(watchMode = false, skipInit = false): Promise<void> {
  try {
    ...
    const context: HudRenderContext = {
      ...
      profileName: process.env.GROK_CONFIG_DIR
        ? basename(process.env.GROK_CONFIG_DIR).replace(/^\./, "")
        : null,
      sessionSummary,
