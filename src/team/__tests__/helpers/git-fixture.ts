...
//   - Worker branches are omg-team/{teamName}/{workerName} (matching getBranchName)
//   - Leader branch must NOT be main/master (M3 hardening) — defaults to 'omg-team-test-leader'
...
export async function createGitFixture(opts: CreateGitFixtureOpts): Promise<GitFixture> {
  const {
    workerCount,
    leaderBranchName = 'omg-team-test-leader',
    teamName = 'test-team',
  ...
  const tmpBase = mkdtempSync(join(tmpdir(), 'omg-test-'));
  ...
  for (let i = 0; i < workerCount; i++) {
    ...
    const branch = `omg-team/${teamName}/${workerName}`;
