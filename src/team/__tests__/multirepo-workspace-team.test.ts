 ...
 * detected" in a multi-repo .omg-workspace layout.
 *
 * In that layout the shared .omg root lives at the workspace anchor (the parent
 ...
 *
 * Setup mirrors the reviewer's reproduction: parent/.omg-workspace + parent/api.
 */
...
describe('multi-repo workspace team writes', () => {
  ...
  beforeEach(() => {
    ...
    parent = mkdtempSync(join(tmpdir(), 'omg-multirepo-team-'));
    writeFileSync(join(parent, '.omg-workspace'), '{}');
    api = join(parent, 'api');
  ...
  it('audit log writes land under the shared .omg, not the sub-repo', () => {
  ...
  it('restart state writes under the shared .omg without traversal error', () => {
  ...
  it('worktree creation + metadata + AGENTS overlay work from the sub-repo', () => {
    ...
    // Worktree and metadata live under the shared workspace .omg, above the repo.
