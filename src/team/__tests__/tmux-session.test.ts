  ...
  it('escapes psmux cmd.exe env vars and quoted launch args without PowerShell syntax', () => {
    ...
    const cmd = buildWorkerStartCommand({
      ...
      envVars: {
        ...
        OMC_TEAM_STATE_ROOT: 'C:\\Users\\Test User\\AppData\\Local\\omg state',
    ...
    expect(cmd).toContain('set "OMC_TEAM_STATE_ROOT=C:\\Users\\Test User\\AppData\\Local\\omg state"');
...
describe('pane readiness startup banners', () => {
  ...
  it('treats Claude Code v2.1.x idle pane (prompt above persistent mode indicator) as ready', () => {
    ...
    // misread as still bootstrapping and OMG never dispatched the inbox to
    // claude workers, leaving them hung with "[OMG] Starting..." forever.
    const capture = [
