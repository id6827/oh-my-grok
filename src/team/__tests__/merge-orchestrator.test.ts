...
describe('commit watcher + auto-merge', () => {
  it('detects a SHA change and triggers a merge', async () => {
    ...
    try {
      ...
      const branchName = `omg-team/demo-team/${sanitizeName(workerName)}`;
      let revParseCount = 0;
      on(
  ...
  it('handles multiple workers independently', async () => {
    ...
    try {
      ...
      const branchA = `omg-team/demo-team/${sanitizeName('alice')}`;
      const branchB = `omg-team/demo-team/${sanitizeName('bob')}`;
      let aCount = 0;
      let bCount = 0;
      on(
  ...
  it('runs preflight reset before each merge', async () => {
    ...
    try {
      ...
      const branchName = `omg-team/demo-team/${sanitizeName('alice')}`;
      let count = 0;
      on(
        (args) => args[0] === 'rev-parse' && args[1] === `refs/heads/${branchName}`,
        () => {
...
describe('M6 recoverFromRestart', () => {
  it('loads persisted SHA state and reports orphaned rebases', async () => {
    ...
    try {
      ...
      mkdirSync(join(repoRoot, '.omg', 'state', 'team', sanitizeName(cfg.teamName)), {
        recursive: true,
      });
      ...
      atomicWriteJson(worktreesMetaPath, [
        {
          path: aliceWtPath,
          branch: 'omg-team/demo-team/alice',
          workerName: 'alice',
...
describe('worker registration', () => {
  ...
  it('is idempotent — registering twice is a no-op', async () => {
    ...
    try {
      ...
      on(
        (args) => args[0] === 'rev-parse' && args[1].startsWith('refs/heads/omg-team'),
        () => 'sha\n',
      );
