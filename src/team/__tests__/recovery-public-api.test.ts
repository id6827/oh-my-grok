...
describe('public dead-worker recovery facade', () => {
  it('classifies authoritative config independently of any manifest before dispatching recovery effects', async () => {
    ...
    try {
      ...
      const configPath = join(cwd, '.omg', 'state', 'team', 'legacy-team', 'config.json');
      ...
      const malformedConfigPath = join(cwd, '.omg', 'state', 'team', 'malformed-team', 'config.json');
      ...
      const malformedRevisionPath = join(cwd, '.omg', 'state', 'team', 'malformed-revision-team', 'config.json');
      ...
      const manifestOnlyPath = join(cwd, '.omg', 'state', 'team', 'manifest-only-team', 'manifest.json');
