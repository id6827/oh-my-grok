...
const BASE_MERGE_ARGS = {
  workerName: 'writer',
  workerBranch: 'omg-team/test-team/writer',
  leaderBranch: 'omg-team/test-team-leader',
...
describe('formatMergeConflictForLeader', () => {
  it('matches snapshot', () => {
    ...
    expect(result).toMatchInlineSnapshot(`
      "### Merge conflict: writer → omg-team/test-team-leader

      **Worker branch:** \`omg-team/test-team/writer\`
      **Leader branch:** \`omg-team/test-team-leader\`
      **Merge base:** \`abc1234\`
      ...
      **Leader: choose strategy.** To resolve, run:
      ...
      \`\`\`sh
      git checkout omg-team/test-team-leader && git merge --no-ff omg-team/test-team/writer
  ...
  it('includes resolution recipe with correct branches', () => {
    const result = formatMergeConflictForLeader(BASE_MERGE_ARGS);
    expect(result).toContain('git checkout omg-team/test-team-leader && git merge --no-ff omg-team/test-team/writer');
  });
...
describe('formatRebaseConflictForWorker', () => {
  it('matches snapshot', () => {
    ...
    expect(result).toMatchInlineSnapshot(`
      "### Rebase conflict: writer onto omg-team/test-team-leader

      **Worker branch:** \`omg-team/test-team/writer\`
      **Base branch:** \`omg-team/test-team-leader\`
      **Base SHA:** \`def5678\`
...
const TEST_CWD = join(tmpdir(), `omg-test-conflict-mailbox-${process.pid}`);
