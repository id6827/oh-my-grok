...
describe('worker-bootstrap', () => {
  ...
  describe('generateWorkerOverlay', () => {
    ...
    it('renders plugin-safe CLI lifecycle examples when omg is unavailable in plugin installs', () => {
      ...
      expect(overlay).toContain('node "$GROK_PLUGIN_ROOT"/bridge/cli.cjs team api read-task');
      expect(overlay).toContain('node "$GROK_PLUGIN_ROOT"/bridge/cli.cjs team api claim-task');
      expect(overlay).toContain('node "$GROK_PLUGIN_ROOT"/bridge/cli.cjs team api transition-task-status');
    });
