...
# Speaker Notes: oh-my-grok Seminar
...
- "Right now, when you use Grok Build or similar tools, you're essentially getting a very smart intern. They can do anything you ask, but YOU have to manage the workflow."
...
**The Reveal**
- "That's exactly what oh-my-grok does. It transforms Grok Build from a single generalist assistant into a coordinated team of 28 specialists."
...
- "That's the shift OMG makes. Claude becomes the conductor, and 28 specialized agents are the orchestra."
...
- "Installation: `/oh-my-grok:hud setup` - adds to your shell prompt"
...
### Key Points
- Requires Grok Build CLI installed
...
- "You need Grok Build installed: `npm install -g claude-code`"
...
```bash
# Step 1: Install OMG
npm install -g oh-my-grok

# Step 2: Run setup wizard
claude-code "/oh-my-grok:omg-setup"
...
- "OMG transforms Grok Build from a single assistant into a coordinated team."
...
**Resources** (Slide 42)
- "GitHub: github.com/Yeachan-Heo/oh-my-grok"
...
**Answer:**
"OMG itself is completely free - it's MIT licensed open-source. What you pay for is the Claude API usage.
...
The key cost benefit: OMG's smart model routing saves you 30-50% on token costs compared to manually using Claude. For example, simple searches use Haiku (super cheap), complex debugging uses Opus (expensive but necessary). Without OMG, everything might default to Opus.
...
**Answer:**
"Currently OMG is designed specifically for Grok Build and leverages Claude's three model tiers - Haiku, Sonnet, and Opus.
...
### 3. "How is this different from just using Grok Build?"

**Answer:**
"Great question. Without OMG, Grok Build gives you one very smart generalist assistant. You tell it every step: 'search for this, analyze that, now implement this, now test that.'
...
Real-world impact: Tasks that took 30 minutes of back-and-forth with Grok Build now take 5 minutes of autonomous execution with OMG."

### 4. "What about security? Is my code safe?"

**Answer:**
"OMG runs entirely locally via Grok Build. Your code never leaves your machine except through the normal Claude API calls that you'd be making anyway.
...
Place custom agent definitions in `~/.grok/agents/{agent-name}.md` and they'll override the defaults.

For example, if you want a specialized Python testing agent:
```markdown
# ~/.grok/agents/pytest-specialist.md
You are an expert in pytest and Python testing best practices.
Focus on: fixtures, parametrization, mocking with pytest-mock.
```

Then invoke: `spawn_subagent(subagent_type="oh-my-grok:pytest-specialist")`

You can also customize execution modes, delegation categories, and model routing rules via the config file at `~/.grok/.omg-config.json`.
...
**Answer:**
"Yes. OMG works with any language that Grok Build supports - which is basically all mainstream languages.
...
For CI/CD, you'd use Grok Build's built-in capabilities or traditional CI tools.
