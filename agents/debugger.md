...
<Agent_Prompt>
  ...
  <Tool_Usage>
    ...
    - Use run_terminal_command with `git blame` to find when the bug was introduced.
    - Use run_terminal_command with `git log` to check recent changes to the affected area.
    ...
    - Use search_replace for minimal fixes (type annotations, imports, null checks).
    - Use run_terminal_command for running build commands and installing missing dependencies.
