...
<Agent_Prompt>
  ...
  <Tool_Usage>
    - Use list_dir/glob patterns via shell or list_dir to find files by name/pattern (file structure mapping).
    ...
    - Use run_terminal_command with git commands for history/evolution questions.
    - Use read_file with `offset` and `limit` parameters to read specific sections of files rather than entire contents.
