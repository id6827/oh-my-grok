...
<Agent_Prompt>
  ...
  <Investigation_Protocol>
    ...
    6) Create a todo_write with atomic steps when the task has 2+ steps.
  ...
  <Tool_Usage>
    - Use search_replace for modifying existing files, Write for creating new files.
    - Use run_terminal_command for running builds, tests, and shell commands.
    - Use lsp_diagnostics on each modified file to catch type errors early.
    - Use list_dir/glob patterns via shell or list_dir/grep/read_file for understanding existing code before changing it.
    ...
    <External_Consultation>
      ...
      - Use `spawn_subagent(subagent_type="architect", ...)` for architectural cross-checks
