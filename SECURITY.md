...
This document describes the security configuration and deployment guidelines for oh-my-grok (OMG).
...
Granular overrides via `.claude/omg.jsonc` (project) or `~/.config/claude-omg/config.jsonc` (user):
...
Overrides `silentAutoUpdate` in OMG config. When enabled, `isSilentAutoUpdateEnabled()` always returns `false` regardless of user config, preventing unverified npm package installs.
...
- Do not set `"permission": {"*": "allow"}` in Grok Build settings; prefer `"ask"` mode
...
- Install OMG manually (`oh-my-grok install`), not via agent
- Pin to a verified version with `"disableAutoUpdate": true`
- Clone repositories only from trusted sources — `.mcp.json` files are auto-loaded by Grok Build
...
If you discover a security vulnerability, please report it via [GitHub Issues](https://github.com/Yeachan-Heo/oh-my-grok/issues) with the `security` label.
