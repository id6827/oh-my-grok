...
**Grok Build를 위한 멀티 에이전트 오케스트레이션. 학습 곡선 제로.**

*Grok Build를 배우지 마세요. 그냥 OMG를 쓰세요.*

[시작하기](#빠른-시작) • [문서](https://yeachan-heo.github.io/oh-my-grok-website) • [CLI 레퍼런스](https://yeachan-heo.github.io/oh-my-grok-website/docs/#cli-reference) • [워크플로우](https://yeachan-heo.github.io/oh-my-grok-website/docs/#workflows) • [마이그레이션 가이드](docs/MIGRATION.md) • [Discord](https://discord.gg/jq6jnSGABY)
...
**v4.1.7**부터 **Team**이 OMG의 표준 오케스트레이션 방식입니다. 레거시 `swarm` 키워드/스킬은 제거되었으니 `team`을 직접 사용하세요.
...
`~/.claude/settings.json`에서 Grok Build 네이티브 팀을 활성화하세요:
...
> 팀이 비활성화된 경우 OMG가 경고를 표시하고 가능한 경우 팀 없이 실행으로 폴백합니다.

### tmux CLI 워커 — Codex & Gemini (v4.4.0+)

**v4.4.0에서 Codex/Gemini MCP 서버**(`x`, `g` 프로바이더)가 **제거됩니다**. CLI 우선 Team 런타임(`omg team ...`)으로 tmux 분할 창에서 실제 CLI 프로세스를 실행하세요:
...
`/omg-teams`는 레거시 호환 스킬로 유지되며, 현재는 내부적으로 `omg team ...`으로 라우팅됩니다.
...
| 실행 표면 | 워커 | 최적 용도 |
|-------|---------|----------|
| `omg team N:codex "..."` | N개 Codex CLI 창 | 코드 리뷰, 보안 분석, 아키텍처 |
| `omg team N:gemini "..."` | N개 Gemini CLI 창 | UI/UX 디자인, 문서, 대용량 컨텍스트 |
| `omg team N:claude "..."` | N개 Claude CLI 창 | tmux에서 Claude CLI를 통한 일반 작업 |
...
> **참고: 패키지 이름** — 프로젝트 브랜드명은 **oh-my-grok** (저장소, 플러그인, 명령어)이지만, npm 패키지는 [`oh-my-grok`](https://www.npmjs.com/package/oh-my-grok)로 배포됩니다. npm/bun으로 CLI 도구를 설치할 때는 `npm install -g oh-my-grok`를 사용하세요.
...
> **참고:** 마켓플레이스 auto-update가 활성화되어 있지 않은 경우, 셋업 실행 전에 `/plugin marketplace update omg`를 수동으로 실행하여 최신 버전을 동기화해야 합니다.
...
### 실행 모드
다양한 사용 사례를 위한 여러 전략 - 완전 자율 빌드부터 토큰 효율적인 리팩토링까지. [자세히 보기 →](https://yeachan-heo.github.io/oh-my-grok-website/docs/#execution-modes)

| 모드 | 특징 | 용도 |
|------|---------|---------|
| **Team (권장)** | 단계별 파이프라인 | 공유 작업 목록에서 협력하는 Claude 에이전트 |
| **omg team (CLI)** | tmux CLI 워커 | Codex/Gemini CLI 작업; 요청 시 실행, 완료 후 종료 |
...
- **HUD 상태바** - 상태바에서 실시간 오케스트레이션 메트릭 확인
  - Grok Build를 `claude --plugin-dir <path>`로 직접 시작하는 경우 (OMG shim 우회), shell에서 `OMC_PLUGIN_ROOT=<path>`를 내보내 HUD 번들이 plugin 로더와 동일한 checkout으로 확인되도록 하세요. 자세한 내용은 [REFERENCE.md의 Plugin directory flags 섹션](./docs/REFERENCE.md#plugin-directory-flags)을 참조하세요.
...
OMG에 기여하고 싶으신가요? [CONTRIBUTING.md](./CONTRIBUTING.md)에서 포킹, 로컬 checkout 설정, 활성 플러그인으로 연결, 테스트 실행, PR 제출 등을 포함한 완전한 개발자 가이드를 참조하세요.

<!-- TODO(i18n): verify translation -->

### 커스텀 스킬

한 번 배운 것을 영원히 재사용합니다. OMG는 디버깅 과정에서 얻은 실전 지식을 이식 가능한 스킬 파일로 추출하고, 관련 상황에서 자동으로 주입합니다.

| | 프로젝트 스코프 | 사용자 스코프 |
|---|---|---|
| **경로** | `.omg/skills/` | `~/.omg/skills/` |
...
| `omg team` | tmux CLI 워커 (codex/gemini/claude) | `omg team 2:codex "security review"` |
...
- `/omg-teams`는 레거시 호환 경로로 남아 있으며 내부적으로 `omg team ...`으로 라우팅됩니다.
...
속도 제한이 리셋될 때 Grok Build 세션을 자동 재개합니다.

```bash
omg wait          # 상태 확인, 가이드 받기
omg wait --start  # 자동 재개 데몬 활성화
omg wait --stop   # 데몬 비활성화
```
...
Grok Build 세션 이벤트를 [OpenClaw](https://openclaw.ai/) 게이트웨이로 전달하여 OpenClaw 에이전트를 통한 자동화된 응답 및 워크플로우를 구성할 수 있습니다.
...
- **[CLI 레퍼런스](https://yeachan-heo.github.io/oh-my-grok-website/docs/#cli-reference)** - 모든 `omg` 명령어, 플래그 및 도구
- **[알림 가이드](https://yeachan-heo.github.io/oh-my-grok-website/docs/#notifications)** - Discord, Telegram, Slack 및 webhook 설정
- **[추천 워크플로우](https://yeachan-heo.github.io/oh-my-grok-website/docs/#workflows)** - 일반 작업을 위한 검증된 스킬 체인
- **[릴리스 노트](https://yeachan-heo.github.io/oh-my-grok-website/docs/#release-notes)** - 각 버전의 새로운 기능
- **[웹사이트](https://yeachan-heo.github.io/oh-my-grok-website)** - 인터랙티브 가이드와 예제
...
OMG는 교차 검증과 디자인 일관성을 위해 외부 AI 제공자를 선택적으로 활용할 수 있습니다. **필수가 아닙니다** — OMG는 이것들 없이도 완벽하게 작동합니다.
