...
echo -e "${BLUE}Oh-My-Grok Uninstaller${NC}"
echo ""

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
. "$SCRIPT_DIR/lib/config-dir.sh"

# Grok Build config directory (defaults to ~/.claude)
GROK_CONFIG_DIR="$(resolve_claude_config_dir)"

echo "This will remove ALL OMG components from:"
echo "  $GROK_CONFIG_DIR"
echo ""
...
rm -f "$GROK_CONFIG_DIR/agents/architect.md"
rm -f "$GROK_CONFIG_DIR/agents/document-specialist.md"
rm -f "$GROK_CONFIG_DIR/agents/explore.md"
rm -f "$GROK_CONFIG_DIR/agents/designer.md"
rm -f "$GROK_CONFIG_DIR/agents/writer.md"
rm -f "$GROK_CONFIG_DIR/agents/vision.md"
rm -f "$GROK_CONFIG_DIR/agents/critic.md"
rm -f "$GROK_CONFIG_DIR/agents/analyst.md"
rm -f "$GROK_CONFIG_DIR/agents/executor.md"
rm -f "$GROK_CONFIG_DIR/agents/planner.md"

# Remove commands
echo -e "${BLUE}Removing commands...${NC}"
rm -f "$GROK_CONFIG_DIR/commands/coordinator.md"
rm -f "$GROK_CONFIG_DIR/commands/omg.md"
rm -f "$GROK_CONFIG_DIR/commands/ultrawork.md"
rm -f "$GROK_CONFIG_DIR/commands/deepsearch.md"
rm -f "$GROK_CONFIG_DIR/commands/analyze.md"
rm -f "$GROK_CONFIG_DIR/commands/plan.md"
rm -f "$GROK_CONFIG_DIR/commands/review.md"
rm -f "$GROK_CONFIG_DIR/commands/planner.md"
rm -f "$GROK_CONFIG_DIR/commands/orchestrator.md"
rm -f "$GROK_CONFIG_DIR/commands/update.md"

# Remove skills
echo -e "${BLUE}Removing skills...${NC}"
rm -rf "$GROK_CONFIG_DIR/skills/ultrawork"
rm -rf "$GROK_CONFIG_DIR/skills/git-master"
rm -rf "$GROK_CONFIG_DIR/skills/frontend-ui-ux"

# Remove hooks
echo -e "${BLUE}Removing hooks...${NC}"
rm -f "$GROK_CONFIG_DIR/hooks/keyword-detector.sh"
rm -f "$GROK_CONFIG_DIR/hooks/stop-continuation.sh"
rm -f "$GROK_CONFIG_DIR/hooks/silent-auto-update.sh"

# Remove version, state, and config files
echo -e "${BLUE}Removing state and config files...${NC}"
rm -f "$GROK_CONFIG_DIR/.omg-version.json"
rm -f "$GROK_CONFIG_DIR/.omg-silent-update.json"
rm -f "$GROK_CONFIG_DIR/.omg-update.log"
rm -f "$GROK_CONFIG_DIR/.omg-config.json"

# Remove hook configurations from settings.json
SETTINGS_FILE="$GROK_CONFIG_DIR/settings.json"
...
if [ -d "$GROK_CONFIG_DIR/../.omg" ] || [ -d ".omg" ]; then
...
echo "  - CLAUDE.md: rm $GROK_CONFIG_DIR/CLAUDE.md"
echo "  - settings.json backup: rm $GROK_CONFIG_DIR/settings.json.bak"
echo ""
echo "To verify complete removal, check:"
echo "  ls -la $GROK_CONFIG_DIR/"
