...
// OMC_MODEL_* is intentionally excluded: those are OMG-internal vars that the OMG bridge
...
function resolveTierAliasToSafeModel(tierAlias) {
  ...
  for (const key of keys) {
    ...
    // OMG-internal vars (OMC_SUBAGENT_MODEL, OMC_MODEL_*) are not read by CC, so a [1m]
...
/**
 * Read the `model:` field from an OMG agent definition's YAML frontmatter.
...
function readAgentDefinitionModel(subagentType) {
  ...
  const agentType = (typeof subagentType === 'string' ? subagentType : '').replace(/^oh-my-grok:/, '');
  ...
  // GROK_PLUGIN_ROOT is tried first when set; the script-relative path is always the
...
/**
 * Resolve the .omg root directory for a given starting directory.
 *
 ...
 *   2) Walk up from startDir looking for a .omg-workspace marker file.
 ...
 * @returns {string} Absolute path to the .omg root directory
 */
function resolveOmcRoot(startDir) {
  ...
  if (process.env.OMC_STATE_DIR) {
    process.stderr.write(
      '[omg] OMC_STATE_DIR is set; resolveOmcRoot() falling through to workspace-marker ' +
      'resolution. Use resolveOmcStateRoot() for full OMC_STATE_DIR support.\n'
    );
  }

  // 2) Walk up looking for .omg-workspace marker
  try {
    let cursor = resolve(dir);
    const home = (() => { try { return resolve(homedir()); } catch { return null; } })();
    while (true) {
      if (existsSync(join(cursor, '.omg-workspace'))) {
...
async function getTodoStatus(directory) {
  ...
  // [$GROK_CONFIG_DIR|~/.claude]/todos/ directory.
...
// `<local-command-stdout>` display echo (which Grok Build also emits for ordinary
...
//
// Grok Build does not expose live `/goal` state to hooks (the PreToolUse payload
...
function extractGoalFromTranscript(rawTranscriptPath, sessionId, cwd) {
  ...
  // Bind to the active session; Grok Build names the transcript `<sessionId>.jsonl`.
...
function extractClaudeGoalSnapshot(data, sessionId, cwd) {
  ...
  // Runtime injected no goal field (the standard Grok Build case): fall back to the
...
function isCancelSkillBootstrapTool(toolName, toolInput) {
  ...
  return /^(?:omg|oh-my-grok|gjc)\s+(?:state\s+(?:clear|read|write|list-active|get-status)|cancel)\b/.test(command.trim());
}
...
function isUltragoalBootstrapTool(toolName, toolInput) {
  ...
  const command = typeof toolInput.command === 'string' ? toolInput.command : '';
  if (!isSingleShellCommand(command)) return false;
  return /^(?:omg|oh-my-grok)\s+ultragoal\s+(?:create(?:-goals)?|complete(?:-goals)?|next|start-next|status|checkpoint|record-review-blockers)\b/.test(command.trim());
}
...
function evaluateUltragoalPreToolEnforcement(stateDir, directory, sessionId, data) {
  ...
  // #3341: Grok Build does not expose live `/goal` state to a PreToolUse hook, so
...
function generateAgentSpawnMessage(toolInput, stateDir, todoStatus, sessionId) {
  ...
  // Grok Build 2.1.178+ removed TeamCreate/TeamDelete. When OMG team state is
  ...
  // Grok Build and should only be treated as legacy metadata.
  ...
  if (teamState && !toolInput.name) {
    ...
    return `[TEAM ROUTING REQUIRED] Team "${teamName}" is active but you are spawning an unnamed subagent. ` +
      `Grok Build 2.1.178+ uses the session's implicit native agent team; TeamCreate and TeamDelete are removed. ` +
      `Spawn teammates directly with Agent/Task name="worker-N" and subagent_type="${agentType}". ` +
      `Do NOT rely on team_name for routing; native Grok Build accepts it only as ignored legacy metadata.`;
  }
...
function generateMessage(toolName, todoStatus, modeActive = false) {
  ...
  if (QUIET_LEVEL >= 2 && toolName === 'todo_write') {
    return '';
  }

  const messages = {
    todo_write: `${todoStatus}Mark todos in_progress BEFORE starting, completed IMMEDIATELY after finishing.`,
...
const SKILL_PROTECTION_MAP = {
  ...
  sciomc: 'medium', learner: 'medium', 'omg-setup': 'medium',
...
function getSkillProtectionLevel(skillName, rawSkillName) {
  ...
  // See: https://github.com/Yeachan-Heo/oh-my-grok/issues/1581
  if (rawSkillName != null && typeof rawSkillName === 'string' &&
      !rawSkillName.toLowerCase().startsWith('oh-my-grok:')) {
    return 'none';
  }
  const normalized = (skillName || '').toLowerCase().replace(/^oh-my-grok:/, '');
...
function loadOmcConfig() {
  const configPaths = [
    join(getClaudeConfigDir(), '.omg-config.json'),
...
function writeSkillActiveState(stateDir, skillName, sessionId, rawSkillName) {
  ...
  // Nesting guard: when a skill (e.g. omg-setup) invokes a child skill
  ...
  // (non-atomic), but this is acceptable because Grok Build sessions are
...
async function main() {
  ...
  try {
    ...
    // Resolve the .omg state root once, honoring OMC_STATE_DIR.
    ...
    if (toolName === 'Task' || toolName === 'Agent') {
      ...
      if (isForceInheritEnabled()) {
        ...
        // fallen through safely). Grok Build reads the agent definition's `model:` field
        ...
        if (!toolModel && toolInput.subagent_type) {
          ...
          if (agentDefModel && !isSubagentSafeModelId(agentDefModel) && !isTierAlias(agentDefModel)
            ...
            const agentType = (toolInput.subagent_type).replace(/^oh-my-grok:/, '');
            console.log(JSON.stringify({
              continue: true,
              hookSpecificOutput: {
                hookEventName: 'PreToolUse',
        ...
        // Task/Agent calls without an explicit model param. Without this, Grok Build
    ...
    // Send notification when ask_user_question is about to execute (user input needed)
    ...
    if (delegationBlock) {
      ...
      // Grok Build hookSpecificOutput shape (`permissionDecision: 'deny'`).
      console.log(JSON.stringify({
        continue: true,
        hookSpecificOutput: {
          hookEventName: 'PreToolUse',
