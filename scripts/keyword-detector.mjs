...
// Resolve OMG package root: GROK_PLUGIN_ROOT (plugin system) or derive from this script's location
const _omcRoot = process.env.GROK_PLUGIN_ROOT ||
...
function isExplicitRalplanSlashInvocation(prompt) {
  return /^\s*\/(?:oh-my-grok:)?ralplan(?:\s|$)/i.test(prompt);
}

function isExplicitAskSlashInvocation(prompt) {
  return /^\s*\/(?:oh-my-grok:)?ask\s+(?:claude|codex|gemini|antigravity|agy|grok|cursor)\b/i.test(prompt);
}
...
const SYSTEM_ECHO_BLOCK_PATTERNS = [
  ...
  // Stop-hook wrapping by the Grok Build harness
...
/**
 * Check if the team feature is enabled in Grok Build settings.
 * Reads settings.json from [$GROK_CONFIG_DIR|~/.claude] and checks for
...
// Read the OMG JSONC config the way src/config/loader.ts does, inlined so the
...
/**
 * Skills the user opted out of via `keywordDetector.disabled` in the OMG
 * config: project `.grok/omg.jsonc` first, then user
 * `~/.config/grok-omg/config.jsonc`, the same JSONC surface
...
function loadDisabledKeywords(directory) {
  const configPaths = [
    join(directory || process.cwd(), '.claude', 'omg.jsonc'),
    join(getOmcUserConfigDir(), 'claude-omg', 'config.jsonc'),
  ];
...
/**
 * Create proper hook output with additionalContext (Grok Build hooks API)
...
async function main() {
  ...
  try {
    ...
    // activate modes in the current Grok Build session.
