...
# shells (e.g. Grok Build hook invocations). Fixes issue #892.
#
# Priority:
#   1. nodeBinary stored in ~/.grok/.omg-config.json (set at setup time)
...
#
# Exits 0 on failure so it never blocks Grok Build hook processing.
...
# ---------------------------------------------------------------------------
if [ -z "$NODE_BIN" ]; then
  printf '[OMG] Error: Could not find node binary. Run /oh-my-grok:omg-setup to fix.\n' >&2
  exit 0  # exit 0 so this hook does not block Grok Build
fi
