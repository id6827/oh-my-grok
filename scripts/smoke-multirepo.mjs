...
// Fixture setup
// --------------------------------------------------------------------------
const FIXTURE = join(tmpdir(), `omg-multirepo-smoke-${process.pid}`);
...
writeFileSync(join(FIXTURE, '.omg-workspace'), JSON.stringify({ id: 'smoke-test' }));
...
// --------------------------------------------------------------------------
console.log('\n=== STEP 1: API dir — workspace root + omg root + project identifier ===');
...
assert(
  ...
  'findWorkspaceRoot must walk up from sub-git-repo to .omg-workspace marker'
);
...
// --------------------------------------------------------------------------
// Step 2 — web/ paths same .omg, different session subdir
// --------------------------------------------------------------------------
console.log('\n=== STEP 2: WEB dir — same .omg, different session ===');
...
// workspace-marker root when .omg-workspace is present (fixed in multi-repo
// rollout). The CLI sets cwd = subprocess cwd; resolution flows through
// getOmcRoot which honors OMC_STATE_DIR > .omg-workspace > git > cwd.
...
if (process.platform === 'win32') {
  ...
  // Assert: no resolved write path contains \.omg\ OUTSIDE the configured OMC_STATE_DIR root
  const containsOmcOutsideRoot =
    writePath.includes('\\.omg\\') &&
    !writePath.startsWith(winStateDir);

  assert(
    !containsOmcOutsideRoot,
    'Windows: resolved write path does not contain \\.omg\\ outside configured OMC_STATE_DIR',
    'Windows: resolved write path contains \\.omg\\ OUTSIDE OMC_STATE_DIR (path escape bug)',
...
const retrofitFixture = join(tmpdir(), `omg-retrofit-smoke-${process.pid}`);
...
writeFileSync(join(retrofitFixture, '.omg-workspace'), JSON.stringify({ id: 'retrofit-test' }));
...
assert(
  retrofitStderr.includes('[omg] workspace-retrofit warning'),
...
// --------------------------------------------------------------------------
// Step 8 — Template drift simulation: AST-grep gate red on raw .omg pattern
// --------------------------------------------------------------------------
console.log('\n=== STEP 8: AST-grep gate — drift fixture triggers non-zero exit ===');

const driftFixtureDir = join(tmpdir(), `omg-drift-gate-${process.pid}`);
...
assert(
  ...
  'AST-grep gate exits ZERO on drift fixture (failed to detect raw .omg pattern)',
  `exit=${gateResult.status}\nstdout=${gateResult.stdout?.slice(0, 300)}\nstderr=${gateResult.stderr?.slice(0, 300)}`,
  'Gate must exit non-zero when raw join(...,.omg,...) is found outside whitelist'
);
...
if (failed === 0) {
  ...
  console.log('  state) anchor to .omg-workspace marker when present.\n');
} else {
