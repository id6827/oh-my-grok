#!/usr/bin/env node
/**
 * Repair OMG plugin cache references after marketplace updates.
 *
 * Grok Build can keep a running session pointed at the previous plugin cache
...
function getClaudeConfigDir() {
  const configured = (process.env.GROK_CONFIG_DIR || join(homedir(), '.claude')).replace(/[\\/]+$/, '');
...
function isValidOmcPluginRoot(root) {
  return existsSync(join(root, 'hooks', 'hooks.json'))
    || existsSync(join(root, 'skills', 'omg-setup', 'SKILL.md'))
...
function rewriteOmcRegistryEntries(installedPluginsPath, latestRoot) {
  ...
  for (const [pluginId, value] of Object.entries(plugins)) {
    if (!pluginId.toLowerCase().includes('oh-my-grok') || !Array.isArray(value)) continue;
...
if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
  ...
  for (const error of result.errors) {
    console.warn(`[OMG] Plugin cache repair warning: ${error}`);
  }
  if (!result.latestRoot) {
    console.log('[OMG] No OMG plugin cache found (normal for new installs)');
  } else if (result.registryUpdated || result.hooksPatched || result.symlinked > 0) {
    console.log(`[OMG] Repaired plugin cache references: active=${result.latestRoot}, symlinked=${result.symlinked}${result.hooksPatched ? ', hooks=platform' : ''}`);
  } else {
    console.log('[OMG] Plugin cache references are current');
  }
}
