...
# Grok Build started. A candidate is usable only when it contains the complete
...
resolve_active_plugin_root() {
  is_valid_plugin_root() {
    ...
    [ -d "$candidate" ] \
      ...
      && [ -s "${candidate}/skills/omg-reference/SKILL.md" ]
  }
  ...
  if [ -f "$installed_plugins" ] && command -v jq >/dev/null 2>&1; then
    active_path=$(jq -r '(.plugins // .) | to_entries[] | select(.key | startswith("oh-my-grok")) | .value[0].installPath // empty' "$installed_plugins" 2>/dev/null || true)
...
ensure_local_omc_git_exclude() {
  ...
  if [ -f "$exclude_path" ] && grep -Fq '# BEGIN OMG local artifacts' "$exclude_path"; then
    ...
    printf '.omx/\n' >> "$exclude_path"
    echo "Updated OMG git exclude for local OMX artifacts"
    return 0
  fi
...
EOF
  echo "Configured git exclude for local OMG/OMX artifacts (preserving .omg/skills/)"
}

install_omc_reference_skill() {
  local source="$1"
  [ -s "$source" ] || { echo "Skipped omg-reference skill install (canonical skill source unavailable)"; return 0; }
...
CANONICAL_OMC_REFERENCE_SKILL="${ACTIVE_PLUGIN_ROOT}/skills/omg-reference/SKILL.md"
...
if [ "$MODE" = "local" ]; then
  ...
  SKILL_TARGET_PATH="${CONFIG_ROOT}/skills/omg-reference/SKILL.md"
...
if [ "$MODE" = "global" ]; then
  ...
  if [ -f "$SETTINGS_FILE" ] && jq -e 'any(.. | objects | .command? | strings; test("(^|[^[:alnum:]_-])(keyword-detector|stop-continuation|persistent-mode|session-start)(\\.(sh|mjs|cjs|js))?([^[:alnum:]_-]|$)"))' "$SETTINGS_FILE" >/dev/null 2>&1; then
    echo "NOTE: Found legacy OMG hook entries in settings.json. Remove only the legacy OMG hook entries from $SETTINGS_FILE; third-party hook entries can remain."
  fi
fi

if [ -f "$CONFIG_DIR/settings.json" ] && grep -q 'oh-my-grok' "$CONFIG_DIR/settings.json"; then
  echo "Plugin verified"
else
  echo "Plugin NOT found - run: claude /install-plugin oh-my-grok"
fi
