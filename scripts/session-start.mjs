...
/** Claude config directory (respects GROK_CONFIG_DIR env var) */
 ...
 *
 * Grok Build SessionStart input currently provides session metadata such as
 * session_id, transcript_path, cwd, source, model, and agent_type, but no
 * stable owner process for the interactive session. In installed OMG hooks the
...
function hasDurableAbandonmentEvidence(marker) {
  ...
  // Same-boot hard-kill cleanup requires a durable owner signal. Grok Build
...
function getRuntimeBaseDir() {
  return process.env.GROK_PLUGIN_ROOT || join(__dirname, '..');
}
...
function validateCwd(candidate) {
  ...
  while (true) {
    ...
    if (existsSync(join(cursor, '.omg-workspace')) || existsSync(join(cursor, '.git'))) {
      return candidate;
    }
...
function formatUpdateNoticeForUser(updateInfo, options = {}) {
  ...
  const action = updateInfo?.source === 'marketplace'
    ? 'To update the plugin channel, run: /plugin marketplace update omg && /omg-setup'
...
// Extract OMG version from CLAUDE.md content
...
function getPluginCacheBase() {
  return join(configDir, 'plugins', 'cache', 'omg', 'oh-my-grok');
}
...
function isManagedPluginCacheRoot(pluginRoot) {
  ...
  // cache path shape still proves it is an OMG managed cache version.
  const unixRoot = normalizedRoot.replace(/\\/g, '/');
  return /\/plugins\/cache\/omg\/oh-my-grok\/\d+\.\d+\.\d+(?:-[A-Za-z0-9.-]+)?$/.test(unixRoot);
}
...
function getMarketplaceCloneVersion() {
  try {
    ...
    const pluginEntry = Array.isArray(marketplaceManifest?.plugins)
      ? marketplaceManifest.plugins.find(plugin => plugin?.name === 'oh-my-grok')
      : null;
...
// Get plugin version from GROK_PLUGIN_ROOT
...
async function checkNpmUpdate(currentVersion) {
  ...
  try {
    const response = await fetch('https://registry.npmjs.org/oh-my-grok/latest', {
  ...
  try {
    if (existsSync(settingsFile)) {
      ...
      if (statusLineCommand?.includes('omg-hud')) {
        const pluginCacheBase = join(configDir, 'plugins', 'cache', 'omg', 'oh-my-grok');
  ...
  try {
    ...
    if (driftInfo && shouldNotifyDrift(driftInfo)) {
      ...
      driftMsg += driftInfo.source === 'marketplace'
        ? `\nRun '/plugin marketplace update omg && /omg-setup' to sync plugin-managed components.`
    ...
    try {
      ...
      if (pluginVersion) {
        ...
        if (updateInfo) {
          const omcConfig = readJsonFile(join(configDir, '.omg-config.json')) || {};
    ...
    if (process.env.GROK_PLUGIN_ROOT) {
      try {
        const omcConfigPath = join(configDir, '.omg-config.json');
...
[OMG] HUD not configured (${hudCheck.reason}). Run /hud setup then restart Grok Build.
</system-reminder>`);
    }
...
${cleanContent}
    ...
    // plugin update whose GROK_PLUGIN_ROOT still points to the old version.
    try {
      const cacheBase = join(configDir, 'plugins', 'cache', 'omg', 'oh-my-grok');
      let versions = [];
      ...
      // Guard against GROK_PLUGIN_ROOT pointing to a stale/deleted version.
      // When an old version directory is removed during upgrade but a running
      // session still has the old GROK_PLUGIN_ROOT in its environment, the
      // directory won't exist. Create a symlink so subsequent hook invocations
      // via run.cjs resolve correctly.
      const pluginRoot = process.env.GROK_PLUGIN_ROOT?.replace(/[\/\\]+$/, ''); // strip trailing separators
