export const WINDOWS_HOOK_PREFIX = 'node "$GROK_PLUGIN_ROOT"/scripts/run.cjs ';
export const UNIX_HOOK_PREFIX = 'sh "$GROK_PLUGIN_ROOT"/scripts/find-node.sh "$GROK_PLUGIN_ROOT"/scripts/run.cjs ';
...
export function normalizeHookCommand(command, prefix = hookPrefixForPlatform()) {
  ...
  if (match) return `${prefix}"$GROK_PLUGIN_ROOT"/scripts/${match[1]}${match[2]}`;

  const absNodeMatch = command.match(absoluteNodeRunCjsPattern);
  if (absNodeMatch) return `${prefix}"$GROK_PLUGIN_ROOT"/scripts/${absNodeMatch[2]}${absNodeMatch[3]}`;
