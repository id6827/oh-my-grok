#!/bin/sh
# OMG HUD cached statusLine launcher.
#
# Grok Build invokes statusLine commands for every render. Starting Node and
...
CONFIG_DIR=${GROK_CONFIG_DIR:-$(cd "$SCRIPT_DIR/.." 2>/dev/null && pwd -P)}
...
if [ -s "$OUTPUT_FILE" ]; then
  cat "$OUTPUT_FILE" 2>/dev/null || printf '[OMG] Starting...\n'
...
# sees the real HUD from the first frame. Grok Build v2.1.x does not
# re-poll the statusLine until user interaction, so an async background
# refresh leaves the pane stuck on "[OMG] Starting..." until they type.
