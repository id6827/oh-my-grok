 ...
 *
 * Reads `agents.<name>.model` from the OMG user/project config (the same
...
function getConfigPaths(cwd) {
  return {
    user: join(getConfigDir(), 'claude-omg', 'config.jsonc'),
    project: join(cwd || process.cwd(), '.claude', 'omg.jsonc'),
  };
}
...
export function resolveConfiguredAgentModel(subagentType, cwd) {
  const raw = (typeof subagentType === 'string' ? subagentType : '').replace(/^oh-my-grok:/, '');
