...
//             "denyMessage": "10+ raw Reads in 2 min — spawn Agent(subagent_type='oh-my-grok:explore', model='haiku', ...). Bypass: ALLOW_RAW_READ=1.",
 ...
 *
 * @param {object} args
 * @param {string} args.toolName - Grok Build tool name (Read|Edit|Write|...)
 * @param {string} [args.stateDir] - Directory used to persist the event window.
 *   Typically `<cwd>/.omg/state` to mirror the rest of OMG state storage.
 * @param {object} [args.env=process.env] - Environment for bypass-flag lookup.
 * @param {Function} [args.loadOmcConfig] - Function returning the resolved OMG
        ...
        return {
          ...
          reason:
            typeof rule.denyMessage === 'string' && rule.denyMessage
              ? rule.denyMessage
              : `[OMG] Force-agent-delegation: ${observed} ${toolName} in last ${windowSeconds}s ` +
