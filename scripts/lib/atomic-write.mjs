/**
 * Atomic file writes for oh-my-grok hooks.
...
function acquireLockAt(lockPath, attempts = 50, requireExclusive = false) {
  ...
  if (!processStart || processStart === 'absent') {
    console.error(`[omg-lock] state_mutation_lock_owner_unverifiable: ${lockPath}`);
    return null;
  }
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    ...
    try {
      ...
      if (disposition === 'unverifiable') {
        console.error(`[omg-lock] state_mutation_lock_unverifiable: ${lockPath}`);
        return null;
      }
