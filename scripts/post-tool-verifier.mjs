...
const debugLog = (...args) => {
  if (process.env.OMC_DEBUG) console.error('[omg:debug:post-tool-verifier]', ...args);
};
...
// Pattern to match Grok Build temp CWD permission errors (false positives on macOS)
...
// Strip Grok Build temp CWD noise before pattern matching
function stripClaudeTempCwdErrors(output) {
  return output.replace(CLAUDE_TEMP_CWD_PATTERN, '');
}

// Pattern matching Grok Build's "Error: Exit code N" prefix line
 ...
 * Returns true when output has Grok Build's "Error: Exit code N" prefix
...
export function isNonZeroExitWithOutput(output) {
  ...
  // Must contain Grok Build's exit code prefix
...
function getPreemptiveCooldownFilePath(directory, sessionId) {
  ...
  const cooldownDir = join(tmpdir(), 'omg-preemptive-compaction');
...
function buildPreemptiveContextMessage(percentUsed, severity) {
  if (severity === 'critical') {
    return `[OMG CRITICAL] Context at ${percentUsed}% (critical threshold: ${getPreemptiveCriticalThreshold()}%). Run /compact now before continuing with more tools or agent fan-out.`;
  }

  return `[OMG WARNING] Context at ${percentUsed}% (warning threshold: ${getPreemptiveWarningThreshold()}%). Plan a /compact soon to preserve room for the next large tool output.`;
}
...
function isConsensusPlanningSkillInvocation(skillName, toolInput) {
  ...
  if (skillName !== 'plan' && skillName !== 'omg-plan') return false;
...
// Detect Grok Build's deterministic write/edit success markers so docs or
  ...
  for (const trackingFile of candidates) {
    try {
      ...
      if (quietLevel < 2 && running.length > 0) {
        parts.push(`Running: ${running.length} [${running.map(a => a.agent_type.replace('oh-my-grok:', '')).join(', ')}]`);
      }
