...
/**
 * OMG Stale Worker Cleanup
 *
 * Detects and terminates stale OMG tmux/CLI worker processes that still
 * carry team identity after the team cancellation path has completed. This is
 * a post-cancellation cleanup tool; Grok Build 2.1.178+ has no native
 ...
 * When omitted, scans for ALL matching OMG worker processes.
 *
...
/**
 * Find OMG tmux/CLI worker processes that match team patterns.
...
function findOrphanProcesses(filterTeam) {
  ...
  try {
    if (process.platform === 'win32') {
      ...
      for (const line of output.split('\n')) {
        // Match OMG worker processes with team context (exclude bare 'node' to avoid over-matching).
        if ((line.includes('claude') || line.includes('codex') || line.includes('gemini') || line.includes('omg') || line.includes('oh-my-claude'))) {
 ...
 * hang waiting for the escalation window to elapse. In practice, OMG agent
 * processes (claude/codex/gemini/omg) respect SIGTERM and exit within
...
function main() {
  ...
  if (processes.length === 0) {
    console.log(JSON.stringify({
      cleaned: 0,
      message: teamName
        ? `No stale OMG worker processes found for team "${teamName}".`
        : 'No stale OMG worker processes found.',
    }));
    process.exit(0);
  }
