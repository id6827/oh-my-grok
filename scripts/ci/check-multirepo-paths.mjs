#!/usr/bin/env node
/**
 * AST-grep CI gate: detect raw .omg path constructions that bypass
...
// Files/dirs that are intentionally allowed to contain raw .omg constructions.
...
const WHITELIST_FILES = new Set([
  ...
  // Multi-repo test fixtures and audits (construct fake .omg trees)
 ...
 * (homedir(), os.homedir(), getClaudeConfigDir(), GROK_CONFIG_DIR). These are
 * NOT workspace state — they're per-user installs of the OMG binary itself.
...
for (const filePath of walkFiles(searchRoot)) {
  ...
  for (const pat of patterns) {
    ...
    for (const match of matches) {
      ...
      // Skip global-config constructions: homedir()/.omg, getClaudeConfigDir()/.omg, etc.
...
if (totalHits === 0) {
  console.log('multirepo-paths gate: OK (no raw .omg constructions found outside whitelist)');
  process.exit(0);
} else {
  console.error(`multirepo-paths gate: FAIL — ${totalHits} raw .omg construction(s) found:\n`);
