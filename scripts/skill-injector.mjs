...
// ============================================================================
// OMG root resolver — walk up from cwd looking for workspace markers.
// Mirrors getOmcRoot from src/lib/worktree-paths.ts — inlined for .mjs.
// ============================================================================

/**
 * Walk up from startDir looking for .omg-workspace, then .git, then fallback
 * to startDir itself. Returns the .omg subdirectory of the found root.
...
function resolveOmcRootSync(startDir) {
  ...
  // Walk up looking for .omg-workspace or .git
  while (dir) {
    if (existsSync(join(dir, '.omg-workspace'))) {
      return join(dir, '.omg');
    }
 ...
 *
 * @param {string} omcRoot - Resolved .omg root directory
...
async function main() {
  try {
    ...
    // Resolve OMG root (walk up from data.cwd looking for workspace markers)
...
if (isDisabled) {
  ...
  USER_SKILLS_DIR = join(cfgDir, 'skills', 'omg-learned');
