...
/**
 * Read a single value from the security section of omg config files.
 */
function readSecurityConfigValue(key) {
  const paths = [
    join(process.cwd(), ".claude", "omg.jsonc"),
    join(homedir(), ".config", "claude-omg", "config.jsonc"),
  ];
...
function isAutopilotRoutingEchoPrompt(promptText) {
  return /^\[MAGIC KEYWORDS?(?: DETECTED)?:\s*AUTOPILOT\s*\]\s*$/i.test(promptText) ||
    /^\/(?:oh-my-grok:|omg:)?autopilot(?:\s+execute)?\s*$/i.test(promptText);
}
...
/**
 * Count incomplete Tasks from Grok Build's native Task system.
 */
 ...
 * When context is exhausted, Grok Build needs to stop so it can compact.
 * Blocking these stops causes a deadlock: can't compact because can't stop,
 * can't continue because context is full.
 *
 * See: https://github.com/Yeachan-Heo/oh-my-grok/issues/213
 */
...
async function main() {
  try {
    ...
    // Grok Build sets stop_hook_active when a Stop hook is already running.
    // Never emit another decision:block in that re-entrant path: doing so trips
    // Grok Build's safety override for repeatedly blocked Stop hooks.
    ...
    // Blocking these causes a deadlock where Grok Build cannot compact.
    // See: https://github.com/Yeachan-Heo/oh-my-grok/issues/213
    ...
    if (
      ...
      if (sessionMatches) {
        ...
        if (iteration < maxIter) {
          ...
          let reason = `[RALPH LOOP - ITERATION ${iteration + 1}/${maxIter}] Work is NOT done. Continue working.\nWhen FULLY complete (after Architect verification), run /oh-my-grok:cancel to cleanly exit ralph mode and clean up all state files. If cancel fails, retry with /oh-my-grok:cancel --force.\n${ralph.state.prompt ? `Task: ${ralph.state.prompt}` : ""}`;
          if (errorGuidance) {
        ...
        if (hardMax > 0 && maxIter >= hardMax) {
          ...
          console.log(
            JSON.stringify({
              decision: "block",
              reason: `[RALPH LOOP - HARD LIMIT] Reached hard max iterations (${hardMax}). Mode auto-disabled. Restart with /oh-my-grok:ralph if needed.`,
            }),
          );
          return;
        }
        ...
        const ralphExtendedReason = `[RALPH LOOP - EXTENDED] Max iterations reached; extending to ${ralph.state.max_iterations} and continuing. When FULLY complete (after Architect verification), run /oh-my-grok:cancel (or --force).`;
        console.log(
          JSON.stringify({
            decision: "block",
    ...
    if (
      ...
      if (sessionMatches && !isUltragoalTerminalState(ultragoal.state, omcRoot)) {
        ...
        let reason = `[ULTRAGOAL #${newCount}/${maxReinforcements}] Ultragoal mode is active. Continue the durable goal workflow, keep the matching Claude /goal active, and checkpoint .omg/ultragoal/ledger.jsonl before stopping. When all ultragoal stories are complete and the final quality gate passes, run /oh-my-grok:cancel to cleanly exit.`;
        ...
        if (phase !== "complete") {
          ...
          if (newCount <= 20) {
            ...
            const cancelGuidance = hasValidSessionId && autopilot.state.session_id === sessionId
              ? " When all phases are complete, run /oh-my-grok:cancel to cleanly exit and clean up this session's autopilot state files. If cancel fails, retry with /oh-my-grok:cancel --force."
              : "";
    ...
    if (
      ...
      if (incomplete > 0) {
        ...
        if (newCount <= 20) {
          ...
          let reason = `[ULTRAPILOT] ${incomplete} workers still running. Continue working. When all workers complete, run /oh-my-grok:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-grok:cancel --force.`;
          if (errorGuidance) {
            reason = errorGuidance + reason;
          }
    ...
    if (
      ...
      if (pending > 0) {
        ...
        if (newCount <= 15) {
          ...
          let reason = `[SWARM ACTIVE] ${pending} tasks remain. Continue working. When all tasks are done, run /oh-my-grok:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-grok:cancel --force.`;
          if (errorGuidance) {
            reason = errorGuidance + reason;
          }
    ...
    if (
      ...
      if (currentStage < totalStages) {
        ...
        if (newCount <= 15) {
          ...
          let reason = `[PIPELINE - Stage ${currentStage + 1}/${totalStages}] Pipeline not complete. Continue working. When all stages complete, run /oh-my-grok:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-grok:cancel --force.`;
          if (errorGuidance) {
            reason = errorGuidance + reason;
          }
    ...
    // Priority 6: Team (native Grok Build teams / staged pipeline)
    if (
      team.state?.active &&
      ...
      if (sessionMatches) {
        ...
        if (phase) {
          ...
          if (newCount <= 20) {
            ...
            let reason = `[TEAM - Phase: ${phase}] Team mode active. Continue working. When all team tasks complete, run /oh-my-grok:cancel to cleanly exit. If cancel fails, retry with /oh-my-grok:cancel --force.`;
            if (errorGuidance) {
              reason = errorGuidance + reason;
            }
      ...
      if (sessionMatches) {
        ...
        if (phase) {
          ...
          if (newCount <= 20) {
            ...
            let reason = `[OMG TEAMS - Phase: ${phase}] OMG Teams workers active. Continue working. When all workers complete, run /oh-my-grok:cancel to cleanly exit. If cancel fails, retry with /oh-my-grok:cancel --force.`;
            if (errorGuidance) {
              reason = errorGuidance + reason;
            }
    ...
    if (
      ...
      if (cycle < maxCycles && !ultraqa.state.all_passing) {
        ...
        let reason = `[ULTRAQA - Cycle ${cycle + 1}/${maxCycles}] Tests not all passing. Continue fixing. When all tests pass, run /oh-my-grok:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-grok:cancel --force.`;
        if (errorGuidance) {
          reason = errorGuidance + reason;
        }
    ...
    if (
      ...
      if (totalIncomplete > 0) {
        ...
        reason += ` ${totalIncomplete} incomplete ${itemType} remain. Continue working. When all work is complete, run /oh-my-grok:cancel to cleanly exit ultrawork mode and clean up state files.`;
      } else if (newCount >= 3) {
        // Reinforce clean-exit guidance once no tracked work remains.
        reason += ` If all work is complete, run /oh-my-grok:cancel to cleanly exit ultrawork mode and clean up state files. If cancel fails, retry with /oh-my-grok:cancel --force. Otherwise, continue working.`;
      } else {
        // Early iterations with no tasks yet still need an immediately visible exit path.
        reason += ` No incomplete tasks detected. If all work is complete, run /oh-my-grok:cancel to cleanly exit ultrawork mode and clean up state files. Otherwise, continue working - create Tasks to track your progress.`;
      }
