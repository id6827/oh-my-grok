 ...
 *
 * Creates a fresh workspace in os.tmpdir() with .omg-workspace + sibling sub-repos.
 * Exercises 15 assertions across 11 subsystems + retrofit + rollback scenarios.
 *
 * Usage (from repo root):
 *   node scripts/audit-multirepo-e2e.mjs
 *   GROK_PLUGIN_ROOT=/other/path node scripts/audit-multirepo-e2e.mjs
 */
...
// GROK_PLUGIN_ROOT may point to the npm-installed version which predates
...
function getProbeDir() {
  if (!_probeDir) {
    ...
    try {
      ...
      _probeDir = mkdtempSync('C:\\Temp\\omg-probe-');
    } catch {
      _probeDir = mkdtempSync(join(tmpdir(), 'omg-probe-'));
    }
  }
  return _probeDir;
}
...
function probe(scriptBody, extraEnv = {}) {
  ...
  // Explicitly set GROK_PLUGIN_ROOT to REPO_ROOT so subprocess probes load
  // the local dist, not the globally installed npm package.
  const env = { ...process.env, GROK_PLUGIN_ROOT: PLUGIN_ROOT, ...extraEnv };
  delete env.GROK_PLUGIN_ROOT; // remove first to avoid stale value
  env.GROK_PLUGIN_ROOT = PLUGIN_ROOT;
...
function buildFixture() {
  ...
  try {
    mkdirSync('C:\\Temp', { recursive: true });
    base = mkdtempSync('C:\\Temp\\omg-audit-');
  } catch {
    base = mkdtempSync(join(tmpdir(), 'omg-audit-'));
  }
  console.log(`\n  Fixture root: ${base}\n`);
  writeFileSync(join(base, '.omg-workspace'), JSON.stringify({ id: 'audit' }));
...
`);
    if (!r.ok) {
      ...
      results.push(r.data.dir === expected
        ...
        : fail(1, 'ultragoalDir should resolve under workspace .omg', expected, r.data.dir));
    }
  }
...
`);
    if (!r.ok) results.push(fail(2, `ralph probe error: ${r.error}`));
      ...
      ? pass(2, `ralph effectiveWrite → workspace .omg`)
      : fail(2, 'ralph path should be under workspace .omg', ws_omc + '/...', r.data.write));
  }
...
`);
    if (!r.ok) results.push(fail(3, `ultrawork probe error: ${r.error}`));
      ...
      ? pass(3, `ultrawork effectiveWrite → workspace .omg`)
      : fail(3, 'ultrawork path should be under workspace .omg', ws_omc + '/...', r.data.write));
  }
...
`);
    if (!r.ok) results.push(fail(4, `autopilot probe error: ${r.error}`));
      ...
      ? pass(4, `autopilot effectiveWrite → workspace .omg`)
      : fail(4, 'autopilot path should be under workspace .omg', ws_omc + '/...', r.data.write));
  }
...
`);
    if (!r.ok) results.push(fail(5, `team probe error: ${r.error}`));
      ...
      results.push(write.startsWith(ws_omc) && root === ws_omc
        ? pass(5, `team effectiveWrite + getOmcRoot → workspace .omg`)
        : fail(5, 'team path or getOmcRoot should be workspace .omg', ws_omc,
...
`);
    if (!r.ok) results.push(fail(6, `ralplan probe error: ${r.error}`));
      ...
      ? pass(6, `ralplan effectiveWrite → workspace .omg`)
      : fail(6, 'ralplan path should be under workspace .omg', ws_omc + '/...', r.data.write));
  }
    ...
    if (!existsSync(SI_SCRIPT)) {
      ...
      const sp = spawnSync(
        ...
        { encoding: 'utf-8', timeout: 15000,
          env: { ...process.env, GROK_PLUGIN_ROOT: PLUGIN_ROOT } }
      );
      ...
      if (sp.status !== 0) {
        ...
        try {
          ...
          results.push(parsed.base_root?.startsWith(ws_omc)
            ...
            : fail(7, 'self-improve base_root should be under workspace .omg',
...
`);
    if (!r.ok) results.push(fail(9, `autoresearch probe error: ${r.error}`));
      ...
      : fail(9, 'autoresearch specs should be under workspace .omg',
...
`);
    if (!r.ok) results.push(fail(10, `notepad/memory probe error: ${r.error}`));
      ...
      results.push(notepadPath === expN && memoryPath === expM
        ? pass(10, `notepad + project-memory → workspace .omg`)
        : fail(10, 'notepad/memory paths should be workspace .omg',
  ...
  try {
    mkdirSync('C:\\Temp', { recursive: true });
    retrofitBase = mkdtempSync('C:\\Temp\\omg-audit-retrofit-');
  } catch {
    retrofitBase = mkdtempSync(join(tmpdir(), 'omg-audit-retrofit-'));
  }
  ...
  results.push(existsSync(legacyStateFile)
    ? pass(11, `legacy api/.omg/state/ralph-state.json created before .omg-workspace drop`)
    : fail(11, 'legacy state file should exist before workspace marker drop', legacyStateFile, 'missing'));

  // 12. drop .omg-workspace marker
  writeFileSync(join(retrofitBase, '.omg-workspace'), JSON.stringify({ id: 'retrofit-test' }));
  results.push(existsSync(join(retrofitBase, '.omg-workspace'))
    ? pass(12, `.omg-workspace marker dropped at parent after legacy state exists`)
    : fail(12, '.omg-workspace marker should exist after write'));
...
`);
    ...
    results.push(hasWarning && hasLegacyPath && hasMigrateHint
      ...
      : fail(13,
          'warnSiblingRetrofit should emit warning with legacy .omg path and OMC_MIGRATE_LEGACY_STATE=1 hint',
          'stderr: workspace-retrofit warning + api/.omg path + OMC_MIGRATE_LEGACY_STATE',
...
`, { OMC_DISABLE_MULTIREPO: '1' });
    ...
    if (!r.ok) {
      ...
      if (root !== ws_omc && root.startsWith(join(base, 'api'))) {
        results.push(pass(15, `OMC_DISABLE_MULTIREPO=1 bypasses workspace anchor → api/.omg`));
      } else if (root === ws_omc) {
