...
function checkRalphRubyDependency() {
  try {
    ...
    console.log('[OMG] Ruby detected for Ralph workflows');
  } catch {
    console.log('[OMG] Warning: Ruby was not found on PATH. Ralph workflows require Ruby and may fail until it is installed.');
    console.log('[OMG] Ubuntu/Debian: sudo apt update && sudo apt install ruby-full');
    console.log('[OMG] macOS: brew install ruby');
    console.log('[OMG] After installing Ruby, restart Grok Build and rerun /oh-my-grok:omg-setup if needed.');
  }
}
...
// safe for the platform that is installing the plugin cache. Grok Build's
...
//
// Patterns handled:
//  1. Current find-node.sh format – sh "$GROK_PLUGIN_ROOT"/scripts/find-node.sh ...
//  2. Legacy find-node.sh format – sh "${GROK_PLUGIN_ROOT}/scripts/find-node.sh" ...
...
try {
  ...
  if (hooksJsonPath && existsSync(hooksJsonPath)) {
    ...
    if (patched) {
      ...
      console.log(`[OMG] Patched hooks.json to use ${platformLabel} hook commands`);
    }
  }
} catch (e) {
...
//    which does NOT include node_modules.  When Grok Build extracts the plugin into its
...
console.log('[OMG] Setup complete! Restart Grok Build to activate HUD.');
